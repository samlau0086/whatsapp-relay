...
import {
  ...
  Users, Wifi, WifiOff, X, ClipboardList, ExternalLink, Bot, Brain, BookOpen, MapPin, Copy, CreditCard, LayoutGrid, List, Eye, EyeOff, ReceiptText,
} from "lucide-react";
...
function ContactCreateDialog({accounts,token,onToken,onClose,onSaved}:{accounts:Account[];token:string;onToken:(token:string)=>void;onClose:()=>void;onSaved:()=>Promise<void>}){
  const [accountId,setAccountId]=useState(accounts[0]?.id??""),[name,setName]=useState(""),[phone,setPhone]=useState(""),[avatar,setAvatar]=useState<File|null>(null),[avatarPickerOpen,setAvatarPickerOpen]=useState(false),[busy,setBusy]=useState(false),[error,setError]=useState("");
