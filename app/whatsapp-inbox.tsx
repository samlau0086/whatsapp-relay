...
export function WhatsAppInbox({initialView="inbox"}:{initialView?:WorkspaceView}) {
  ...
  useEffect(()=>{setReplyTo(null);},[effectiveActiveId]);
  ...
  async function queueTextMessage(text:string,translationSourceText?:string){
    ...
    const clientMessageId=crypto.randomUUID(),quoted=replyTo?messageQuote(replyTo):undefined;setDraft("");setReplyTo(null);setTranslationPreview(null);setTranslationError("");
  ...
  async function sendMediaAsset(asset:MediaAsset,caption:string){
    if(!active||!apiToken)return;
    const kind=mediaKind(asset.mimeType),clientMessageId=crypto.randomUUID(),quoted=replyTo?messageQuote(replyTo):undefined;setDraft("");setReplyTo(null);
  ...
  return <main className="relay-shell">
    ...
    {materialLibraryOpen&&active&&<MaterialLibrarySendDialog customerName={active.name} initialCaption={draft} request={taskRequest} onToken={setApiToken} onClose={()=>setMaterialLibraryOpen(false)} onSend={sendMediaAsset}/>}
...
function SettingsPanel({token,role,accounts,onToken,onToast}:{token:string;role:string;accounts:Account[];onToken:(token:string)=>void;onToast:(text:string)=>void}){
  const [tab,setTab]=useState<"cloud"|"agent"|"tasks"|"knowledge"|"translation"|"speech"|"email"|"currency"|"orders">("cloud");
  ...
  return <section className="management-panel settings-panel"><header className="management-head"><div><span className="eyebrow">系统设置</span><h1>工作区配置</h1><p>集中管理 WhatsApp、自动回复、知识库、Provider 和业务规则。</p></div></header><nav className="settings-tabs" aria-label="系统设置"><button className={tab==="cloud"?"active":""} onClick={()=>setTab("cloud")}><Phone size={15}/>WhatsApp API</button><button className={tab==="agent"?"active":""} onClick={()=>setTab("agent")}><Bot size={15}/>AI Agent</button><button className={tab==="tasks"?"active":""} onClick={()=>setTab("tasks")}><Clock3 size={15}/>任务 Agent</button><button className={tab==="knowledge"?"active":""} onClick={()=>setTab("knowledge")}><BookOpen size={15}/>知识库</button><button className={tab==="translation"?"active":""} onClick={()=>setTab("translation")}><Languages size={15}/>AI 翻译</button><button className={tab==="speech"?"active":""} onClick={()=>setTab("speech")}><Mic size={15}/>AI 语音</button><button className={tab==="email"?"active":""} onClick={()=>setTab("email")}><Mail size={15}/>邮件发送</button><button className={tab==="currency"?"active":""} onClick={()=>setTab("currency")}><CreditCard size={15}/>货币管理</button><button className={tab==="orders"?"active":""} onClick={()=>setTab("orders")}><ClipboardList size={15}/>订单设置</button></nav>{tab==="cloud"?<CloudApiSettingsPanel token={token} onToken={onToken} onToast={onToast}/>:tab==="agent"?<AiAgentSettingsPanel token={token} accounts={accounts} onToken={onToken} onToast={onToast}/>:tab==="tasks"?<TaskAgentSettingsPanel token={token} accounts={accounts} onToken={onToken} onToast={onToast}/>:tab==="knowledge"?<KnowledgeBaseSettingsPanel token={token} onToken={onToken} onToast={onToast}/>:tab==="translation"?<TranslationSettingsPanel token={token} onToken={onToken} onToast={onToast}/>:tab==="speech"?<TtsSettingsPanel token={token} role={role} onToken={onToken} onToast={onToast}/>:tab==="email"?<EmailSettingsPanel token={token} onToken={onToken} onToast={onToast}/>:tab==="currency"?<CurrencySettingsPanel token={token} role={role} onToken={onToken} onToast={onToast}/>:<OrderSettingsPanel token={token} onToken={onToken} onToast={onToast}/>}</section>;
}
