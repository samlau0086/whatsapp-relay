...
type OrderItem={id:string;orderNumber:number;productName:string;amount:number;currency:string;description:string;createdAt:string;createdByName:string;messageStatus:string;attachments:Array<{id:string;name:string;mimeType:string}>};
...
export function WhatsAppInbox() {
  ...
  return <main className="relay-shell">
    ...
    {detailsOpen&&active&&<CrmDetailsPanel active={active} token={apiToken} user={user} role={userRole} onToken={setApiToken} onClose={()=>setDetailsOpen(false)} onToast={setToast} onConversationChange={async change=>{await updateConversation(change);}} onChanged={async()=>{await Promise.all([loadWorkspace(apiToken,true),loadMessages(apiToken,active.id)]);}}/>}</>
...
function CrmDetailsPanel({active,token,user,role,onToken,onClose,onToast,onConversationChange,onChanged}:{active:Conversation;token:string;user:User|null;role:string;onToken:(token:string)=>void;onClose:()=>void;onToast:(text:string)=>void;onConversationChange:(change:Record<string,unknown>)=>Promise<void>;onChanged:()=>Promise<void>}){
  ...
  return <><button className="details-backdrop" onClick={onClose} aria-label="关闭联系人详情"/><aside className="details-panel crm-details" aria-label="联系人详情"><header><h3>联系人详情</h3><button onClick={onClose} className="icon-button" aria-label="关闭详情"><X size={17}/></button></header><div className="contact-card"><span className="avatar large" style={{background:active.color}}>{active.initials}</span><h2>{active.name}</h2><p>{active.phone||"号码待同步"}</p><span className="contact-online"><i className={`status-dot ${active.accountStatus==="online"?"online":""}`}/>{statusText(active.accountStatus)}</span></div>{loading?<div className="crm-loading"><RefreshCw className="spin" size={18}/>读取客户资料…</div>:details?<>
    <div className="detail-section crm-section"><div className="detail-title"><h4>订单状态</h4><button onClick={()=>setOrderOpen(true)}><Plus size={12}/>创建订单</button></div>{details.orders.length?<div className="order-list">{details.orders.map(order=><article key={order.id}><span><b>#{String(order.orderNumber).padStart(6,"0")} · {order.productName}</b><small>{order.currency} {order.amount.toFixed(2)} · {formatDateTime(order.createdAt)}</small></span><em className={`delivery-state ${order.messageStatus}`}>{deliveryText(order.messageStatus)}</em></article>)}</div>:<p className="crm-empty">尚未创建订单</p>}</div>
    ...
    <div className="detail-section"><h4>会话信息</h4><dl><div><dt>负责坐席</dt><dd>{active.assignedUserId===user?.id?"我":active.assignedUserId?"其他坐席":"未分配"}</dd></div><div><dt>接入账号</dt><dd>{active.account}</dd></div><div><dt>客户阶段</dt><dd>{stageName(details.customerStage)}</dd></div><div><dt>会话状态</dt><dd className="green-text">{active.conversationStatus==="open"?"进行中":active.conversationStatus==="closed"?"已关闭":"已归档"}</dd></div></dl><button className="conversation-state-button" onClick={()=>void onConversationChange({status:active.conversationStatus==="closed"?"open":"closed"})}>{active.conversationStatus==="closed"?"重新打开会话":"关闭会话"}</button></div></>:null}{error&&<p className="crm-error">{error}</p>}<div className="security-note"><ShieldCheck size={16}/><span><b>中心真实数据</b><small>CRM 资料保存在团队 PostgreSQL 中</small></span></div></aside>{orderOpen&&<OrderDialog active={active} token={token} onToken={onToken} onClose={()=>setOrderOpen(false)} onCreated={async orderNumber=>{setOrderOpen(false);onToast(`订单 #${String(orderNumber).padStart(6,"0")} 已进入发送队列`);await load();await onChanged();}}/>}</>;
}

function OrderDialog({active,token,onToken,onClose,onCreated}:{active:Conversation;token:string;onToken:(token:string)=>void;onClose:()=>void;onCreated:(orderNumber:number)=>Promise<void>}){
  const [productName,setProductName]=useState(""),[amount,setAmount]=useState(""),[currency,setCurrency]=useState("USD"),[description,setDescription]=useState(""),[files,setFiles]=useState<File[]>([]),[busy,setBusy]=useState(false),[error,setError]=useState("");const inputRef=useRef<HTMLInputElement>(null);
  ...
  function choose(next:File[]){const accepted=next.filter(file=>["image/png","image/jpeg"].includes(file.type));if(accepted.length!==next.length){setError("仅支持 PNG 或 JPG 图片");return;}if(accepted.length>3){setError("最多添加 3 张图片");return;}setError("");setFiles(accepted);}
  async function submit(){const value=Number(amount);if(!Number.isFinite(value)||value<=0||!/^\d+(?:\.\d{1,2})?$/.test(amount)){setError("请输入最多两位小数的有效金额");return;}setBusy(true);setError("");try{const mediaIds:string[]=[];for(const file of files){const form=new FormData();form.append("file",file);const uploaded=await authorizedFetch(`/api/v1/media?accountId=${encodeURIComponent(active.accountId)}`,token,{method:"POST",body:form});if(uploaded.token!==token)onToken(uploaded.token);if(!uploaded.response.ok)throw new Error(`${file.name} 上传失败`);const body=await uploaded.response.json() as {mediaId:string};mediaIds.push(body.mediaId);}const created=await authorizedFetch(`/api/v1/conversations/${active.id}/orders`,token,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({clientOrderId:crypto.randomUUID(),productName:productName.trim()||undefined,amount:value,currency,description:description.trim()||undefined,attachmentMediaIds:mediaIds})});if(created.token!==token)onToken(created.token);const body=await created.response.json().catch(()=>({})) as {orderNumber?:number;error?:string};if(!created.response.ok||!body.orderNumber)throw new Error(body.error??`创建失败（HTTP ${created.response.status}）`);await onCreated(body.orderNumber);}catch(reason){setError(reason instanceof Error?reason.message:"创建订单失败");setBusy(false);}}
  return <div className="modal-backdrop order-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget&&!busy)onClose();}}><section className="login-dialog order-dialog" role="dialog" aria-modal="true" aria-labelledby="order-title"><button className="login-close" onClick={onClose} disabled={busy} aria-label="关闭"><X size={17}/></button><span className="login-logo"><ShoppingBag size={20}/></span><h2 id="order-title">创建订单</h2><p>为 {active.name} 创建订单；系统会通过当前 WhatsApp 会话发送订单摘要和附件。</p><label>商品名称 · 选填<input value={productName} onChange={event=>setProductName(event.target.value)} maxLength={120} placeholder="添加商品名称" autoFocus/></label><div className="order-money"><label>金额<input value={amount} onChange={event=>setAmount(event.target.value)} inputMode="decimal" placeholder="0.00"/></label><label>货币<select value={currency} onChange={event=>setCurrency(event.target.value)}>{["USD","CNY","EUR","GBP","JPY","HKD","SGD","AUD","CAD","AED"].map(item=><option key={item}>{item}</option>)}</select></label></div><label>说明 · 选填<textarea value={description} onChange={event=>setDescription(event.target.value)} maxLength={2000} placeholder="添加说明"/></label><label>附件 · 选填<small>最多 3 张 PNG/JPG</small><button type="button" className="order-dropzone" onClick={()=>inputRef.current?.click()} onDragOver={event=>event.preventDefault()} onDrop={event=>{event.preventDefault();choose(Array.from(event.dataTransfer.files));}}><UploadCloud size={22}/><span>{files.length?files.map(file=>file.name).join("、"):"拖放图片或点击选择"}</span></button><input ref={inputRef} hidden type="file" multiple accept="image/png,image/jpeg" onChange={event=>choose(Array.from(event.target.files??[]))}/></label>{error&&<span className="login-error">{error}</span>}<p className="order-disclosure">创建后会立即进入 WhatsApp 持久发送队列。</p><button className="login-submit" disabled={busy||!amount} onClick={()=>void submit()}>{busy?"正在创建…":"创建订单"}</button><small className="dialog-hint">Ctrl / Cmd + Enter 快速提交</small></section></div>;
}
