  ...
  return (
    <main className="relay-shell">
      ...
      <nav className="rail" aria-label="全局导航">
        ...
        <div className="rail-bottom"><button className="rail-button" aria-label="帮助"><CircleHelp size={18} /></button><button className="rail-button" aria-label="设置"><Settings size={18} /></button><button className="profile-button" onClick={()=>setAuthOpen(true)} aria-label="登录与账户"><span className="avatar small coral">林</span></button></div>
      </nav>
...
function LoginDialog({connected,onClose,onLogin,onLogout}:{connected:boolean;onClose:()=>void;onLogin:(token:string)=>void;onLogout:()=>void}){
  const [email,setEmail]=useState("admin@example.com");const [password,setPassword]=useState("");const [error,setError]=useState("");const [busy,setBusy]=useState(false);
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose();}}><section className="login-dialog" role="dialog" aria-modal="true" aria-labelledby="login-title"><button className="login-close" onClick={onClose} aria-label="关闭"><X size={17}/></button><span className="login-logo"><ShieldCheck size={21}/></span><h2 id="login-title">{connected?"中心平台已连接":"登录 RelayDesk"}</h2><p>{connected?"当前工作台正在读取中心平台的真实会话数据。":"使用管理员邀请的坐席账号登录。"}</p>{connected?<button className="login-submit danger" onClick={onLogout}>退出中心平台</button>:<><label>邮箱<input value={email} onChange={(event)=>setEmail(event.target.value)} autoComplete="username"/></label><label>密码<input type="password" value={password} onChange={(event)=>setPassword(event.target.value)} onKeyDown={(event)=>{if(event.key==="Enter")void submit();}} autoComplete="current-password"/></label>{error&&<span className="login-error">{error}</span>}<button className="login-submit" disabled={busy} onClick={()=>void submit()}>{busy?"正在连接...":"登录并加载会话"}</button></>}</section></div>;
}
