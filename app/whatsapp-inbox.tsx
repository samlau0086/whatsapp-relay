...
type ContactProfile={id:string;accountId:string;accountName:string;alias:string;contactName:string;name:string;phone:string;note:string;emails:ContactEmail[];primaryEmail:string|null;methods:ContactMethod[];addresses:CustomerAddress[];conversationId:string|null;lastMessageAt:string|null;updatedAt:string};
...
export function WhatsAppInbox() {
  ...
  return <main className="relay-shell">
    ...
    {detailsOpen&&active&&<CrmDetailsPanel key={active.id} active={active} token={apiToken} user={user} role={userRole} translationPreference={translationPreference} onToken={setApiToken} onClose={()=>setDetailsOpen(false)} onToast={setToast} onConversationChange={async change=>{await updateConversation(change);}} onChanged={async()=>{await Promise.all([loadWorkspace(apiToken,true),loadMessages(apiToken,active.id)]);}} onDeleted={async()=>{setDetailsOpen(false);setActiveId("");setMessages(all=>{const next={...all};delete next[active.id];return next;});await loadWorkspace(apiToken,true);}}/>}</>
      :view==="contacts"?<ContactManagement token={apiToken} accounts={accounts} onToken={setApiToken} onToast={setToast} onConversation={conversationId=>{const found=conversations.find(item=>item.id===conversationId);if(!found){setToast("该会话不在当前列表，请在消息中心搜索联系人");openInbox();return;}setActiveId(conversationId);setDetailsOpen(true);openInbox();}}/>
...
function ContactManagement({token,accounts,onToken,onToast,onConversation}:{token:string;accounts:Account[];onToken:(token:string)=>void;onToast:(text:string)=>void;onConversation:(conversationId:string)=>void}){
  const [contacts,setContacts]=useState<ContactProfile[]>([]),[query,setQuery]=useState(""),[accountId,setAccountId]=useState(""),[offset,setOffset]=useState(0),[total,setTotal]=useState(0),[loading,setLoading]=useState(true),[error,setError]=useState(""),[editingId,setEditingId]=useState<string|null>(null),[addressEditingId,setAddressEditingId]=useState<string|null>(null);const pageSize=30;
  ...
  return <section className="management-panel contact-management"><header className="management-head"><div><span className="eyebrow">团队联系人目录</span><h1>联系人管理</h1><p>集中维护联系人邮箱、其他联系方式和业务备注；联系人仍由 WhatsApp 会话自动建立。</p></div><button className="secondary-action" onClick={()=>void load()}><RefreshCw size={15}/>刷新</button></header>
...
function ContactEditDialog({contactId,token,onToken,onClose,onSaved}:{contactId:string;token:string;onToken:(token:string)=>void;onClose:()=>void;onSaved:(profile:ContactProfile)=>Promise<void>}){
  const [profile,setProfile]=useState<ContactProfile|null>(null),[alias,setAlias]=useState(""),[note,setNote]=useState(""),[emails,setEmails]=useState<ContactEmail[]>([]),[methods,setMethods]=useState<ContactMethod[]>([]),[loading,setLoading]=useState(true),[busy,setBusy]=useState(false),[error,setError]=useState("");
  useEffect(()=>{let cancelled=false;void (async()=>{try{const result=await authorizedFetch(`/api/v1/contacts/${contactId}`,token);if(result.token!==token)onToken(result.token);if(!result.response.ok)throw new Error(`联系人加载失败（HTTP ${result.response.status}）`);const next=mapContactProfile(await result.response.json() as Record<string,unknown>);if(cancelled)return;setProfile(next);setAlias(next.alias);setNote(next.note);setEmails(next.emails);setMethods(next.methods);}catch(reason){if(!cancelled)setError(reason instanceof Error?reason.message:"联系人加载失败");}finally{if(!cancelled)setLoading(false);}})();return()=>{cancelled=true;};},[contactId,token,onToken]);
