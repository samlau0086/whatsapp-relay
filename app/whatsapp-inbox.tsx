...
type ProactiveTemplate={id:string;language:string;body:string};
...
function ProactiveOutreachSettingsPanel({token,accounts,onToken,onToast}:{token:string;accounts:Account[];onToken:(token:string)=>void;onToast:(text:string)=>void}){
  const defaultTemplate="Hi {{contactName}}, just checking in. Is there anything I can help with?";
  const [accountId,setAccountId]=useState(accounts[0]?.id??""),[loading,setLoading]=useState(false),[saving,setSaving]=useState(false),[scanning,setScanning]=useState(false),[error,setError]=useState(""),[settings,setSettings]=useState({enabled:false,maxTouchesPerYear:5,maxTouchesPerDay:20,localSendStart:"10:00",localSendEnd:"17:00",templates:[{id:"default",language:"",body:defaultTemplate}] as ProactiveTemplate[],holidays:[] as ProactiveHoliday[]});
  const readTemplates=(value:unknown):ProactiveTemplate[]=>{const collect=(input:unknown,language=""):ProactiveTemplate[]=>{if(typeof input==="string")return input.trim()?([{id:`${language||"default"}-${Date.now().toString(36)}`,language,body:input.trim()}]):[];if(Array.isArray(input))return input.flatMap(item=>collect(item));if(!input||typeof input!=="object")return[];const record=input as Record<string,unknown>,body=String(record.body??record.text??record.content??"").trim();if(body)return[{id:String(record.id??`${language||record.language||"default"}-${Date.now().toString(36)}`),language:String(record.language??language??""),body}];if(Array.isArray(record.templates))return record.templates.flatMap(item=>collect(item));return Object.entries(record).flatMap(([key,item])=>key==="templates"?[]:collect(item,key==="default"?"":key));};return collect(value);};
