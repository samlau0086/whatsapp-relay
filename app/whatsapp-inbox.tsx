...
type ContactProfile={id:string;accountId:string;accountName:string;alias:string;contactName:string;name:string;phone:string;note:string;emails:ContactEmail[];primaryEmail:string|null;methods:ContactMethod[];conversationId:string|null;lastMessageAt:string|null;updatedAt:string};
    ...
    <>
      ...
      <aside className="details-panel crm-details" aria-label="联系人详情">
        ...
        <div className="contact-card">
          ...
          <button className="contact-profile-edit" onClick={()=>setContactEditing(true)}><Pencil size={13}/>编辑联系人</button>
        </div>
...
function ContactManagement({token,accounts,onToken,onToast,onConversation}:{token:string;accounts:Account[];onToken:(token:string)=>void;onToast:(text:string)=>void;onConversation:(conversationId:string)=>void}){
  const [contacts,setContacts]=useState<ContactProfile[]>([]),[query,setQuery]=useState(""),[accountId,setAccountId]=useState(""),[offset,setOffset]=useState(0),[total,setTotal]=useState(0),[loading,setLoading]=useState(true),[error,setError]=useState(""),[editingId,setEditingId]=useState<string|null>(null);const pageSize=30;
