...
function AgentConversationBar({conversationId,token,refreshKey,onToken,onToast,onUseDraft,onSent}:{conversationId:string;token:string;refreshKey:string;onToken:(token:string)=>void;onToast:(text:string)=>void;onUseDraft:(text:string)=>void;onSent:()=>void}){
  ...
  async function resolveDraft(action:"send"|"dismiss"){if(!state?.draft)return;const draftId=String(state.draft.id);hiddenDraftIdsRef.current.add(draftId);setState(current=>current?{...current,draft:null}:current);setBusy(true);const result=await authorizedFetch(`/api/v1/ai-drafts/${draftId}/${action}`,token,{method:"POST",headers:{"content-type":"application/json"},body:action==="send"?JSON.stringify({text:state.draft.text_content}):undefined});if(result.token!==token)onToken(result.token);setBusy(false);if(result.response.ok){onToast(action==="send"?"AI 草稿已进入发送队列":"AI 草稿已忽略");await load();onSent();}else{onToast(action==="send"?"AI 草稿发送失败":"AI 草稿已隐藏，请稍后重试");}}
