...
function AgentConversationBar({conversationId,token,refreshKey,onToken,onToast,onUseDraft,onSent}:{conversationId:string;token:string;refreshKey:string;onToken:(token:string)=>void;onToast:(text:string)=>void;onUseDraft:(text:string)=>void;onSent:()=>void}){
  const [state,setState]=useState<{mode:string;account_enabled:boolean;draft:AgentDraft|null}|null>(null),[busy,setBusy]=useState(false),[blockedSession,setBlockedSession]=useState(""),loadSequenceRef=useRef(0);
  const sessionKey=`${conversationId}:${token}`,authenticationFailed=blockedSession===sessionKey;
  const load=useCallback(async()=>{const requestSequence=++loadSequenceRef.current;try{const result=await authorizedFetch(`/api/v1/conversations/${conversationId}/agent`,token);if(requestSequence!==loadSequenceRef.current)return;if(result.token!==token)onToken(result.token);if(result.response.status===401){setBlockedSession(sessionKey);return;}if(result.response.ok){setBlockedSession("");setState(await result.response.json());}}catch{}},[conversationId,token,onToken,sessionKey]);
  ...
  async function resolveDraft(action:"send"|"dismiss"){if(!state?.draft)return;setBusy(true);const result=await authorizedFetch(`/api/v1/ai-drafts/${state.draft.id}/${action}`,token,{method:"POST",headers:{"content-type":"application/json"},body:action==="send"?JSON.stringify({text:state.draft.text_content}):undefined});if(result.token!==token)onToken(result.token);setBusy(false);if(result.response.ok){setState(current=>current?{...current,draft:null}:current);onToast(action==="send"?"AI 草稿已进入发送队列":"AI 草稿已忽略");await load();onSent();}}
  async function handleUseDraft(){if(!state?.draft)return;const text=state.draft.text_content;setBusy(true);const result=await authorizedFetch(`/api/v1/ai-drafts/${state.draft.id}/dismiss`,token,{method:"POST"});if(result.token!==token)onToken(result.token);setBusy(false);if(result.response.ok){setState(current=>current?{...current,draft:null}:current);onUseDraft(text);onToast("回复建议已放入输入框，可继续编辑后发送");await load();onSent();}}
