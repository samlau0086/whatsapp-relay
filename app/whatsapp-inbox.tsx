...
type ProductItem={id:string;sku:string;name:string;defaultUnitAmount:number;priceTiers:ProductPriceTier[];currency:string;imageMediaId:string|null;imageName:string;tags:TagItem[];createdAt:string;updatedAt:string};
...
function CurrencySettingsPanel({token,role,onToken,onToast}:{token:string;role:string;onToken:(token:string)=>void;onToast:(text:string)=>void}){
  const [config,setConfig]=useState<CurrencyConfig>(DEFAULT_CURRENCY_CONFIG),[loading,setLoading]=useState(true),[saving,setSaving]=useState(false),[refreshingRates,setRefreshingRates]=useState(false),[error,setError]=useState("");
  ...
  async function refreshRates(){const codes=config.currencies.map(item=>item.code.trim().toUpperCase());if(!config.currencies.length||codes.some(code=>! /^[A-Z]{3}$/.test(code))||new Set(codes).size!==codes.length||config.currencies.some(item=>!item.name.trim())){setError("更新汇率前，请先填写不重复的三位币种代码和显示名称");return;}setRefreshingRates(true);setError("");try{const payload={baseCurrency:config.baseCurrency,currencies:config.currencies.map(item=>({...item,code:item.code.toUpperCase(),rate:item.code===config.baseCurrency?1:Math.max(Number(item.rate)||1,0.00000001)}))},result=await authorizedFetch("/api/v1/admin/currencies/refresh-rates",token,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(payload)});if(result.token!==token)onToken(result.token);const body=await result.response.json().catch(()=>({})) as CurrencyConfig&{message?:string;error?:string;source?:string;rateDate?:string;updatedCount?:number};if(!result.response.ok)throw new Error(body.message??(result.response.status===403?"只有管理员可以更新汇率":body.error)??`HTTP ${result.response.status}`);setConfig({baseCurrency:body.baseCurrency,currencies:body.currencies});onToast(`已从 ${body.source??"公共汇率服务"} 更新并保存 ${body.updatedCount??Math.max(0,body.currencies.length-1)} 个汇率${body.rateDate?`（数据日期 ${body.rateDate}）`:""}`);}catch(reason){setError(reason instanceof Error?reason.message:"公共汇率更新失败");}finally{setRefreshingRates(false);}}
