...
type ContactProfile={id:string;accountId:string;accountName:string;alias:string;contactName:string;name:string;phone:string;avatarUrl:string|null;note:string;birthday:ContactDate|null;specialDates:ContactSpecialDate[];emails:ContactEmail[];primaryEmail:string|null;methods:ContactMethod[];addresses:CustomerAddress[];conversationId:string|null;hasConversation:boolean;lastMessageAt:string|null;updatedAt:string};
...
function CrmDetailsPanel({
  ...
  async function sendOrder(order: OrderItem, format: "text" | "image", translate: boolean, targetLanguage?: string, email?:{recipientEmailIds:string[];subject:string;messageBody:string}) {
    ...
    try {
      ...
      onToast(
        `订单 #${order.orderNumber} 已${order.status === "draft" ? "按" : "重新按"}${translate?languageName(targetLanguage??order.targetLanguage):"英文"}${format === "image" ? "完整图片版" : "文字版"}进入${email?"邮件":"WhatsApp"}发送队列`,
      );
      await load();
      await onChanged();
    } catch (reason) {
    ...
    <>
      ...
      <aside className="details-panel crm-details" aria-label="联系人详情">
        ...
        <div className="contact-card">
          ...
          {details?.contact?.methods.slice(0,2).map((method,index)=><p className="contact-method-summary" key={`${method.type}-${index}`}>{method.label||contactMethodName(method.type)}：{method.value}</p>)}
...
function OrderSendDialog({order,emails,defaultTargetLanguage,busy,onClose,onSend}:{order:OrderItem;emails:Array<ContactEmail&{id:string}>;defaultTargetLanguage:string;busy:boolean;onClose:()=>void;onSend:(format:"text"|"image",translate:boolean,targetLanguage?:string,email?:{recipientEmailIds:string[];subject:string;messageBody:string})=>void}){
  const initialTargetLanguage=defaultTargetLanguage||"en";
  const [format,setFormat]=useState<"text"|"image">("text"),[translate,setTranslate]=useState(!isEnglishLanguage(initialTargetLanguage)),[targetLanguage,setTargetLanguage]=useState(initialTargetLanguage),[channel,setChannel]=useState<"whatsapp"|"email">("whatsapp"),[recipientIds,setRecipientIds]=useState(emails.filter(item=>item.isPrimary).map(item=>item.id)),[subject,setSubject]=useState(`Order #${order.orderNumber}`),[messageBody,setMessageBody]=useState("Hi,\n\nPlease find your order details below.\n\nBest regards,");
...
function SettingsPanel({token,role,accounts,onToken,onToast}:{token:string;role:string;accounts:Account[];onToken:(token:string)=>void;onToast:(text:string)=>void}){
  const [tab,setTab]=useState<"agent"|"tasks"|"knowledge"|"translation"|"speech"|"email"|"currency"|"orders">("agent");
  ...
  return <section className="management-panel settings-panel"><header className="management-head"><div><span className="eyebrow">系统设置</span><h1>工作区配置</h1><p>集中管理自动回复、知识库、Provider 和业务规则。</p></div></header><nav className="settings-tabs" aria-label="系统设置"><button className={tab==="agent"?"active":""} onClick={()=>setTab("agent")}><Bot size={15}/>AI Agent</button><button className={tab==="tasks"?"active":""} onClick={()=>setTab("tasks")}><Clock3 size={15}/>任务 Agent</button><button className={tab==="knowledge"?"active":""} onClick={()=>setTab("knowledge")}><BookOpen size={15}/>知识库</button><button className={tab==="translation"?"active":""} onClick={()=>setTab("translation")}><Languages size={15}/>AI 翻译</button><button className={tab==="speech"?"active":""} onClick={()=>setTab("speech")}><Mic size={15}/>AI 语音</button><button className={tab==="email"?"active":""} onClick={()=>setTab("email")}><Mail size={15}/>邮件发送</button><button className={tab==="currency"?"active":""} onClick={()=>setTab("currency")}><CreditCard size={15}/>货币管理</button><button className={tab==="orders"?"active":""} onClick={()=>setTab("orders")}><ClipboardList size={15}/>订单设置</button></nav>{tab==="agent"?<AiAgentSettingsPanel token={token} accounts={accounts} onToken={onToken} onToast={onToast}/>:tab==="tasks"?<TaskAgentSettingsPanel token={token} accounts={accounts} onToken={onToken} onToast={onToast}/>:tab==="knowledge"?<KnowledgeBaseSettingsPanel token={token} onToken={onToken} onToast={onToast}/>:tab==="translation"?<TranslationSettingsPanel token={token} onToken={onToken} onToast={onToast}/>:tab==="speech"?<TtsSettingsPanel token={token} role={role} onToken={onToken} onToast={onToast}/>:tab==="email"?<EmailSettingsPanel token={token} onToken={onToken} onToast={onToast}/>:tab==="currency"?<CurrencySettingsPanel token={token} role={role} onToken={onToken} onToast={onToast}/>:<OrderSettingsPanel token={token} onToken={onToken} onToast={onToast}/>}</section>;
}
...
function TaskAgentSettingsPanel({token,accounts,onToken,onToast}:{token:string;accounts:Account[];onToken:(token:string)=>void;onToast:(text:string)=>void}){
  ...
  const [accountId,setAccountId]=useState(accounts[0]?.id??""),[settings,setSettings]=useState({holidayRegions:["global"],enabledHolidays:["new_year","valentines","halloween","christmas"],defaultLeadDays:14,draftLeadHours:72,defaultSendMode:"approval",leapDayPolicy:"feb28",defaultTools:tools.filter(tool=>tool!=="queue_message")}),[loading,setLoading]=useState(false);
  const load=useCallback(async()=>{if(!accountId)return;setLoading(true);const result=await authorizedFetch(`/api/v1/accounts/${accountId}/task-settings`,token);if(result.token!==token)onToken(result.token);if(result.response.ok){const body=await result.response.json() as Record<string,unknown>;setSettings({holidayRegions:(body.holiday_regions as string[])??["global"],enabledHolidays:(body.enabled_holidays as string[])??[],defaultLeadDays:Number(body.default_lead_days??14),draftLeadHours:Number(body.draft_lead_hours??72),defaultSendMode:String(body.default_send_mode??"approval"),leapDayPolicy:String(body.leap_day_policy??"feb28"),defaultTools:(body.default_tools as string[])??[]});}setLoading(false);},[accountId,token,onToken]);useEffect(()=>{const timer=setTimeout(()=>void load(),0);return()=>clearTimeout(timer);},[load]);
  async function save(){setLoading(true);const result=await authorizedFetch(`/api/v1/accounts/${accountId}/task-settings`,token,{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify(settings)});if(result.token!==token)onToken(result.token);setLoading(false);onToast(result.response.ok?"任务 Agent 规则已保存":"任务 Agent 规则保存失败");}
  const holidays=[["new_year","新年"],["valentines","情人节"],["halloween","万圣节"],["christmas","圣诞节"]];
  return <div className="agent-settings-grid"><section className="provider-form agent-card"><header><div><h2>节日与日期任务</h2><p>系统提前创建任务，临近发送时再读取最新资料生成草稿。</p></div></header><label>WhatsApp 账号<select value={accountId} onChange={event=>setAccountId(event.target.value)}>{accounts.map(account=><option key={account.id} value={account.id}>{account.name}</option>)}</select></label><div className="provider-form-grid"><label>默认提前天数<input type="number" min="0" max="365" value={settings.defaultLeadDays} onChange={event=>setSettings(value=>({...value,defaultLeadDays:Number(event.target.value)}))}/></label><label>草稿提前小时<input type="number" min="0" max="8760" value={settings.draftLeadHours} onChange={event=>setSettings(value=>({...value,draftLeadHours:Number(event.target.value)}))}/></label><label>默认发送策略<select value={settings.defaultSendMode} onChange={event=>setSettings(value=>({...value,defaultSendMode:event.target.value}))}><option value="approval">审批后发送</option><option value="auto">自动发送</option></select></label><label>2 月 29 日策略<select value={settings.leapDayPolicy} onChange={event=>setSettings(value=>({...value,leapDayPolicy:event.target.value}))}><option value="feb28">非闰年于 2 月 28 日</option><option value="mar1">非闰年于 3 月 1 日</option><option value="leap_year_only">仅闰年创建</option></select></label></div><fieldset className="kb-assignment"><legend>启用节日</legend>{holidays.map(([id,label])=><label key={id}><input type="checkbox" checked={settings.enabledHolidays.includes(id)} onChange={event=>setSettings(value=>({...value,enabledHolidays:event.target.checked?[...value.enabledHolidays,id]:value.enabledHolidays.filter(item=>item!==id)}))}/>{label}</label>)}</fieldset></section><section className="provider-form agent-card"><header><div><h2>工具权限</h2><p>任务可继承这些账号默认权限；高风险发送工具需明确开启。</p></div></header><fieldset className="task-tools">{tools.map(tool=><label key={tool} className={tool==="queue_message"?"risk":""}><input type="checkbox" checked={settings.defaultTools.includes(tool)} onChange={event=>setSettings(value=>({...value,defaultTools:event.target.checked?[...value.defaultTools,tool]:value.defaultTools.filter(item=>item!==tool)}))}/><span><b>{names[tool]}</b>{tool==="queue_message"&&<small>允许自动任务进入 WhatsApp 队列</small>}</span></label>)}</fieldset><button className="primary-action" disabled={loading||!accountId} onClick={()=>void save()}><Check size={14}/>{loading?"正在保存…":"保存任务 Agent 规则"}</button></section></div>;
}
...
function ContactManagement({token,role,accounts,onToken,onToast,onConversation}:{token:string;role:string;accounts:Account[];onToken:(token:string)=>void;onToast:(text:string)=>void;onConversation:(conversationId:string)=>void}){
  const [contacts,setContacts]=useState<ContactProfile[]>([]),[query,setQuery]=useState(""),[accountId,setAccountId]=useState(""),[offset,setOffset]=useState(0),[total,setTotal]=useState(0),[loading,setLoading]=useState(true),[error,setError]=useState(""),[creating,setCreating]=useState(false),[editingId,setEditingId]=useState<string|null>(null),[addressEditingId,setAddressEditingId]=useState<string|null>(null);const pageSize=30;
  ...
  return <section className="management-panel contact-management"><header className="management-head"><div><span className="eyebrow">团队联系人目录</span><h1>联系人管理</h1><p>新建联系人并维护头像、WhatsApp 号码、邮箱、其他联系方式和业务备注。</p></div><div><button className="secondary-action" onClick={()=>void load()}><RefreshCw size={15}/>刷新</button><button className="primary-action" onClick={()=>setCreating(true)} disabled={!accounts.length}><UserPlus size={15}/>新建联系人</button></div></header>
    ...
    {creating&&<ContactCreateDialog accounts={accounts} token={token} onToken={onToken} onClose={()=>setCreating(false)} onSaved={async()=>{setCreating(false);setOffset(0);onToast("联系人已创建");await load();}}/>}
...
function ContactAddressDialog({contactId,token,onToken,onClose,onSaved}:{contactId:string;token:string;onToken:(token:string)=>void;onClose:()=>void;onSaved:(profile:ContactProfile)=>Promise<void>}){
  ...
  function addAddress(){setAddresses(items=>[...items,{id:"",label:"收货地址",recipientName:profile?.name??"",phone:profile?.phone??"",address:""}]);}
  ...
  async function save(){if(!profile)return;if(addresses.some(item=>!item.label.trim()||!item.address.trim())){setError("请填写地址名称和详细地址，或移除未完成的地址");return;}setBusy(true);setError("");try{const result=await authorizedFetch(`/api/v1/contacts/${contactId}`,token,{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({alias:profile.alias,note:profile.note,emails:profile.emails.map(({label,email,isPrimary})=>({label,email,isPrimary})),methods:profile.methods.map(({type,label,value})=>({type,label,value})),addresses:addresses.map(item=>({...(item.id?{id:item.id}:{}),label:item.label,recipientName:item.recipientName,phone:item.phone,address:item.address}))})});if(result.token!==token)onToken(result.token);const body=await result.response.json().catch(()=>({})) as Record<string,unknown>&{error?:string};if(!result.response.ok)throw new Error(body.error??`地址保存失败（HTTP ${result.response.status}）`);await onSaved(mapContactProfile(body));}catch(reason){setError(reason instanceof Error?reason.message:"地址保存失败");setBusy(false);}}
    ...
    <section className="login-dialog address-dialog" role="dialog" aria-modal="true" aria-labelledby="address-dialog-title">
      ...
      {loading?<div className="contact-dialog-loading"><RefreshCw className="spin" size={17}/>正在读取地址…</div>:<>
        ...
        <div className="contact-address-list">{addresses.map((item,index)=><article key={item.id||`new-${index}`}>
          <header><MapPin size={15}/><b>地址 {index+1}</b><button type="button" onClick={()=>setAddresses(items=>items.filter((_,itemIndex)=>itemIndex!==index))} aria-label="移除地址"><Trash2 size={14}/></button></header>
...
function ContactEditDialog({contactId,token,onToken,onClose,onSaved}:{contactId:string;token:string;onToken:(token:string)=>void;onClose:()=>void;onSaved:(profile:ContactProfile)=>Promise<void>}){
  const [profile,setProfile]=useState<ContactProfile|null>(null),[alias,setAlias]=useState(""),[phone,setPhone]=useState(""),[note,setNote]=useState(""),[birthday,setBirthday]=useState<ContactDate|null>(null),[specialDates,setSpecialDates]=useState<ContactSpecialDate[]>([]),[emails,setEmails]=useState<ContactEmail[]>([]),[methods,setMethods]=useState<ContactMethod[]>([]),[avatarFile,setAvatarFile]=useState<File|null>(null),[removeAvatar,setRemoveAvatar]=useState(false),[loading,setLoading]=useState(true),[busy,setBusy]=useState(false),[error,setError]=useState("");
  ...
  useEffect(()=>{let cancelled=false;void (async()=>{try{const result=await authorizedFetch(`/api/v1/contacts/${contactId}`,token);if(result.token!==token)onToken(result.token);if(!result.response.ok)throw new Error(`联系人加载失败（HTTP ${result.response.status}）`);const next=mapContactProfile(await result.response.json() as Record<string,unknown>);if(cancelled)return;setProfile(next);setAlias(next.alias);setPhone(next.phone);setNote(next.note);setBirthday(next.birthday);setSpecialDates(next.specialDates);setEmails(next.emails);setMethods(next.methods);}catch(reason){if(!cancelled)setError(reason instanceof Error?reason.message:"联系人加载失败");}finally{if(!cancelled)setLoading(false);}})();return()=>{cancelled=true;};},[contactId,token,onToken]);
...
function OrderManagement({token,accounts,onToken,onToast,onConversation}:{token:string;accounts:Account[];onToken:(token:string)=>void;onToast:(text:string)=>void;onConversation:(conversationId:string)=>void}){
  ...
  return <section className="management-panel order-management"><header className="management-head"><div><span className="eyebrow">会话订单中心</span><h1>订单管理</h1><p>集中查看、编辑和删除从客户会话中创建的订单。</p></div><button className="secondary-action" onClick={()=>void load(true)}><RefreshCw size={15}/>刷新</button></header>
    ...
    {editing&&<OrderDialog order={editing} active={{id:editing.conversationId,name:editing.customerName,initials:"",color:"#477a62",account:editing.accountName,accountId:editing.accountId,phone:editing.customerPhone,contactId:"",alias:"",contactName:editing.customerName,primaryEmail:"",contactMethods:[],preview:"",time:"",unread:0,accountStatus:"online",assignedUserId:null,favorite:false,conversationStatus:"open",customerStage:"new",tags:[],remindAt:null}} token={token} onToken={onToken} onClose={()=>setEditing(null)} onCreated={async orderNumber=>{setEditing(null);onToast(`订单 #${orderNumber} 已更新`);cursorRef.current=null;await load(true);}}/>}
  </section>;
}
...
function ProductManagement({token,role,onToken,onToast,onOpenMaterials}:{token:string;role:string;onToken:(token:string)=>void;onToast:(text:string)=>void;onOpenMaterials:()=>void}){
  const [products,setProducts]=useState<ProductItem[]>([]),[currencyConfig,setCurrencyConfig]=useState<CurrencyConfig>(DEFAULT_CURRENCY_CONFIG),[loading,setLoading]=useState(true),[error,setError]=useState(""),[query,setQuery]=useState(""),[currency,setCurrency]=useState(""),[tag,setTag]=useState(""),[tagNames,setTagNames]=useState<string[]>([]),[page,setPage]=useState(1),[jumpPage,setJumpPage]=useState(""),[total,setTotal]=useState(0),[view,setView]=useState<"card"|"list">("card"),[editing,setEditing]=useState<ProductItem|"new"|null>(null),[importing,setImporting]=useState(false),[selected,setSelected]=useState<string[]>([]),[bulkEditing,setBulkEditing]=useState(false),[generating,setGenerating]=useState(false);const loadVersion=useRef(0);
  const load=useCallback(async()=>{const version=++loadVersion.current;setLoading(true);try{const params=new URLSearchParams({limit:String(PRODUCT_PAGE_SIZE),offset:String((page-1)*PRODUCT_PAGE_SIZE)});if(query.trim())params.set("q",query.trim());if(currency)params.set("currency",currency);if(tag)params.set("tag",tag);const [result,currencyResult]=await Promise.all([authorizedFetch(`/api/v1/products?${params}`,token),authorizedFetch("/api/v1/currencies",token)]);if(version!==loadVersion.current)return;if(result.token!==token)onToken(result.token);else if(currencyResult.token!==token)onToken(currencyResult.token);if(!result.response.ok)throw new Error(`产品库加载失败（HTTP ${result.response.status}）`);const body=await result.response.json() as {data:Array<Record<string,unknown>>;total:number;tags:string[]};if(page>1&&!body.data.length){setPage(page-1);return;}setProducts(body.data.map(mapProduct));setTotal(body.total);setTagNames(body.tags);if(currencyResult.response.ok)setCurrencyConfig(await currencyResult.response.json() as CurrencyConfig);setError("");}catch(reason){if(version===loadVersion.current)setError(reason instanceof Error?reason.message:"产品库加载失败");}finally{if(version===loadVersion.current)setLoading(false);}},[token,onToken,page,query,currency,tag]);
  ...
  useEffect(()=>{const timer=window.setTimeout(()=>void load(),query?250:0);return()=>window.clearTimeout(timer);},[load,query]);
  const pageCount=Math.max(1,Math.ceil(total/PRODUCT_PAGE_SIZE)),pageItems=productPaginationItems(page,pageCount),visible=products;
  ...
  return <section className="management-panel product-management"><header className="management-head"><div><span className="eyebrow">团队共享目录</span><h1>产品库</h1><p>集中维护产品 SKU、阶梯价格、图片和标签，创建订单或发送产品卡片时直接选用。</p></div><div><button className="secondary-action" onClick={()=>void load()}><RefreshCw size={15}/>刷新</button><button className="secondary-action" onClick={()=>setImporting(true)}><UploadCloud size={15}/>CSV 导入</button><button className="primary-action" onClick={()=>setEditing("new")}><Plus size={15}/>新增产品</button></div></header>
    <div className="product-filters"><label><Search size={14}/><input value={query} onChange={event=>{setQuery(event.target.value);setPage(1);}} placeholder="搜索产品名称、SKU 或描述"/></label><select value={currency} onChange={event=>{setCurrency(event.target.value);setPage(1);}} aria-label="按币种筛选"><option value="">全部币种</option>{currencyConfig.currencies.map(item=><option key={item.code} value={item.code}>{item.code} · {item.name}</option>)}</select><select value={tag} onChange={event=>{setTag(event.target.value);setPage(1);}} aria-label="按标签筛选"><option value="">全部标签</option>{tagNames.map(item=><option key={item}>{item}</option>)}</select><div className="product-view-tools"><span>共 {total} 个产品</span><div role="group" aria-label="产品显示方式"><button className={view==="card"?"active":""} onClick={()=>setView("card")} aria-label="卡片视图" title="卡片视图"><LayoutGrid size={14}/></button><button className={view==="list"?"active":""} onClick={()=>setView("list")} aria-label="列表视图" title="列表视图"><List size={14}/></button></div></div></div>
    ...
    {generating&&<CollageGenerateDialog productIds={selected} request={collageRequest} onClose={()=>setGenerating(false)} onGenerated={()=>{setGenerating(false);setSelected([]);onToast("拼图素材已生成");onOpenMaterials();}}/>}
