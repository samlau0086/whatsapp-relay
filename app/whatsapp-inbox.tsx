...
type MessengerOAuthSession={id:string;status:"pending"|"processing"|"pages_ready"|"completed"|"failed";expiresAt:string;lastError:string|null};
function MessengerSettingsPanel({token,onToken,onToast}:{token:string;onToken:(token:string)=>void;onToast:(text:string)=>void}){
  ...
  useEffect(()=>{const timer=setTimeout(()=>void load(),0);return()=>clearTimeout(timer);},[load]);
  const loadCandidates=useCallback(async(sessionId:string)=>{const result=await authorizedFetch(`/api/v1/admin/messenger/oauth/sessions/${sessionId}`,token);if(result.token!==token)onToken(result.token);const body=await result.response.json().catch(()=>({})) as {session?:MessengerOAuthSession;pages?:MessengerOAuthCandidate[];error?:string;message?:string};if(!result.response.ok||!body.pages||!body.session){setError(body.message??body.error??`HTTP ${result.response.status}`);return "failed" as const;}if(body.session.status==="pending"||body.session.status==="processing")return body.session.status;setOauthSessionId(sessionId);if(body.session.status==="failed"){setError(body.session.lastError??"Facebook 授权失败");return body.session.status;}setCandidates(body.pages);setSelectedPages(body.pages.map(page=>page.pageId));setError(body.pages.length?"":body.session.lastError??"Facebook 没有返回可管理的 Page。");return body.session.status;},[token,onToken]);
  ...
  async function startOAuth(){setBusy(true);setError("");const result=await authorizedFetch("/api/v1/admin/messenger/oauth/start",token,{method:"POST"});if(result.token!==token)onToken(result.token);const body=await result.response.json().catch(()=>({})) as {authorizationUrl?:string;sessionId?:string;error?:string;message?:string};setBusy(false);if(!result.response.ok||!body.authorizationUrl||!body.sessionId){setError(body.message??body.error??`HTTP ${result.response.status}`);return;}const popup=window.open(body.authorizationUrl,"relaydesk-meta-oauth","popup=yes,width=640,height=760");if(!popup){setError("浏览器阻止了 Facebook 授权弹窗，请允许本站打开弹窗后重试。");return;}setOauthSessionId(body.sessionId);for(let attempt=0;attempt<150;attempt++){await new Promise(resolve=>window.setTimeout(resolve,2000));const status=await loadCandidates(body.sessionId);if(status==="pages_ready"||status==="failed")return;}setError("等待 Facebook 授权结果超时，请关闭弹窗后重试。");}
