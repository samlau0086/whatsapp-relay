...
type OrderItem={id:string;orderNumber:number;amount:number;currency:string;description:string;status:string;translateOnSend:boolean;targetLanguage:string;createdAt:string;createdByName:string;messageStatus:string;items:OrderProductItem[];fees:OrderFeeItem[]};
...
function CrmDetailsPanel({active,token,user,role,translationPreference,translationConfigured,onToken,onClose,onToast,onConversationChange,onChanged}:{active:Conversation;token:string;user:User|null;role:string;translationPreference:TranslationPreference;translationConfigured:boolean;onToken:(token:string)=>void;onClose:()=>void;onToast:(text:string)=>void;onConversationChange:(change:Record<string,unknown>)=>Promise<void>;onChanged:()=>Promise<void>}){
  ...
  async function sendOrder(order:OrderItem){setBusy(true);setError("");try{const result=await authorizedFetch(`/api/v1/conversations/${active.id}/orders/${order.id}/send`,token,{method:"POST"});if(result.token!==token)onToken(result.token);const body=await result.response.json().catch(()=>({})) as {message?:string};if(!result.response.ok)throw new Error(body.message??`订单发送失败（HTTP ${result.response.status}）`);onToast(`订单 #${String(order.orderNumber).padStart(6,"0")} 已进入发送队列`);await load();await onChanged();}catch(reason){setError(reason instanceof Error?reason.message:"订单发送失败");}finally{setBusy(false);}}
...
type DraftProduct={id:string;name:string;quantity:string;unitAmount:string;image:File|null};
type DraftFee={id:string;name:string;amount:string};
const newDraftProduct=():DraftProduct=>({id:crypto.randomUUID(),name:"",quantity:"1",unitAmount:"",image:null});

function OrderDialog({active,token,translationPreference,translationConfigured,onToken,onClose,onCreated}:{active:Conversation;token:string;translationPreference:TranslationPreference;translationConfigured:boolean;onToken:(token:string)=>void;onClose:()=>void;onCreated:(orderNumber:number)=>Promise<void>}){
  const [products,setProducts]=useState<DraftProduct[]>(()=>[newDraftProduct()]),[fees,setFees]=useState<DraftFee[]>([]),[currency,setCurrency]=useState("USD"),[description,setDescription]=useState(""),[translateOnSend,setTranslateOnSend]=useState(()=>translationPreference.enabled&&translationConfigured),[busy,setBusy]=useState(false),[error,setError]=useState("");
  ...
  function chooseImage(id:string,file:File|undefined){if(file&&!["image/png","image/jpeg"].includes(file.type)){setError("产品图片仅支持 PNG 或 JPG");return;}setError("");updateProduct(id,{image:file??null});}
  async function submit(){const money=/^\d+(?:\.\d{1,2})?$/;if(products.some(item=>!item.name.trim()||!/^\d+$/.test(item.quantity)||Number(item.quantity)<1||!money.test(item.unitAmount))){setError("请完整填写每件商品的名称、数量和最多两位小数的单价");return;}if(fees.some(fee=>!fee.name.trim()||!money.test(fee.amount)||Number(fee.amount)<=0)){setError("请完整填写每项费用的名称和金额");return;}if(total<=0){setError("订单总额必须大于 0");return;}setBusy(true);setError("");try{let accessToken=token;const items=[];for(const product of products){let imageMediaId:string|undefined;if(product.image){const form=new FormData();form.append("file",product.image);const uploaded=await authorizedFetch(`/api/v1/media?accountId=${encodeURIComponent(active.accountId)}`,accessToken,{method:"POST",body:form});accessToken=uploaded.token;if(uploaded.token!==token)onToken(uploaded.token);if(!uploaded.response.ok)throw new Error(`${product.image.name} 上传失败`);const body=await uploaded.response.json() as {mediaId:string};imageMediaId=body.mediaId;}items.push({name:product.name.trim(),quantity:Number(product.quantity),unitAmount:Number(product.unitAmount),...(imageMediaId?{imageMediaId}:{})});}const created=await authorizedFetch(`/api/v1/conversations/${active.id}/orders`,accessToken,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({clientOrderId:crypto.randomUUID(),currency,description:description.trim()||undefined,translateOnSend,targetLanguage:translateOnSend?translationPreference.customerLanguage:undefined,items,fees:fees.map(fee=>({name:fee.name.trim(),amount:Number(fee.amount)}))})});if(created.token!==token)onToken(created.token);const body=await created.response.json().catch(()=>({})) as {orderNumber?:number;message?:string;error?:string};if(!created.response.ok||!body.orderNumber)throw new Error(body.message??body.error??`创建失败（HTTP ${created.response.status}）`);await onCreated(body.orderNumber);}catch(reason){setError(reason instanceof Error?reason.message:"创建订单失败");setBusy(false);}}
  return <div className="modal-backdrop order-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget&&!busy)onClose();}}><section className="login-dialog order-dialog order-builder" role="dialog" aria-modal="true" aria-labelledby="order-title"><button className="login-close" onClick={onClose} disabled={busy} aria-label="关闭"><X size={17}/></button><span className="login-logo"><ShoppingBag size={20}/></span><h2 id="order-title">创建订单</h2><p>订单先保存为草稿；在右侧栏确认发送时，才会翻译并进入 WhatsApp 队列。</p><div className="order-builder-head"><b>Products</b><label>Currency<select value={currency} onChange={event=>setCurrency(event.target.value)}>{["USD","CNY","EUR","GBP","JPY","HKD","SGD","AUD","CAD","AED"].map(item=><option key={item}>{item}</option>)}</select></label></div><div className="order-products">{products.map((product,index)=><article key={product.id} className="order-product"><header><b>Product {index+1}</b>{products.length>1&&<button onClick={()=>setProducts(all=>all.filter(item=>item.id!==product.id))} aria-label={`删除商品 ${index+1}`}><Trash2 size={13}/></button>}</header><label>Product name<input value={product.name} onChange={event=>updateProduct(product.id,{name:event.target.value})} maxLength={120} placeholder="Product name" autoFocus={index===0}/></label><div className="order-item-grid"><label>Quantity<input value={product.quantity} onChange={event=>updateProduct(product.id,{quantity:event.target.value})} inputMode="numeric"/></label><label>Unit price<input value={product.unitAmount} onChange={event=>updateProduct(product.id,{unitAmount:event.target.value})} inputMode="decimal" placeholder="0.00"/></label></div><label className="product-image-input">Product image · Optional<input type="file" accept="image/png,image/jpeg" onChange={event=>chooseImage(product.id,event.target.files?.[0])}/><span><UploadCloud size={14}/>{product.image?product.image.name:"Add PNG/JPG image"}</span></label></article>)}</div><button className="order-add-row" disabled={products.length>=50} onClick={()=>setProducts(all=>[...all,newDraftProduct()])}><Plus size={13}/>Add product</button><div className="order-fees-head"><b>Additional fees</b><button disabled={fees.length>=20} onClick={()=>setFees(all=>[...all,{id:crypto.randomUUID(),name:"",amount:""}])}><Plus size={12}/>Add fee</button></div>{fees.length?<div className="order-fees">{fees.map((fee,index)=><div key={fee.id}><input value={fee.name} onChange={event=>updateFee(fee.id,{name:event.target.value})} maxLength={80} placeholder={`Fee ${index+1} name`}/><input value={fee.amount} onChange={event=>updateFee(fee.id,{amount:event.target.value})} inputMode="decimal" placeholder="0.00"/><button onClick={()=>setFees(all=>all.filter(item=>item.id!==fee.id))} aria-label={`删除费用 ${index+1}`}><Trash2 size={13}/></button></div>)}</div>:<p className="order-empty-fees">No additional fees</p>}<label>Order notes · Optional<textarea value={description} onChange={event=>setDescription(event.target.value)} maxLength={2000} placeholder="Order notes in English"/></label><label className="translation-toggle order-translation"><span><b>AI translation on send</b><small>{translationConfigured?`Translate the English order to ${languageName(translationPreference.customerLanguage)} only when Send is clicked`:`Translation provider is not configured`}</small></span><input type="checkbox" checked={translateOnSend} disabled={!translationConfigured} onChange={event=>setTranslateOnSend(event.target.checked)}/></label><div className="order-total"><span>Total</span><b>{currency} {total.toFixed(2)}</b></div>{error&&<span className="login-error">{error}</span>}<p className="order-disclosure">Saving creates a draft only. Nothing will be sent to the customer yet.</p><button className="login-submit" disabled={busy||total<=0} onClick={()=>void submit()}>{busy?"Saving draft…":"Save order draft"}</button><small className="dialog-hint">Ctrl / Cmd + Enter</small></section></div>;
}
