...
type Conversation = {
  ...
  preview:string; lastDirection:"in"|"out"|null; time:string; unread:number; accountStatus:string; assignedUserId:string|null;
...
export function WhatsAppInbox({initialView="inbox"}:{initialView?:WorkspaceView}) {
  ...
  async function setContextConversationStatus(){
    ...
    if(next==="closed"&&!window.confirm(`关闭与“${item.name}”的会话？之后仍可从“已关闭”中重新打开。`))return;
    setContextBusy(true);
