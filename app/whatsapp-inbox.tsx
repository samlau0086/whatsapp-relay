...
function ContactManagement({token,role,accounts,onToken,onToast,onConversation}:{token:string;role:string;accounts:Account[];onToken:(token:string)=>void;onToast:(text:string)=>void;onConversation:(conversationId:string)=>void}){
  ...
  async function unblock(contact:ContactProfile){if(!contact.conversationId||!await confirmAction(`解除对“${contact.name}”的拉黑？对方可以再次向此 WhatsApp 账号发送消息。`,{title:"解除拉黑",confirmLabel:"解除拉黑",tone:"warning"}))return;const result=await authorizedFetch(`/api/v1/conversations/${contact.conversationId}/block`,token,{method:"DELETE"});if(result.token!==token)onToken(result.token);const body=await result.response.json().catch(()=>({})) as {message?:string};if(!result.response.ok){onToast(body.message??`解除拉黑失败（HTTP ${result.response.status}）`);return;}onToast(`已提交对“${contact.name}”的解除拉黑请求`);window.setTimeout(()=>void load(),1200);}
