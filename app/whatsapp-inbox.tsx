...
function ProductManagement({token,role,onToken,onToast}:{token:string;role:string;onToken:(token:string)=>void;onToast:(text:string)=>void}){
  const [products,setProducts]=useState<ProductItem[]>([]),[currencyConfig,setCurrencyConfig]=useState<CurrencyConfig>(DEFAULT_CURRENCY_CONFIG),[loading,setLoading]=useState(true),[error,setError]=useState(""),[query,setQuery]=useState(""),[currency,setCurrency]=useState(""),[tag,setTag]=useState(""),[tagNames,setTagNames]=useState<string[]>([]),[page,setPage]=useState(1),[total,setTotal]=useState(0),[view,setView]=useState<"card"|"list">("card"),[editing,setEditing]=useState<ProductItem|"new"|null>(null),[importing,setImporting]=useState(false);const loadVersion=useRef(0);
  ...
  const pageCount=Math.max(1,Math.ceil(total/PRODUCT_PAGE_SIZE)),visible=products;
  ...
  return <section className="management-panel product-management"><header className="management-head"><div><span className="eyebrow">团队共享目录</span><h1>产品库</h1><p>集中维护产品 SKU、阶梯价格、图片和标签，创建订单或发送产品卡片时直接选用。</p></div><div><button className="secondary-action" onClick={()=>void load()}><RefreshCw size={15}/>刷新</button><button className="secondary-action" onClick={()=>setImporting(true)}><UploadCloud size={15}/>CSV 导入</button><button className="primary-action" onClick={()=>setEditing("new")}><Plus size={15}/>新增产品</button></div></header>
    ...
    {!loading&&!error&&total>0&&<nav className="product-pagination" aria-label="产品分页"><button disabled={page<=1} onClick={()=>setPage(value=>Math.max(1,value-1))}>上一页</button><span>第 {page} / {pageCount} 页</span><button disabled={page>=pageCount} onClick={()=>setPage(value=>Math.min(pageCount,value+1))}>下一页</button></nav>}
