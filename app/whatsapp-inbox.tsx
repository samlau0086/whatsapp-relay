...
function PayPalSettingsPanel({token,onToken,onToast}:{token:string;onToken:(token:string)=>void;onToast:(text:string)=>void}){
  const [setting,setSetting]=useState({enabled:false,environment:"sandbox" as "sandbox"|"live",clientIdConfigured:false,clientSecretConfigured:false,referenceTemplate:"Order #{{orderNumber}}",noteTemplate:"{{orderNotes}}",itemNameTemplate:"{{productName}}"}),[clientId,setClientId]=useState(""),[clientSecret,setClientSecret]=useState(""),[loading,setLoading]=useState(true),[saving,setSaving]=useState(false),[error,setError]=useState("");
  const load=useCallback(async()=>{setLoading(true);setError("");try{const result=await authorizedFetch("/api/v1/admin/paypal-settings",token);if(result.token!==token)onToken(result.token);const body=await result.response.json() as Record<string,unknown>;if(!result.response.ok)throw new Error(String(body.message??body.error??`HTTP ${result.response.status}`));setSetting({enabled:Boolean(body.enabled),environment:body.environment==="live"?"live":"sandbox",clientIdConfigured:Boolean(body.clientIdConfigured),clientSecretConfigured:Boolean(body.clientSecretConfigured),referenceTemplate:String(body.referenceTemplate??"Order #{{orderNumber}}"),noteTemplate:String(body.noteTemplate??"{{orderNotes}}"),itemNameTemplate:String(body.itemNameTemplate??"{{productName}}")});setClientId(String(body.clientId??""));setClientSecret(String(body.clientSecret??""));}catch(reason){setError(reason instanceof Error?reason.message:"PayPal 配置加载失败");}finally{setLoading(false);}},[token,onToken]);
  useEffect(()=>{const timer=window.setTimeout(()=>void load(),0);return()=>window.clearTimeout(timer);},[load]);
  async function save(){setSaving(true);setError("");try{const result=await authorizedFetch("/api/v1/admin/paypal-settings",token,{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({enabled:setting.enabled,environment:setting.environment,clientId:clientId.trim()||undefined,clientSecret:clientSecret.trim()||undefined,referenceTemplate:setting.referenceTemplate,noteTemplate:setting.noteTemplate,itemNameTemplate:setting.itemNameTemplate})});if(result.token!==token)onToken(result.token);const body=await result.response.json().catch(()=>({})) as Record<string,unknown>;if(!result.response.ok)throw new Error(String(body.message??body.error??`HTTP ${result.response.status}`));onToast(`PayPal ${setting.environment==="live"?"Live":"Sandbox"} 配置已保存${setting.enabled?"并启用":""}`);await load();}catch(reason){setError(reason instanceof Error?reason.message:"PayPal 配置保存失败");}finally{setSaving(false);}}
