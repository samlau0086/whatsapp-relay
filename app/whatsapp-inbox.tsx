...
function AgentManagement({token,role,accounts,onToken,onToast}:{token:string;role:string;accounts:Account[];onToken:(token:string)=>void;onToast:(text:string)=>void}){
  ...
  return <section className="management-panel"><header className="management-head"><div><span className="eyebrow">设备与连接</span><h1>Agent 管理</h1><p>查看所有已注册 Windows Agent、连接状态和所管理的 WhatsApp 账号。</p></div><div><button className="secondary-action" onClick={()=>void load()}><RefreshCw size={15}/>刷新</button>{role==="admin"&&<button className="primary-action" onClick={()=>void createAgent()}><Plus size={15}/>注册新 Agent</button>}</div></header>
    ...
    {role==="admin"&&<DetachedAccountMigration accounts={detachedAccounts} onMigrate={account=>setMigration({account})}/>}
...
function DetachedAccountMigration({accounts,onMigrate}:{accounts:ManagedAgentAccount[];onMigrate:(account:ManagedAgentAccount)=>void}){
  return <section className="agent-accounts"><h2>已移除账号的数据迁移 <span>{accounts.length}</span></h2><p>这些账号已从 Agent 移除，但中心端仍保留会话和相关数据。选择一个账号后，可将其数据迁移到新登录的空 WhatsApp 账号。</p>{accounts.map(account=><div key={account.id}><i className="status-dot"/><span><b>{account.display_name}</b><small>{account.phone_e164||account.status_reason||"已从 Agent 移除"}</small></span><em>已移除</em><button title="迁移旧账号数据" onClick={()=>onMigrate(account)}><ArrowRightLeft size={14}/>迁移数据</button></div>)}</section>;
}
