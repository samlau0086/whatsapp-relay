...
import { conversationCountsPath, conversationListPath, conversationSummaryPath, type ConversationAgentMode, type ConversationCustomerStage, type ConversationDateFilter, type ConversationFollowupFilter, type ConversationLatestOrderStatus, type ConversationListFilter } from "./conversation-date-filter";
...
function productCacheKey(subject:string,input:{page:number;pageSize:number;query:string;currency:string;category:string;brand:string;tag:string;stock:string}){
...
type OrderDocumentHtmlPreview={documentType:"inq"|"qt"|"sc"|"pi"|"ci";template:{blocks:OrderDocumentTemplateBlock[]};blocks:Array<{id:string;type:string;lines:string[];itemIndexes?:number[];outOfStockLineIndexes?:number[]}>};
...
type ChatMessage = {
  ...
  email?:{subject:string;from:string;to:string[];attachments:Array<{id:string;name:string;mime:string;size?:string}>;warning?:string;jobId?:string};
...
export function WhatsAppInbox({initialView="inbox"}:{initialView?:WorkspaceView}) {
  ...
  async function sendMessage(){
    ...
    if(emailMode){await sendEmailMessage();return;}
  ...
  async function sendEmailMessage(){
    if(!active||!apiToken||!draft.trim()||!activePrimaryEmail||!emailMailboxId)return;
    ...
    setMessages(all=>({...all,[active.id]:[...(all[active.id]??[]),{id:optimisticId,direction:"out",kind:"text",text:draft.trim(),time:formatTime(new Date()),occurredAt:new Date().toISOString(),status:"queued",email:{subject,from:emailMailboxes.find(item=>item.id===emailMailboxId)?.address??"",to:[activePrimaryEmail],attachments:[]},comments:[]}] }));
    const result=await authorizedFetch(`/api/v1/conversations/${active.id}/email-sends`,apiToken,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({clientSendId,mailboxId:emailMailboxId,recipientEmail:activePrimaryEmail,subject,body:draft.trim(),attachmentIds:[],replyToMessageId:replyTo?.message.id})});
    if(result.token!==apiToken)setApiToken(result.token);
    ...
    setDraft("");setEmailSubject("");setReplyTo(null);setToast("邮件已进入发送队列");void loadMessages(result.token,active.id);
  }
    ...
    <main className={`relay-shell ${filtersHidden ? "filters-hidden" : ""} ${conversationListHidden ? "conversation-list-hidden" : ""} ${!detailsOpen || !active ? "details-hidden" : ""}`}>
            ...
            {active ? (
              <>
                ...
                {active.conversationType!=="group"&&<AgentConversationBar
                  conversationId={active.id}
                  token={apiToken}
                  translationTargetLanguage={translationPreference.customerLanguage}
                  refreshKey={latestMessageId}
                  onToken={setApiToken}
                  onToast={setToast}
                  onUseDraft={setDraft}
                  onSent={() => {void loadMessages(apiToken, active.id);void loadConversations(apiToken);}}
                />}
                {active.accountStatus !== "online" && (
                ...
                {false && currentEmailActivities.length > 0 && <div className="email-activity-stream" aria-hidden="true" />}
                <div
                  ref={messagesRef}
                  className="messages"
                  aria-live="polite"
                    ...
                    currentMessages.map((message) => (
                      ...
                      >
                        ...
                        >
                          ...
                          {message.email&&<div className="email-message-meta"><span><Mail size={13}/>邮件</span><b>{message.email.subject}</b><small>{message.direction==="out"?`发件：${message.email.from}`:`收件：${message.email.from}`} · {message.direction==="out"?`收件：${message.email.to.join(", ")}`:`发给：${message.email.to.join(", ")}`}</small>{message.email.attachments.length>0&&<small>附件：{message.email.attachments.map(item=>item.name).join("、")}</small>}{message.email.warning&&<small className="message-failure">{message.email.warning}</small>}</div>}
      ...
      {(composerImageDragging || composerImageBusy) && (
                      ...
                      <textarea
                        ...
                        placeholder={emailMode?"输入邮件正文":"输入消息；可粘贴或拖入图片发送"}
                      />
                      <div className="composer-icons">
                        <button type="button" className={emailMode?"active":""} onClick={()=>setEmailMode(value=>!value)} aria-label="Email 模式" title="Email 模式"><Mail size={18}/></button>
                        <button
        ...
        <ContactManagement
          ...
          onStartConversation={async (contact) => {
            ...
            if (!contact.phone.trim()&&!contact.whatsappUsername?.trim()) {
              setToast("该联系人未设置 WhatsApp 号码或用户名");
              return;
            }
            if (account?.transport==="cloud"&&!contact.phone.trim()) {
              setToast("Cloud API 仅支持通过 WhatsApp 号码发起会话");
              return;
            }
            const result=await authorizedFetch("/api/v1/conversations",apiToken,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({accountId:contact.accountId,...(contact.phone.trim()?{phone:contact.phone}:{whatsappUsername:contact.whatsappUsername}),displayName:contact.name,clientMessageId:crypto.randomUUID()})});
            if(result.token!==apiToken)setApiToken(result.token);
        ...
        <SettingsPanel
          ...
          onToken={setApiToken}
          onToast={setToast}
          onAccountsChanged={()=>void loadAccounts(apiToken)}
        />
      ) : (
        <HelpPanel
          onInbox={() => openInbox()}
    ...
    <>
      ...
      <aside className="details-panel crm-details" aria-label="联系人详情">
        ...
        <div className="contact-card">
          ...
          {details?.contact?.whatsappUsername&&<p className="contact-method-summary">WhatsApp 用户名：@{details.contact.whatsappUsername}</p>}{details?.contact?.primaryEmail&&<p className="contact-primary-email"><Mail size={12}/>{details.contact.primaryEmail}</p>}
          ...
          <div id={listboxId} className="product-search-options" role="listbox">
            {filtered.length ? filtered.map((item, index) => (
              ...
              >
                ...
                <strong>{item.isInStock ? `${item.currency} ${item.defaultUnitAmount.toFixed(2)}` : "Out Of Stock"}</strong>
      ...
      >
        ...
        <div className="order-products">
          {products.map((product, index) => {
            ...
            const outOfStock = product.mode === "library" && product.productId ? catalog.find(item=>item.id===product.productId)?.isInStock === false : false;
            return <article key={product.id} className="order-product">
              ...
              <div className="order-item-grid">
                ...
                <label>
                  成交单价
                  {outOfStock ? <strong className="out-of-stock-label">Out Of Stock</strong> : <input
...
function MediaDialog({accountId,conversationId,token,initialCaption,translationEnabled,onToken,onToast,onClose,onSend}:{accountId:string;conversationId:string;token:string;initialCaption:string;translationEnabled:boolean;onToken:(token:string)=>void;onToast:(text:string)=>void;onClose:()=>void;onSend:(asset:MediaAsset,caption:string)=>Promise<void>}){
  ...
  const loadPage=useCallback(async(offset:number,replace:boolean,version:number,signal?:AbortSignal)=>{
    ...
    setError("");
    try{
       const params=new URLSearchParams({accountId,limit:String(MEDIA_PAGE_SIZE),offset:String(offset),kind:filter});if(conversationOnly)params.set("conversationId",conversationId);if(debouncedQuery)params.set("q",debouncedQuery);
  ...
  },[accountId,conversationId,conversationOnly,debouncedQuery,filter]);
...
function AgentConversationBar({conversationId,token,translationTargetLanguage,refreshKey,onToken,onToast,onUseDraft,onSent}:{conversationId:string;token:string;translationTargetLanguage:string;refreshKey:string;onToken:(token:string)=>void;onToast:(text:string)=>void;onUseDraft:(text:string)=>void;onSent:()=>void}){
  const [state,setState]=useState<{mode:string;account_enabled:boolean;draft:AgentDraft|null}|null>(null),[busy,setBusy]=useState(false),[blockedSession,setBlockedSession]=useState(""),loadSequenceRef=useRef(0),hiddenDraftIdsRef=useRef(new Set<string>());
  const sessionKey=`${conversationId}:${token}`,authenticationFailed=blockedSession===sessionKey;
  const load=useCallback(async()=>{const requestSequence=++loadSequenceRef.current;try{const result=await authorizedFetch(`/api/v1/conversations/${conversationId}/agent`,token);if(requestSequence!==loadSequenceRef.current)return;if(result.token!==token)onToken(result.token);if(result.response.status===401){setBlockedSession(sessionKey);return;}if(result.response.ok){setBlockedSession("");const next=await result.response.json() as {mode:string;account_enabled:boolean;draft:AgentDraft|null};if(next.draft&&hiddenDraftIdsRef.current.has(String(next.draft.id)))next.draft=null;setState(next);}}catch{}},[conversationId,token,onToken,sessionKey]);
  useEffect(()=>{if(authenticationFailed)return;const first=window.setTimeout(()=>void load(),0),poll=window.setInterval(()=>void load(),8000);return()=>{window.clearTimeout(first);window.clearInterval(poll);};},[load,refreshKey,authenticationFailed]);
  async function setMode(mode:"cautious"|"full"|"human_paused"){setBusy(true);const result=await authorizedFetch(`/api/v1/conversations/${conversationId}/agent`,token,{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({mode})});if(result.token!==token)onToken(result.token);setBusy(false);if(result.response.ok){onToast(mode==="cautious"?"已启用谨慎接管，证据不足时将等待人工确认":mode==="full"?"已启用完全接管，Agent 回复无需人工确认":"已切换人工接管，待跟进任务已取消");await load();onSent();}}
  async function resolveDraft(action:"send"|"dismiss"){if(!state?.draft)return;const draft=state.draft,draftId=String(draft.id);hiddenDraftIdsRef.current.add(draftId);setState(current=>current?{...current,draft:null}:current);setBusy(true);const result=await authorizedFetch(`/api/v1/ai-drafts/${draftId}/${action}`,token,{method:"POST",headers:{"content-type":"application/json"},body:action==="send"?JSON.stringify({text:draft.text_content,translationSourceText:draft.reply_zh?.trim()||undefined,translationTargetLanguage:draft.reply_zh?.trim()?translationTargetLanguage:undefined}):undefined});if(result.token!==token)onToken(result.token);setBusy(false);if(result.response.ok){onToast(action==="send"?"AI 草稿已进入发送队列":"AI 草稿已忽略");await load();onSent();}else{onToast(action==="send"?"AI 草稿发送失败":"AI 草稿已隐藏，请稍后重试");}}
  async function handleUseDraft(){if(!state?.draft)return;const draftId=String(state.draft.id),text=state.draft.text_content;hiddenDraftIdsRef.current.add(draftId);setState(current=>current?{...current,draft:null}:current);setBusy(true);const result=await authorizedFetch(`/api/v1/ai-drafts/${draftId}/dismiss`,token,{method:"POST"});if(result.token!==token)onToken(result.token);setBusy(false);if(result.response.ok){onUseDraft(text);onToast("回复建议已放入输入框，可继续编辑后发送");await load();onSent();}else{onUseDraft(text);onToast("回复建议已放入输入框，但服务端状态更新失败");}}
  if(!state)return null;const paused=state.mode==="human_paused",full=state.mode==="full",enabled=state.account_enabled;
  return <div className={`agent-conversation-bar ${paused?"paused":full?"full":""} ${enabled?"":"disabled"}`}><span><Bot size={15}/><b>{enabled?(paused?"人工接管":full?"完全接管":"谨慎接管"):"AI 自动回复未配置"}</b><small>{enabled?(paused?"当前会话不会自动回复或跟进":full?"当前会话完全由 Agent 回复，无需人工确认":"可靠回复自动发送，证据不足时等待人工确认"):"请先在系统设置中启用该账号的 AI 能力"}</small></span><div className="takeover-switch" role="group" aria-label="会话接管方式"><button className={!paused&&!full&&enabled?"active cautious":""} disabled={busy||!enabled} onClick={()=>void setMode("cautious")} aria-pressed={!paused&&!full&&enabled}><ShieldCheck size={15}/>谨慎接管</button><button className={full&&enabled?"active full":""} disabled={busy||!enabled} onClick={()=>void setMode("full")} aria-pressed={full&&enabled}><Bot size={15}/>完全接管</button><button className={paused&&enabled?"active human":""} disabled={busy||!enabled} onClick={()=>void setMode("human_paused")} aria-pressed={paused&&enabled}><Users size={15}/>人工接管</button></div>{state.draft&&<div className="agent-draft"><span><Sparkles size={14}/><b>AI 建议回复</b><small>{state.draft.reason}</small></span><div className="agent-draft-copy"><section><b>发送内容</b><p>{state.draft.text_content}</p></section><section className="zh"><b>中文参考</b><p>{state.draft.reply_zh||"历史建议未生成中文参考"}</p></section></div><div><button onClick={()=>void handleUseDraft()}>放入输入框</button><button onClick={()=>void resolveDraft("dismiss")}>忽略</button><button className="primary" onClick={()=>void resolveDraft("send")}>确认发送</button></div></div>}</div>;
}
...
function SettingsPanel({token,role,accounts,onToken,onToast,onAccountsChanged}:{token:string;role:string;accounts:Account[];onToken:(token:string)=>void;onToast:(text:string)=>void;onAccountsChanged:()=>void}){
  const searchParams=useSearchParams();
  ...
  useEffect(()=>{
    const onPopState=()=>{
      ...
      setTab(next&&SETTINGS_TABS.has(next as SettingsTab)?next as SettingsTab:oauthTab?"messenger":"cloud");
    };
    window.addEventListener("popstate",onPopState);
    return()=>window.removeEventListener("popstate",onPopState);
  },[oauthTab]);
...
const API_KEY_SCOPE_LABELS:Record<ApiKeyScope,string>={"products:read":"读取产品","products:write":"更新产品","messages:read":"读取消息","messages:send":"发送消息","conversations:write":"更新会话与标签","contacts:write":"更新联系人资料"};
...
type MessengerOAuthSession={id:string;status:"pending"|"processing"|"pages_ready"|"completed"|"failed";expiresAt:string;lastError:string|null;createdAt?:string;updatedAt?:string};
const MESSENGER_OAUTH_SESSION_KEY="relayMessengerOAuthSession";
function MessengerSettingsPanel({token,onToken,onToast,onAccountsChanged}:{token:string;onToken:(token:string)=>void;onToast:(text:string)=>void;onAccountsChanged:()=>void}){
  const [pages,setPages]=useState<MessengerPageAdmin[]>([]),[form,setForm]=useState({displayName:"",pageId:"",pageAccessToken:"",appSecret:""}),[oauthForm,setOauthForm]=useState({appId:"",configurationId:"",appSecret:"",enabled:true}),[oauthSettings,setOauthSettings]=useState<MessengerOAuthSettings|null>(null),[oauthSessionId,setOauthSessionId]=useState(()=>typeof window==="undefined"?"":sessionStorage.getItem(MESSENGER_OAUTH_SESSION_KEY)??""),[oauthStatus,setOauthStatus]=useState(""),[candidates,setCandidates]=useState<MessengerOAuthCandidate[]>([]),[selectedPages,setSelectedPages]=useState<string[]>([]),[busy,setBusy]=useState(false),[error,setError]=useState(""),[verifyToken,setVerifyToken]=useState("");
  ...
  useEffect(()=>{const timer=setTimeout(()=>void load(),0);return()=>clearTimeout(timer);},[load]);
  const applyOAuthResult=useCallback((session:MessengerOAuthSession,nextPages:MessengerOAuthCandidate[])=>{setOauthSessionId(session.id);if(session.status==="pending"){setOauthStatus("等待在 Facebook 弹窗中完成授权…");return session.status;}if(session.status==="processing"){setOauthStatus("Facebook 已返回，正在读取可管理的 Pages…");return session.status;}sessionStorage.removeItem(MESSENGER_OAUTH_SESSION_KEY);if(session.status==="failed"){setOauthStatus("Facebook 授权失败");setError(session.lastError??"Facebook 授权失败");return session.status;}if(session.status==="completed"){setOauthStatus("最近一次 Facebook 授权已完成");return session.status;}setCandidates(nextPages);setSelectedPages(nextPages.map(page=>page.pageId));setOauthStatus(nextPages.length?`Facebook 已返回 ${nextPages.length} 个可管理 Page`:`Facebook 已完成授权，但返回了 0 个可管理 Page`);setError(nextPages.length?"":session.lastError??"Facebook 没有返回可管理的 Page。");return session.status;},[]);
  const loadCandidates=useCallback(async(sessionId:string)=>{try{const result=await authorizedFetch(`/api/v1/admin/messenger/oauth/sessions/${sessionId}`,token);if(result.token!==token)onToken(result.token);const body=await result.response.json().catch(()=>({})) as {session?:MessengerOAuthSession;pages?:MessengerOAuthCandidate[];error?:string;message?:string};if(!result.response.ok||!body.pages||!body.session){const message=body.message??body.error??`HTTP ${result.response.status}`;if(result.response.status>=500){setOauthStatus(`暂时无法查询授权结果（${message}），正在重试…`);return "retry" as const;}sessionStorage.removeItem(MESSENGER_OAUTH_SESSION_KEY);setOauthStatus("无法读取 Facebook 授权结果");setError(message);return "failed" as const;}return applyOAuthResult(body.session,body.pages);}catch(reason){setOauthStatus("网络暂时中断，正在重试 Facebook 授权结果…");setError(reason instanceof Error?reason.message:"无法查询 Facebook 授权结果");return "retry" as const;}},[token,onToken,applyOAuthResult]);
  useEffect(()=>{let cancelled=false;const recover=async()=>{if(oauthSessionId)return;try{const result=await authorizedFetch("/api/v1/admin/messenger/oauth/sessions/latest",token);if(result.token!==token)onToken(result.token);if(!result.response.ok)return;const body=await result.response.json() as {session?:MessengerOAuthSession;pages?:MessengerOAuthCandidate[]};if(cancelled||!body.session||!body.pages||body.session.status==="completed")return;applyOAuthResult(body.session,body.pages);if(body.session.status==="pending"||body.session.status==="processing")sessionStorage.setItem(MESSENGER_OAUTH_SESSION_KEY,body.session.id);}catch{/* The regular settings load still works if diagnostics are temporarily unavailable. */}};void recover();return()=>{cancelled=true;};},[oauthSessionId,token,onToken,applyOAuthResult]);
  useEffect(()=>{if(!oauthSessionId)return;let cancelled=false;const startedAt=Date.now();const poll=async()=>{while(!cancelled&&Date.now()-startedAt<5*60_000){const status=await loadCandidates(oauthSessionId);if(cancelled||status==="pages_ready"||status==="failed"||status==="completed")return;await new Promise(resolve=>window.setTimeout(resolve,2000));}if(!cancelled){sessionStorage.removeItem(MESSENGER_OAUTH_SESSION_KEY);setOauthStatus("等待 Facebook 授权结果超时");setError("等待 Facebook 授权结果超时，请重新点击“使用 Facebook 连接 Pages”。");}};void poll();return()=>{cancelled=true;};},[oauthSessionId,loadCandidates]);
  useEffect(()=>{function receive(event:MessageEvent){if(event.origin!==new URL(API_URL||window.location.origin).origin||event.data?.type!=="relaydesk:messenger-oauth")return;if(event.data.error){setOauthStatus("Facebook 授权失败");setError(String(event.data.error));return;}if(event.data.sessionId){const sessionId=String(event.data.sessionId);sessionStorage.setItem(MESSENGER_OAUTH_SESSION_KEY,sessionId);setOauthSessionId(sessionId);}}window.addEventListener("message",receive);const timer=window.setTimeout(()=>{const query=new URLSearchParams(window.location.search),sessionId=query.get("messengerOauth"),oauthError=query.get("messengerOauthError");if(sessionId){sessionStorage.setItem(MESSENGER_OAUTH_SESSION_KEY,sessionId);setOauthSessionId(sessionId);}if(oauthError){setOauthStatus("Facebook 授权失败");setError(oauthError);}},0);return()=>{window.clearTimeout(timer);window.removeEventListener("message",receive);};},[]);
  ...
  async function deleteOAuthSettings(){if(!oauthSettings?.configured||!await confirmAction("删除后，Facebook OAuth 配置、未完成的授权会话，以及通过 OAuth 连接的 Messenger Page 凭据都会移除；历史聊天、联系人和消息会保留。",{title:"删除 Facebook OAuth 配置？",confirmLabel:"删除配置",tone:"danger"}))return;setBusy(true);setError("");const result=await authorizedFetch("/api/v1/admin/messenger/oauth/settings",token,{method:"DELETE"});if(result.token!==token)onToken(result.token);const body=await result.response.json().catch(()=>({})) as {message?:string;error?:string};setBusy(false);if(!result.response.ok){setError(body.message??body.error??`HTTP ${result.response.status}`);return;}setVerifyToken("");setCandidates([]);setSelectedPages([]);setOauthSessionId("");sessionStorage.removeItem(MESSENGER_OAUTH_SESSION_KEY);onToast("Facebook OAuth 配置及其 Page 连接已删除");onAccountsChanged();await load();}
  async function resetOAuthVerifyToken(){setBusy(true);setError("");const result=await authorizedFetch("/api/v1/admin/messenger/oauth/verify-token/reset",token,{method:"POST"});if(result.token!==token)onToken(result.token);const body=await result.response.json().catch(()=>({})) as {verifyToken?:string;error?:string;message?:string};setBusy(false);if(!result.response.ok){setError(body.message??body.error??`HTTP ${result.response.status}`);return;}setVerifyToken(body.verifyToken??"");onToast("Webhook Verify Token 已重置");await load();}
  async function startOAuth(){setBusy(true);setError("");setCandidates([]);setSelectedPages([]);setOauthStatus("正在创建 Facebook 授权会话…");try{const result=await authorizedFetch("/api/v1/admin/messenger/oauth/start",token,{method:"POST"});if(result.token!==token)onToken(result.token);const body=await result.response.json().catch(()=>({})) as {authorizationUrl?:string;sessionId?:string;error?:string;message?:string};if(!result.response.ok||!body.authorizationUrl||!body.sessionId){setOauthStatus("无法开始 Facebook 授权");setError(body.message??body.error??`HTTP ${result.response.status}`);return;}const popup=window.open(body.authorizationUrl,"relaydesk-meta-oauth","popup=yes,width=640,height=760");if(!popup){setOauthStatus("Facebook 授权弹窗被浏览器阻止");setError("浏览器阻止了 Facebook 授权弹窗，请允许本站打开弹窗后重试。");return;}sessionStorage.setItem(MESSENGER_OAUTH_SESSION_KEY,body.sessionId);setOauthSessionId(body.sessionId);setOauthStatus("等待在 Facebook 弹窗中完成授权…");}catch(reason){setOauthStatus("无法开始 Facebook 授权");setError(reason instanceof Error?reason.message:"Facebook 授权请求失败");}finally{setBusy(false);}}
  async function connectSelected(){if(!oauthSessionId||!selectedPages.length)return;setBusy(true);setError("");const result=await authorizedFetch(`/api/v1/admin/messenger/oauth/sessions/${oauthSessionId}/connect`,token,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({pageIds:selectedPages})});if(result.token!==token)onToken(result.token);const body=await result.response.json().catch(()=>({})) as {results?:Array<{pageId:string;ok:boolean;error?:string}>;error?:string;message?:string};setBusy(false);if(!result.response.ok&&result.response.status!==207){setError(body.message??body.error??`HTTP ${result.response.status}`);return;}const failed=body.results?.filter(item=>!item.ok)??[];if(failed.length){setOauthStatus("部分 Page 连接失败");setError(failed.map(item=>`${item.pageId}: ${item.error??"连接失败"}`).join("\n"));}else{setOauthStatus("Facebook Pages 已连接");onToast(`已通过 Facebook OAuth 连接 ${body.results?.length??selectedPages.length} 个 Page`);}sessionStorage.removeItem(MESSENGER_OAUTH_SESSION_KEY);setCandidates([]);setSelectedPages([]);setOauthSessionId("");await load();}
...
type ProactiveTemplate={id:string;language:string;scenario:""|"first_touch"|"follow_up";customerStage:""|"new"|"considering"|"qualified";body:string};
type ProactiveHoliday={id:string;name:string;month:number;day:number;region:string};
function ProactiveOutreachSettingsPanel({token,accounts,onToken,onToast}:{token:string;accounts:Account[];onToken:(token:string)=>void;onToast:(text:string)=>void}){
  const defaultTemplates=()=>[{id:"first-touch",language:"",scenario:"first_touch" as const,customerStage:"" as const,body:"Hi {{contactName}}, just checking in. Is there anything I can help with?"},{id:"follow-up",language:"",scenario:"follow_up" as const,customerStage:"" as const,body:"Hi {{contactName}}, following up in case you still need any help. Would you like to continue?"}];
  const [accountId,setAccountId]=useState(accounts[0]?.id??""),[loading,setLoading]=useState(false),[saving,setSaving]=useState(false),[scanning,setScanning]=useState(false),[error,setError]=useState(""),[settings,setSettings]=useState({enabled:false,maxTouchesPerYear:5,maxTouchesPerDay:20,localSendStart:"10:00",localSendEnd:"17:00",templates:defaultTemplates(),holidays:[] as ProactiveHoliday[]});
  const readTemplates=(value:unknown):ProactiveTemplate[]=>{const collect=(input:unknown,language=""):ProactiveTemplate[]=>{if(typeof input==="string")return input.trim()?([{id:`${language||"default"}-${Date.now().toString(36)}`,language,scenario:"",customerStage:"",body:input.trim()}]):[];if(Array.isArray(input))return input.flatMap(item=>collect(item));if(!input||typeof input!=="object")return[];const record=input as Record<string,unknown>,body=String(record.body??record.text??record.content??"").trim(),scenario=record.scenario==="first_touch"||record.scenario==="follow_up"?record.scenario:"",stages=Array.isArray(record.customerStages)?record.customerStages:record.customerStage?[record.customerStage]:[];if(body)return[{id:String(record.id??`${language||record.language||"default"}-${Date.now().toString(36)}`),language:String(record.language??language??""),scenario,customerStage:stages.includes("new")?"new":stages.includes("considering")?"considering":stages.includes("qualified")?"qualified":"",body}];if(Array.isArray(record.templates))return record.templates.flatMap(item=>collect(item));return Object.entries(record).flatMap(([key,item])=>key==="templates"?[]:collect(item,key==="default"?"":key));};return collect(value);};
  const readHolidays=(value:unknown):ProactiveHoliday[]=>{if(!value||typeof value!=="object")return[];return Object.entries(value as Record<string,unknown>).flatMap(([region,items])=>Array.isArray(items)?items.flatMap((item,index)=>{if(!item||typeof item!=="object")return[];const record=item as Record<string,unknown>;return[{id:String(record.id??`${region}-${index}`),name:String(record.name??""),month:Number(record.month??1),day:Number(record.day??1),region}];}):[]);};
  const load=useCallback(async()=>{if(!accountId)return;setLoading(true);setError("");try{const result=await authorizedFetch(`/api/v1/accounts/${accountId}/proactive-outreach`,token);if(result.token!==token)onToken(result.token);const body=await result.response.json().catch(()=>({})) as Record<string,unknown>;if(!result.response.ok)throw new Error(String(body.error??"主动触达设置读取失败"));const templates=readTemplates(body.message_templates),holidays=readHolidays(body.country_holidays);setSettings({enabled:Boolean(body.enabled),maxTouchesPerYear:Number(body.max_touches_per_year??5),maxTouchesPerDay:Number(body.max_touches_per_day??20),localSendStart:String(body.local_send_start??"10:00").slice(0,5),localSendEnd:String(body.local_send_end??"17:00").slice(0,5),templates:templates.length?templates:defaultTemplates(),holidays});}catch(reason){setError(reason instanceof Error?reason.message:"主动触达设置读取失败");}finally{setLoading(false);}},[accountId,token,onToken]);
  ...
  useEffect(()=>{const timer=window.setTimeout(()=>void load(),0);return()=>window.clearTimeout(timer);},[load]);
  const payload=()=>({enabled:settings.enabled,maxTouchesPerYear:settings.maxTouchesPerYear,maxTouchesPerDay:settings.maxTouchesPerDay,localSendStart:settings.localSendStart,localSendEnd:settings.localSendEnd,messageTemplates:{templates:settings.templates.filter(item=>item.body.trim()).map(item=>({id:item.id,language:item.language.trim()||undefined,scenario:item.scenario||undefined,customerStages:item.customerStage?[item.customerStage]:undefined,body:item.body.trim()}))},countryHolidays:settings.holidays.filter(item=>item.name.trim()).reduce<Record<string,Array<{id:string;name:string;month:number;day:number}>>>((all,item)=>{const region=item.region.trim()||"global";(all[region]??=[]).push({id:item.id,name:item.name.trim(),month:item.month,day:item.day});return all;},{})});
  async function save(quiet=false){if(!accountId)return false;if(!settings.templates.some(item=>item.body.trim())){setError("请至少保留一条系统模板");return false;}setSaving(true);setError("");try{const result=await authorizedFetch(`/api/v1/accounts/${accountId}/proactive-outreach`,token,{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify(payload())});if(result.token!==token)onToken(result.token);if(!result.response.ok){const body=await result.response.json().catch(()=>({})) as {error?:string};throw new Error(body.error??"主动触达设置保存失败");}if(!quiet)onToast("主动触达设置已保存");return true;}catch(reason){setError(reason instanceof Error?reason.message:"主动触达设置保存失败");return false;}finally{setSaving(false);}}
  async function scan(){if(!(await save(true)))return;setScanning(true);try{const result=await authorizedFetch(`/api/v1/accounts/${accountId}/proactive-outreach/scan`,token,{method:"POST"});if(result.token!==token)onToken(result.token);if(!result.response.ok)throw new Error("立即扫描失败");onToast("已开始扫描符合条件的客户");await load();}catch(reason){setError(reason instanceof Error?reason.message:"立即扫描失败");}finally{setScanning(false);}}
  const patchTemplate=(index:number,change:Partial<ProactiveTemplate>)=>setSettings(value=>({...value,templates:value.templates.map((item,itemIndex)=>itemIndex===index?{...item,...change}:item)}));
  const patchHoliday=(index:number,change:Partial<ProactiveHoliday>)=>setSettings(value=>({...value,holidays:value.holidays.map((item,itemIndex)=>itemIndex===index?{...item,...change}:item)}));
  return <div className="agent-settings-grid proactive-settings-layout"><section className="provider-form agent-card proactive-settings"><header><div><h2>主动触达</h2><p>覆盖 new、considering、qualified 客户；AI 优先生成，失败时使用这里定义的系统模板。</p></div><button type="button" className="secondary-action" disabled={loading} onClick={()=>void load()}><RefreshCw size={15}/>刷新</button></header><label>WhatsApp 账号<select value={accountId} onChange={event=>setAccountId(event.target.value)}>{accounts.map(account=><option key={account.id} value={account.id}>{account.name}</option>)}</select></label><label className="provider-toggle proactive-toggle"><input type="checkbox" checked={settings.enabled} onChange={event=>setSettings(value=>({...value,enabled:event.target.checked}))}/><span><b>启用主动触达</b><small>客户回复、退订、人工接管和触达上限仍会自动抑制后续发送。</small></span></label><div className="provider-form-grid"><label>每位客户年度最多触达<input type="number" min="1" max="12" value={settings.maxTouchesPerYear} onChange={event=>setSettings(value=>({...value,maxTouchesPerYear:Number(event.target.value)}))}/></label><label>每日最多发送跟进<input type="number" min="1" max="500" value={settings.maxTouchesPerDay} onChange={event=>setSettings(value=>({...value,maxTouchesPerDay:Number(event.target.value)}))}/><small>该账号当天所有主动跟进合计的发送上限。</small></label><label>本地发送开始<input type="time" value={settings.localSendStart} onChange={event=>setSettings(value=>({...value,localSendStart:event.target.value}))}/></label><label>本地发送结束<input type="time" value={settings.localSendEnd} onChange={event=>setSettings(value=>({...value,localSendEnd:event.target.value}))}/></label></div><div className="proactive-settings-block"><div><b>系统模板</b><small>不是 Meta 审核模板。每条规则只会在设定的触达场景与客户阶段命中；仅在渠道服务窗口内作为 AI 的安全兜底，窗口外会自动顺延。</small></div><div className="proactive-template-labels" aria-hidden="true"><span>触达场景</span><span>客户阶段</span><span>客户语言</span></div>{settings.templates.map((template,index)=><div className="proactive-template-row" key={template.id}><select value={template.scenario} onChange={event=>patchTemplate(index,{scenario:event.target.value as ProactiveTemplate["scenario"]})} aria-label="触达场景"><option value="">所有触达</option><option value="first_touch">首次触达</option><option value="follow_up">后续未回复</option></select><select value={template.customerStage} onChange={event=>patchTemplate(index,{customerStage:event.target.value as ProactiveTemplate["customerStage"]})} aria-label="客户阶段"><option value="">全部阶段</option><option value="new">New</option><option value="considering">Considering</option><option value="qualified">Qualified</option></select><input value={template.language} onChange={event=>patchTemplate(index,{language:event.target.value})} placeholder="语言（留空为默认）" aria-label="模板语言"/><textarea maxLength={280} value={template.body} onChange={event=>patchTemplate(index,{body:event.target.value})} placeholder="输入跟进文案" aria-label="系统模板文案"/><span>{template.body.length}/280</span><button type="button" className="danger-text" disabled={settings.templates.length===1} onClick={()=>setSettings(value=>({...value,templates:value.templates.filter((_,itemIndex)=>itemIndex!==index)}))}><Trash2 size={14}/></button></div>)}<button type="button" className="secondary-action" onClick={()=>setSettings(value=>({...value,templates:[...value.templates,{id:`template-${Date.now().toString(36)}`,language:"",scenario:"first_touch",customerStage:"",body:""}]}))}><Plus size={14}/>新增模板</button><p className="proactive-hint">推荐分别配置“首次触达”和“后续未回复”；同一场景下会优先匹配客户语言与客户阶段，再使用该场景的通用模板。“所有触达”只在没有更具体规则时才会回退。可用变量：<code>{"{{contactName}}"}</code>、<code>{"{{companyName}}"}</code>、<code>{"{{customerStage}}"}</code>、<code>{"{{lastMessage}}"}</code>。</p></div><div className="proactive-settings-block"><div><b>节假日与周末</b><small>周末和以下节日不会发送，系统会自动顺延到下一个可发送日。地区支持搜索中文、English 或国家/地区短代码。</small></div>{settings.holidays.map((holiday,index)=><div className="proactive-holiday-row" key={holiday.id}><input value={holiday.name} onChange={event=>patchHoliday(index,{name:event.target.value})} placeholder="节日名称"/><RegionPicker value={holiday.region} onChange={region=>patchHoliday(index,{region})}/><input type="number" min="1" max="12" value={holiday.month} onChange={event=>patchHoliday(index,{month:Number(event.target.value)})}/><span>月</span><input type="number" min="1" max="31" value={holiday.day} onChange={event=>patchHoliday(index,{day:Number(event.target.value)})}/><span>日</span><button type="button" className="danger-text" onClick={()=>setSettings(value=>({...value,holidays:value.holidays.filter((_,itemIndex)=>itemIndex!==index)}))}><Trash2 size={14}/></button></div>)}<button type="button" className="secondary-action" onClick={()=>setSettings(value=>({...value,holidays:[...value.holidays,{id:`holiday-${Date.now().toString(36)}`,name:"",month:1,day:1,region:"global"}]}))}><Plus size={14}/>新增节假日</button></div>{error&&<span className="login-error">{error}</span>}<div className="proactive-actions"><button className="primary-action" disabled={loading||saving||!accountId} onClick={()=>void save()}><Check size={14}/>{saving?"正在保存…":"保存主动触达设置"}</button><button type="button" className="secondary-action" disabled={loading||saving||scanning||!settings.enabled||!accountId} onClick={()=>void scan()}><Sparkles size={14}/>{scanning?"正在扫描…":"立即扫描"}</button></div></section></div>;
}
...
function PaymentMethodsSettingsPanel({token,onToken,onToast}:{token:string;onToken:(token:string)=>void;onToast:(text:string)=>void}){
  ...
  async function copyProfile(){if(!method||!profile)return;const body=await call(`/api/v1/admin/payment-methods/${method.id}/profiles`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({name:`${profile.name} Copy`,enabled:false,publicFields:profile.publicFields,instructions:profile.instructions,environment:profile.environment??undefined,referenceTemplate:profile.referenceTemplate,noteTemplate:profile.noteTemplate,itemNameTemplate:profile.itemNameTemplate,paymentTerm:profile.paymentTerm,paypalFeeRatePercent:profile.paypalFeeRatePercent,paypalFixedFee:profile.paypalFixedFee,paypalFeeLabel:profile.paypalFeeLabel})},"付款 Profile 已复制");if(body?.id)setSelectedProfileId(String(body.id));}
...
function ContactEditDialog({contactId,token,onToken,onClose,onSaved}:{contactId:string;token:string;onToken:(token:string)=>void;onClose:()=>void;onSaved:(profile:ContactProfile)=>Promise<void>}){
  ...
  return <><div className="modal-backdrop contact-dialog-backdrop" role="presentation"><section className="login-dialog contact-dialog" role="dialog" aria-modal="true" aria-labelledby="contact-dialog-title">
    ...
    {loading?<div className="contact-dialog-loading"><RefreshCw className="spin" size={17}/>正在读取联系人资料…</div>:profile?<>
      <p>{profile.contactName||profile.phone||`@${profile.whatsappUsername??""}`} · {profile.accountName}</p>
      ...
      <div className="contact-create-grid"><label>联系人名称<input value={alias} onChange={event=>setAlias(event.target.value)} maxLength={80} placeholder={profile.contactName||profile.phone||`@${profile.whatsappUsername??""}`}/><small>团队维护的显示名称不会被 WhatsApp 同步覆盖。</small></label><label>WhatsApp 号码<input value={phone} onChange={event=>setPhone(event.target.value)} inputMode="tel" disabled={profile.hasConversation&&Boolean(profile.phone)}/><small>{profile.hasConversation&&profile.phone?"该联系人已有对应会话，号码不可修改。":"手机号和用户名至少填写一个；号码需包含国家或地区代码。"}</small></label><label>WhatsApp 用户名<input value={whatsappUsername} onChange={event=>setWhatsappUsername(event.target.value.replace(/^@/,""))} maxLength={100} placeholder="例如：sam_lee"/><small>不含 @；收到 WhatsApp 消息时会自动同步。</small></label><label>First name<input value={firstName} onChange={event=>setFirstName(event.target.value)} maxLength={80} placeholder="名字"/></label><label>Middle name<input value={middleName} onChange={event=>setMiddleName(event.target.value)} maxLength={80} placeholder="中间名（可选）"/></label><label>Last name<input value={lastName} onChange={event=>setLastName(event.target.value)} maxLength={80} placeholder="姓氏"/></label></div>
...
function OrderDocumentHtmlPreviewDialog({order,preview,token,onToken,onClose}:{order:OrderItem;preview:OrderDocumentHtmlPreview;token:string;onToken:(token:string)=>void;onClose:()=>void}){
  return <div className="modal-backdrop order-template-preview-backdrop" role="presentation"><section className="login-dialog order-template-preview-dialog" role="dialog" aria-modal="true" aria-labelledby="order-template-preview-title"><button className="login-close" onClick={onClose} aria-label="关闭"><X size={17}/></button><header><span className="login-logo"><FileText size={19}/></span><div><h2 id="order-template-preview-title">{preview.documentType.toUpperCase()} HTML 预览</h2><p>{ORDER_DOCUMENT_LABELS[preview.documentType]} · 订单 #{order.orderNumber}</p></div></header><div className="order-template-preview-sheet">{preview.template.blocks.map(definition=>{const block=preview.blocks.find(value=>value.id===definition.id);if(!block)return null;const fontSize=definition.fontSize==="large"?"16px":definition.fontSize==="small"?"11px":"13px",style={color:definition.textColor,backgroundColor:definition.backgroundColor,textAlign:definition.align,fontSize},imageSize=definition.imageSize==="large"?152:definition.imageSize==="small"?52:88,vertical=definition.imageLayout==="top"||definition.imageLayout==="bottom",imageFirst=definition.imageLayout!=="right"&&definition.imageLayout!=="bottom",content=<div className="order-template-preview-lines">{block.lines.map((line,index)=>{const itemIndex=block.itemIndexes?.[index],item=typeof itemIndex==="number"&&itemIndex>=0?order.items[itemIndex]:null,outOfStock=block.outOfStockLineIndexes?.includes(index)??false,lineStyle=outOfStock?{color:"#D92D20",fontWeight:700}:undefined;return item&&definition.showProductImages!==false?<div className="order-template-preview-item" key={`${definition.id}-${index}`}><ProductImage mediaId={item.imageMediaId} externalUrl={item.imageUrl} token={token} onToken={onToken} alt={item.name} className="order-template-preview-product-image"/><p style={lineStyle}>{line||"\u00a0"}</p></div>:<p key={`${definition.id}-${index}`} style={lineStyle}>{line||"\u00a0"}</p>;})}</div>;if(definition.type==="customImage"&&definition.imageUrl)return <section key={definition.id} className="order-template-preview-block" style={style}><img src={definition.imageUrl} alt="" style={{width:imageSize,height:imageSize,objectFit:"contain"}}/></section>;if(definition.type==="imageText"&&definition.imageUrl)return <section key={definition.id} className="order-template-preview-block order-template-preview-image-text" style={{...style,flexDirection:vertical?"column":"row"}}><img src={definition.imageUrl} alt="" style={{width:vertical?"100%":imageSize,height:imageSize,objectFit:"contain",order:imageFirst?0:2}}/><div style={{order:imageFirst?1:0}}>{content}</div></section>;return <section key={definition.id} className={`order-template-preview-block ${definition.type==="divider"?"divider":""}`} style={style}>{content}</section>;})}</div></section></div>;
}
...
function ProductManagement({token,role,onToken,onToast}:{token:string;role:string;onToken:(token:string)=>void;onToast:(text:string)=>void}){
  const [products,setProducts]=useState<ProductItem[]>([]),[currencyConfig,setCurrencyConfig]=useState<CurrencyConfig>(DEFAULT_CURRENCY_CONFIG),[loading,setLoading]=useState(true),[refreshing,setRefreshing]=useState(false),[exporting,setExporting]=useState(false),[error,setError]=useState(""),[query,setQuery]=useState(""),[currency,setCurrency]=useState(""),[category,setCategory]=useState(""),[brand,setBrand]=useState(""),[tag,setTag]=useState(""),[stock,setStock]=useState(""),[tagNames,setTagNames]=useState<string[]>([]),[categoryNames,setCategoryNames]=useState<string[]>([]),[brandNames,setBrandNames]=useState<string[]>([]),[page,setPage]=useState(1),[pageSize,setPageSize]=useState<number>(PRODUCT_PAGE_SIZES[0]),[jumpPage,setJumpPage]=useState(""),[total,setTotal]=useState(0),[view,setView]=useState<"card"|"list">("card"),[editing,setEditing]=useState<ProductItem|"new"|null>(null),[importing,setImporting]=useState(false),[smartImporting,setSmartImporting]=useState(false),[selected,setSelected]=useState<string[]>([]),[bulkEditing,setBulkEditing]=useState(false),[generating,setGenerating]=useState(false);const loadVersion=useRef(0);
  ...
  const load=useCallback(async(options:{force?:boolean}={})=>{
    ...
    const input={page,pageSize,query:query.trim(),currency,category,brand,tag,stock},key=productCacheKey(subject,input);
    const params=new URLSearchParams({limit:String(pageSize),offset:String((page-1)*pageSize)});
     if(input.query)params.set("q",input.query);if(currency)params.set("currency",currency);if(category)params.set("category",category);if(brand)params.set("brand",brand);if(tag)params.set("tag",tag);if(stock)params.set("stock",stock);
  ...
  },[token,onToken,page,pageSize,query,currency,category,brand,tag,stock]);
  ...
  useEffect(()=>{
    ...
    const timer=window.setTimeout(()=>{
      ...
      for(const targetPage of adjacentPages){
        const input={page:targetPage,pageSize,query:query.trim(),currency,category,brand,tag,stock},key=productCacheKey(subject,input);
    ...
    return()=>window.clearTimeout(timer);
  },[loading,error,token,onToken,page,pageCount,pageSize,query,currency,category,brand,tag,stock]);
...
function NewConversationDialog({accounts,token,onToken,onClose,onCreated}:{accounts:Account[];token:string;onToken:(token:string)=>void;onClose:()=>void;onCreated:(conversationId:string,accountId:string,accessToken:string)=>Promise<void>}){
  const preferred=accounts.find(account=>account.status==="online")?.id??accounts[0]?.id??"";const [accountId,setAccountId]=useState(preferred);const [phone,setPhone]=useState("");const [whatsappUsername,setWhatsappUsername]=useState("");const [displayName,setDisplayName]=useState("");const [firstMessage,setFirstMessage]=useState("");const [busy,setBusy]=useState(false);const [error,setError]=useState("");const account=accounts.find(item=>item.id===accountId);
  ...
  async function submit(){if(!accountId||(!phone.trim()&&!whatsappUsername.trim())||!firstMessage.trim())return;if(account?.transport==="cloud"&&!phone.trim()){setError("Cloud API 仅支持通过 WhatsApp 号码发起会话");return;}setBusy(true);setError("");try{const result=await authorizedFetch("/api/v1/conversations",token,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({accountId,...(phone.trim()?{phone}:{whatsappUsername:whatsappUsername.trim()}),displayName:displayName.trim()||undefined,firstMessage:firstMessage.trim(),clientMessageId:crypto.randomUUID()})});if(result.token!==token)onToken(result.token);const body=await result.response.json() as {conversationId?:string;error?:string;details?:{fieldErrors?:Record<string,string[]>}};if(!result.response.ok||!body.conversationId){const detail=body.details?.fieldErrors?Object.values(body.details.fieldErrors).flat()[0]:undefined;throw new Error(detail||({account_not_found:"发送账号不存在或已解绑",invalid_request:"请检查号码、用户名和消息内容"} as Record<string,string>)[body.error??""]||`创建失败（HTTP ${result.response.status}）`);}await onCreated(body.conversationId,accountId,result.token);}catch(reason){setError(reason instanceof Error?reason.message:"新建会话失败");}finally{setBusy(false);}}
  return <div className="modal-backdrop" role="presentation"><section className="login-dialog new-conversation-dialog" role="dialog" aria-modal="true" aria-labelledby="new-conversation-title"><button className="login-close" onClick={onClose} disabled={busy} aria-label="关闭"><X size={17}/></button><span className="login-logo"><MessageCircle size={21}/></span><h2 id="new-conversation-title">新建 WhatsApp 会话</h2><p>仅向单个联系人发送，不提供群发。手机号和用户名至少填写一个；Cloud API 仅支持手机号。</p><label>发送账号<select value={accountId} onChange={event=>setAccountId(event.target.value)} disabled={busy||!accounts.length}>{accounts.length?accounts.map(item=><option value={item.id} key={item.id}>{item.name}（{statusText(item.status)}）</option>):<option value="">暂无已绑定账号</option>}</select></label><div className="conversation-form-grid"><label>WhatsApp 号码<input value={phone} onChange={event=>setPhone(event.target.value)} placeholder="例如：+8613800138000" inputMode="tel" autoFocus/></label><label>WhatsApp 用户名<input value={whatsappUsername} onChange={event=>setWhatsappUsername(event.target.value.replace(/^@/,""))} maxLength={100} placeholder="例如：alice_123"/><small>不含 @；可与手机号同时填写。</small></label><label>联系人名称（可选）<input value={displayName} onChange={event=>setDisplayName(event.target.value)} maxLength={80} placeholder="客户名称"/></label></div><label>首条消息<textarea value={firstMessage} onChange={event=>setFirstMessage(event.target.value)} maxLength={65536} placeholder="输入要发送的消息" onKeyDown={event=>{if(event.key==="Enter"&&(event.ctrlKey||event.metaKey))void submit();}}/></label>{account&&account.status!=="online"&&<span className="new-conversation-warning"><Clock3 size={13}/>当前账号离线，消息会先进入持久队列。</span>}{error&&<span className="login-error">{error}</span>}<button className="login-submit" disabled={busy||!accountId||(!phone.trim()&&!whatsappUsername.trim())||!firstMessage.trim()} onClick={()=>void submit()}>{busy?"正在创建…":"创建会话并发送"}</button><small className="dialog-hint">Ctrl / Cmd + Enter 快速提交</small></section></div>;
}
