...
export function WhatsAppInbox({initialView="inbox"}:{initialView?:WorkspaceView}) {
  ...
  async function retryMessage(message:ChatMessage){
    ...
    if(message.status==="uncertain"&&!window.confirm("这条消息可能已经送达。确定仍要重新发送吗？"))return;
  ...
  function addMessageToQuickReplies(message:ChatMessage){
    ...
    const next=[reply,...savedQuickReplies.filter(item=>item.sourceMessageId!==message.id)];localStorage.setItem(quickReplyStorageKey(userId,active.accountId),JSON.stringify(next));setSavedQuickReplies(next);
  ...
  return <main className="relay-shell">
    ...
    {materialLibraryOpen&&active&&<MaterialLibrarySendDialog accountId={active.accountId} conversationId={active.id} customerName={active.name} initialCaption={draft} request={taskRequest} onToken={setApiToken} onClose={()=>setMaterialLibraryOpen(false)} onSent={message=>{setDraft("");setToast(message);void loadMessages(apiToken,active.id);}}/>}
...
function QuickReplyDropdown({open,accountId,token,disabled,draftCaption,translationEnabled,savedReplies,onToken,onOpenChange,onText,onMedia}:{open:boolean;accountId:string;token:string;disabled:boolean;draftCaption:string;translationEnabled:boolean;savedReplies:SavedQuickReply[];onToken:(token:string)=>void;onOpenChange:(open:boolean)=>void;onText:(text:string)=>void;onMedia:(asset:MediaAsset,caption?:string)=>void}){
  ...
  useEffect(()=>{
    ...
    const timer=window.setTimeout(()=>{setLoading(true);setError("");inputRef.current?.focus();void (async()=>{try{const result=await authorizedFetch(`/api/v1/media?accountId=${encodeURIComponent(accountId)}&limit=100`,token,{signal:controller.signal});if(result.token!==token)onToken(result.token);const body=await result.response.json().catch(()=>({data:[]})) as {data?:Array<Record<string,unknown>>};if(!result.response.ok)throw new Error(`HTTP ${result.response.status}`);setAssets((body.data??[]).map(mapMediaAsset));}catch(reason){if(!controller.signal.aborted)setError(reason instanceof Error?reason.message:"快捷回复加载失败");}finally{if(!controller.signal.aborted)setLoading(false);}})();},0);
  ...
  const savedTexts=savedReplies.filter(item=>!item.attachment&&(filter==="all"||filter==="text")&&`${item.title} ${item.text} ${item.tags}`.toLocaleLowerCase().includes(normalized));
  const savedMedia=savedReplies.filter(item=>item.attachment&&(filter==="all"||filter===mediaKind(item.attachment.mimeType))&&`${item.title} ${item.text} ${item.tags} ${item.attachment.fileName}`.toLocaleLowerCase().includes(normalized));
  const savedMediaIds=new Set(savedMedia.map(item=>item.attachment!.id));
  const media=assets.filter(item=>{const kind=mediaKind(item.mimeType);return!savedMediaIds.has(item.id)&&(filter==="all"||filter===kind)&&`${item.fileName} ${kindText(kind)}`.toLocaleLowerCase().includes(normalized);});
  return <div className="quick-reply" ref={rootRef}><button type="button" className={`quick-reply-trigger ${open?"active":""}`} disabled={disabled} onClick={()=>onOpenChange(!open)} aria-haspopup="listbox" aria-expanded={open} aria-label="快捷回复"><Zap size={15}/><span>快捷回复</span>{savedReplies.length>0&&<i>{savedReplies.length}</i>}<ChevronDown size={12}/></button>{open&&<section className="quick-reply-menu" aria-label="搜索快捷回复"><header><label><Search size={14}/><input ref={inputRef} role="combobox" aria-expanded="true" aria-controls="quick-reply-options" value={query} onChange={event=>setQuery(event.target.value)} placeholder="搜索标题、内容、标签或文件名"/></label><button type="button" onClick={()=>onOpenChange(false)} aria-label="关闭快捷回复"><X size={14}/></button></header><div className="quick-reply-filters" role="tablist">{[["all","全部"],["text","文本"],["image","图文"],["audio","语音"],["video","视频"],["document","文件"]].map(([value,label])=><button type="button" key={value} role="tab" aria-selected={filter===value} className={filter===value?"active":""} onClick={()=>setFilter(value)}>{label}</button>)}</div><div className="quick-reply-options" id="quick-reply-options" role="listbox">{savedTexts.map(item=><button type="button" role="option" aria-selected="false" key={item.id} onClick={()=>onText(item.text)}><span className="quick-reply-kind saved"><Bookmark size={14}/></span><span><b>{item.title}</b><small>{item.text}</small></span><em>已保存</em></button>)}{texts.map(item=><button type="button" role="option" aria-selected="false" key={item.id} onClick={()=>onText(item.text)}><span className="quick-reply-kind text"><MessageCircle size={14}/></span><span><b>{item.title}</b><small>{item.text}</small></span>{translationEnabled&&<em><Languages size={10}/>自动翻译</em>}</button>)}{savedMedia.map(item=>{const asset=item.attachment!,kind=mediaKind(asset.mimeType);return <button type="button" role="option" aria-selected="false" key={item.id} onClick={()=>onMedia(asset,item.text)}><span className={`quick-reply-kind ${kind}`}>{kind==="audio"?<Mic size={14}/>:kind==="image"?<LayoutGrid size={14}/>:<FileText size={14}/>}</span><span><b>{item.title}</b><small>{kindText(kind)}{item.text?` · ${item.text}`:""}</small></span><em>已保存</em></button>})}{media.map(asset=>{const kind=mediaKind(asset.mimeType);return <button type="button" role="option" aria-selected="false" key={asset.id} onClick={()=>onMedia(asset)}><span className={`quick-reply-kind ${kind}`}>{kind==="audio"?<Mic size={14}/>:kind==="image"?<LayoutGrid size={14}/>:<FileText size={14}/>}</span><span><b>{asset.fileName}</b><small>{kindText(kind)} · {formatBytes(asset.size)}{kind==="image"&&draftCaption?` · 附带当前输入内容`:""}</small></span>{translationEnabled&&kind==="image"&&draftCaption&&<em><Languages size={10}/>说明自动翻译</em>}</button>})}{loading&&<p className="quick-reply-state"><RefreshCw className="spin" size={13}/>正在读取媒体快捷回复…</p>}{error&&<p className="quick-reply-state error">媒体快捷回复加载失败 · {error}</p>}{!loading&&!savedTexts.length&&!texts.length&&!savedMedia.length&&!media.length&&<p className="quick-reply-state">没有匹配的快捷回复</p>}</div><footer>文本会放入输入框供编辑；图文、语音等媒体将直接进入发送队列。</footer></section>}</div>;
}
...
function SettingsPanel({token,role,accounts,onToken,onToast}:{token:string;role:string;accounts:Account[];onToken:(token:string)=>void;onToast:(text:string)=>void}){
  const [tab,setTab]=useState<"cloud"|"apiKeys"|"agent"|"tasks"|"knowledge"|"translation"|"speech"|"email"|"currency"|"orders">("cloud");
  ...
  return <section className="management-panel settings-panel"><header className="management-head"><div><span className="eyebrow">系统设置</span><h1>工作区配置</h1><p>集中管理 WhatsApp、自动回复、知识库、Provider 和业务规则。</p></div></header><nav className="settings-tabs" aria-label="系统设置"><button className={tab==="cloud"?"active":""} onClick={()=>setTab("cloud")}><Phone size={15}/>WhatsApp API</button><button className={tab==="apiKeys"?"active":""} onClick={()=>setTab("apiKeys")}><ShieldCheck size={15}/>API 密钥</button><button className={tab==="agent"?"active":""} onClick={()=>setTab("agent")}><Bot size={15}/>AI Agent</button><button className={tab==="tasks"?"active":""} onClick={()=>setTab("tasks")}><Clock3 size={15}/>任务 Agent</button><button className={tab==="knowledge"?"active":""} onClick={()=>setTab("knowledge")}><BookOpen size={15}/>知识库</button><button className={tab==="translation"?"active":""} onClick={()=>setTab("translation")}><Languages size={15}/>AI 翻译</button><button className={tab==="speech"?"active":""} onClick={()=>setTab("speech")}><Mic size={15}/>AI 语音</button><button className={tab==="email"?"active":""} onClick={()=>setTab("email")}><Mail size={15}/>邮件发送</button><button className={tab==="currency"?"active":""} onClick={()=>setTab("currency")}><CreditCard size={15}/>货币管理</button><button className={tab==="orders"?"active":""} onClick={()=>setTab("orders")}><ClipboardList size={15}/>订单设置</button></nav>{tab==="cloud"?<CloudApiSettingsPanel token={token} onToken={onToken} onToast={onToast}/>:tab==="apiKeys"?<ApiKeySettingsPanel token={token} onToken={onToken} onToast={onToast}/>:tab==="agent"?<AiAgentSettingsPanel token={token} accounts={accounts} onToken={onToken} onToast={onToast}/>:tab==="tasks"?<TaskAgentSettingsPanel token={token} accounts={accounts} onToken={onToken} onToast={onToast}/>:tab==="knowledge"?<KnowledgeBaseSettingsPanel token={token} onToken={onToken} onToast={onToast}/>:tab==="translation"?<TranslationSettingsPanel token={token} onToken={onToken} onToast={onToast}/>:tab==="speech"?<TtsSettingsPanel token={token} role={role} onToken={onToken} onToast={onToast}/>:tab==="email"?<EmailSettingsPanel token={token} onToken={onToken} onToast={onToast}/>:tab==="currency"?<CurrencySettingsPanel token={token} role={role} onToken={onToken} onToast={onToast}/>:<OrderSettingsPanel token={token} onToken={onToken} onToast={onToast}/>}</section>;
}
