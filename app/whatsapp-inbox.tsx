...
function GroupDetailsPanel({active,accounts,token,onToken,onClose,onToast,onConversation}:{active:Conversation;accounts:Account[];token:string;onToken:(token:string)=>void;onClose:()=>void;onToast:(text:string)=>void;onConversation:(conversationId:string)=>Promise<void>}){
  ...
  </aside>{creatingMember&&account&&<ContactCreateDialog accounts={[account]} token={token} onToken={onToken} initialAccountId={active.accountId} initialPhone={participantLabel(creatingMember.phone_jid||creatingMember.participant_jid)} initialName={creatingMember.display_name||""} lockAccount onClose={()=>setCreatingMember(null)} onSaved={async profile=>{setCreatingMember(null);onToast("联系人已添加");await load();setViewingContactId(profile.id);}}/>}{viewingContactId&&<ContactProfilePreviewDialog contactId={viewingContactId} token={token} onToken={onToken} onClose={()=>setViewingContactId(null)}/>}</>;
}
...
function ContactCreateDialog({accounts,token,onToken,onClose,onSaved,initialAccountId="",initialPhone="",initialName="",lockAccount=false}:{accounts:Account[];token:string;onToken:(token:string)=>void;onClose:()=>void;onSaved:(profile:ContactProfile)=>Promise<void>;initialAccountId?:string;initialPhone?:string;initialName?:string;lockAccount?:boolean}){
  const [accountId,setAccountId]=useState(initialAccountId||accounts[0]?.id||""),[firstName,setFirstName]=useState(initialName.slice(0,80)),[middleName,setMiddleName]=useState(""),[lastName,setLastName]=useState(""),[phone,setPhone]=useState(initialPhone),[avatar,setAvatar]=useState<File|null>(null),[avatarPickerOpen,setAvatarPickerOpen]=useState(false),[busy,setBusy]=useState(false),[error,setError]=useState("");
...
function ContactProfilePreviewDialog({contactId,token,onToken,onClose}:{contactId:string;token:string;onToken:(token:string)=>void;onClose:()=>void}){
  ...
  return <div className="modal-backdrop contact-dialog-backdrop" role="presentation"><section className="login-dialog contact-preview-dialog" role="dialog" aria-modal="true" aria-labelledby="contact-preview-title"><button className="login-close" onClick={onClose} aria-label="关闭"><X size={17}/></button>{!profile&&!error?<div className="contact-dialog-loading"><RefreshCw className="spin" size={17}/>正在读取联系人资料…</div>:error?<span className="login-error">{error}</span>:profile&&<><header className="contact-preview-heading"><ContactAvatar contact={profile} token={token} onToken={onToken} size="large"/><div><h2 id="contact-preview-title">{profile.name}</h2><p>{profile.accountName}</p></div></header><dl className="contact-preview-fields"><div><dt>WhatsApp</dt><dd>{profile.phone||"未填写"}</dd></div><div><dt>Primary Email</dt><dd>{profile.primaryEmail||"未填写"}</dd></div><div><dt>偏好语言</dt><dd>{profile.preferredLanguage?languageName(profile.preferredLanguage):"未设置"}</dd></div><div><dt>时区</dt><dd>{profile.effectiveTimezone}</dd></div></dl>{profile.methods.length>0&&<section className="contact-preview-section"><h3>其他联系方式</h3>{profile.methods.map((method,index)=><p key={method.id??index}><b>{method.label||contactMethodName(method.type)}</b><span>{method.value}</span></p>)}</section>}{profile.note&&<section className="contact-preview-section"><h3>联系人备注</h3><div>{profile.note}</div></section>}<ContactSocialLinks methods={profile.methods}/><footer className="contact-dialog-actions"><button className="primary-action" onClick={onClose}>关闭</button></footer></>}</section></div>;
}
