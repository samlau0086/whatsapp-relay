...
export function WhatsAppInbox({initialView="inbox"}:{initialView?:WorkspaceView}) {
  ...
  function deleteQuickReply(item:SavedQuickReply){
    if(!window.confirm(`删除快捷回复“${item.title}”？`))return;persistQuickReplies(savedQuickReplies.filter(value=>value.id!==item.id));setToast("快捷回复已删除");
  }
