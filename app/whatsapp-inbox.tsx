...
type ShippingRule={shippingClassId:string|null;shippingClassName:string|null;mode:"quantity"|"weight";firstItemPrice?:number;additionalItemPrice?:number;firstWeight?:number;additionalWeight?:number;weightUnit?:WeightUnit;firstWeightPrice?:number;additionalWeightPrice?:number};
    ...
    >
      ...
      >
        ...
        <section className="order-shipping-calculator">
          ...
          {shippingQuoteLoading?<p className="order-empty-fees">正在计算建议运费…</p>:shippingQuote?<div className="shipping-quote-result"><header><span>建议运费</span><strong>{currency} {shippingQuote.amount.toFixed(2)}</strong><button type="button" className="primary-action" onClick={()=>{setShippingAmount(shippingQuote.amount.toFixed(2));setAcceptCalculatedShipping(true);}}>一键采用</button></header><div>{shippingQuote.breakdown.map((item,index)=><p key={`${item.shippingClassId??"default"}-${index}`}><b>{item.shippingClassName}</b><span>{item.mode==="quantity"?`${item.quantity} 件`:`${Number(item.weightAmount??0).toFixed(3)} ${item.weightUnit}`}</span><em>{currency} {item.orderAmount.toFixed(2)}</em></p>)}</div><small>模板 {shippingQuote.template.name} v{shippingQuote.template.version} · {shippingQuote.template.currency} {shippingQuote.templateAmount.toFixed(2)} × {shippingQuote.exchange.rate.toFixed(6)}{shippingQuote.exchange.rateDate?` · 汇率日期 ${shippingQuote.exchange.rateDate}`:""}</small>{acceptCalculatedShipping&&<small className="shipping-quote-applied">已采用；保存时服务端将重新核算并留存快照。</small>}</div>:shippingQuoteError?<span className="login-error">{shippingQuoteError}</span>:<p className="order-empty-fees">选择已启用的运费模板后自动计算</p>}
