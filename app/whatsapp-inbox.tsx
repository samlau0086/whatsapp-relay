                      ...
                      >
                        ...
                        >
                          ...
                          <div className="message-actions">
                            ...
                            <button className="message-comment-action" onClick={()=>setExpandedMessageComments(all=>{const next=new Set(all);if(next.has(message.id))next.delete(message.id);else next.add(message.id);return next;})} aria-label="内部评论" title="内部评论（客户不可见）"><MessageSquare size={14}/>{message.comments.length>0&&<i>{message.comments.length}</i>}</button>
                          </div>
                          {message.quoted && (
                            <QuotedMessage
                          ...
                          {(expandedMessageComments.has(message.id)||message.comments.length>0)&&<section className="message-comments" aria-label="内部评论">
