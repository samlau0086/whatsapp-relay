...
function OrderSettingsPanel({token,onToken,onToast}:{token:string;onToken:(token:string)=>void;onToast:(text:string)=>void}){
  ...
  async function select(next:"number"|"paypal"|"product-card"|TemplateFormat){if(next===section)return;if(dirty&&!await confirmAction("当前模板的未保存修改将会丢失。",{title:"放弃未保存的修改？",confirmLabel:"放弃并离开",tone:"warning"}))return;setDirty(false);setSection(next);}
