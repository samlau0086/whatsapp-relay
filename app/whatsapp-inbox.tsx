...
type ConversationContact={accountId:string;phone:string;name:string};
...
export function WhatsAppInbox({initialView="inbox"}:{initialView?:WorkspaceView}) {
  ...
  const [newConversationOpen,setNewConversationOpen]=useState(false),[newConversationContact,setNewConversationContact]=useState<ConversationContact|null>(null);
    ...
    <main className={`relay-shell ${filtersHidden ? "filters-hidden" : ""} ${conversationListHidden ? "conversation-list-hidden" : ""}`}>
      ...
      {view === "inbox" ? (
            ...
            <div className="workspace-title">
              ...
              <span className="workspace-title-actions">
                ...
                <button
                  ...
                  onClick={() => {setNewConversationContact(null);setNewConversationOpen(true);}}
                  aria-label="新建会话"
                  title="新建会话"
                >
                  <Plus size={16} />
        ...
        <ContactManagement
          ...
          onStartConversation={(contact) => {
            ...
            setNewConversationContact({accountId:contact.accountId,phone:contact.phone,name:contact.name});
      ...
      {newConversationOpen && (
        <NewConversationDialog
          accounts={accounts.filter(account=>account.platform==="whatsapp"&&(!newConversationContact||account.id===newConversationContact.accountId))}
          initialContact={newConversationContact??undefined}
          token={apiToken}
          onToken={setApiToken}
          onClose={() => {setNewConversationOpen(false);setNewConversationContact(null);}}
          onCreated={async (conversationId, accountId, accessToken) => {
            setNewConversationOpen(false);
            setNewConversationContact(null);
            navigate("inbox");
            setFilter("全部会话");
...
function NewConversationDialog({accounts,initialContact,token,onToken,onClose,onCreated}:{accounts:Account[];initialContact?:ConversationContact;token:string;onToken:(token:string)=>void;onClose:()=>void;onCreated:(conversationId:string,accountId:string,accessToken:string)=>Promise<void>}){
  const preferred=initialContact?.accountId??accounts.find(account=>account.status==="online")?.id??accounts[0]?.id??"";const [accountId,setAccountId]=useState(preferred);const [phone,setPhone]=useState(initialContact?.phone??"");const [displayName,setDisplayName]=useState(initialContact?.name??"");const [firstMessage,setFirstMessage]=useState("");const [busy,setBusy]=useState(false);const [error,setError]=useState("");const account=accounts.find(item=>item.id===accountId);
  if(account?.transport==="cloud")return <CloudNewConversationDialog accounts={accounts} accountId={accountId} onAccountId={setAccountId} initialContact={initialContact} token={token} onToken={onToken} onClose={onClose} onCreated={onCreated}/>;
  ...
  return <div className="modal-backdrop" role="presentation"><section className="login-dialog new-conversation-dialog" role="dialog" aria-modal="true" aria-labelledby="new-conversation-title"><button className="login-close" onClick={onClose} disabled={busy} aria-label="关闭"><X size={17}/></button><span className="login-logo"><MessageCircle size={21}/></span><h2 id="new-conversation-title">{initialContact?`向 ${initialContact.name} 发起会话`:"新建 WhatsApp 会话"}</h2><p>仅向单个号码发送，不提供群发。号码必须包含国家或地区代码。</p><label>发送账号<select value={accountId} onChange={event=>setAccountId(event.target.value)} disabled={busy||!accounts.length}>{accounts.length?accounts.map(item=><option value={item.id} key={item.id}>{item.name}（{statusText(item.status)}）</option>):<option value="">暂无已绑定账号</option>}</select></label><div className="conversation-form-grid"><label>WhatsApp 号码<input value={phone} onChange={event=>setPhone(event.target.value)} placeholder="例如：+8613800138000" inputMode="tel" readOnly={Boolean(initialContact)} autoFocus/></label><label>联系人名称（可选）<input value={displayName} onChange={event=>setDisplayName(event.target.value)} maxLength={80} placeholder="客户名称" readOnly={Boolean(initialContact)}/></label></div><label>首条消息<textarea value={firstMessage} onChange={event=>setFirstMessage(event.target.value)} maxLength={65536} placeholder="输入要发送的消息" onKeyDown={event=>{if(event.key==="Enter"&&(event.ctrlKey||event.metaKey))void submit();}}/></label>{account&&account.status!=="online"&&<span className="new-conversation-warning"><Clock3 size={13}/>当前账号离线，消息会先进入持久队列。</span>}{error&&<span className="login-error">{error}</span>}<button className="login-submit" disabled={busy||!accountId||!phone.trim()||!firstMessage.trim()} onClick={()=>void submit()}>{busy?"正在创建…":"创建会话并发送"}</button><small className="dialog-hint">Ctrl / Cmd + Enter 快速提交</small></section></div>;
}

function CloudNewConversationDialog({accounts,accountId,onAccountId,initialContact,token,onToken,onClose,onCreated}:{accounts:Account[];accountId:string;onAccountId:(id:string)=>void;initialContact?:ConversationContact;token:string;onToken:(token:string)=>void;onClose:()=>void;onCreated:(conversationId:string,accountId:string,accessToken:string)=>Promise<void>}){
  const [phone,setPhone]=useState(initialContact?.phone??""),[displayName,setDisplayName]=useState(initialContact?.name??""),[templates,setTemplates]=useState<CloudTemplate[]>([]),[assets,setAssets]=useState<MediaAsset[]>([]),[selected,setSelected]=useState(""),[headerMediaId,setHeaderMediaId]=useState(""),[values,setValues]=useState<string[]>([]),[busy,setBusy]=useState(false),[error,setError]=useState("");
