...
type OrderProductItem={id:string;name:string;sku:string;quantity:number;unitAmount:number;weightAmount:number|null;weightUnit:WeightUnit|null;shippingClassId:string|null;shippingClassName:string|null;imageMediaId:string|null;imageName:string;productId:string|null};
...
function ContactManagement({token,role,accounts,onToken,onToast,onConversation}:{token:string;role:string;accounts:Account[];onToken:(token:string)=>void;onToast:(text:string)=>void;onConversation:(conversationId:string)=>void}){
  ...
  function jumpToContactPage(){const target=Number(jumpPage);if(Number.isInteger(target)&&target>0)goToPage(target);}
...
function mapProduct(item:Record<string,unknown>):ProductItem{const priceTiers=Array.isArray(item.priceTiers)?(item.priceTiers as Array<Record<string,unknown>>).map(tier=>({minQuantity:Number(tier.minQuantity),unitAmount:Number(tier.unitAmount)})):[],variants=Array.isArray(item.variants)?(item.variants as Array<Record<string,unknown>>).map(variant=>({id:String(variant.id),attributes:(variant.attributes??{}) as Record<string,string>,sku:String(variant.sku),priceTiers:Array.isArray(variant.priceTiers)?(variant.priceTiers as Array<Record<string,unknown>>).map(tier=>({minQuantity:Number(tier.minQuantity),unitAmount:Number(tier.unitAmount)})):[],imageMediaId:variant.imageMediaId?String(variant.imageMediaId):null})):[];return{id:String(item.id),sku:String(item.sku??""),name:String(item.name),description:String(item.description??""),category:String(item.category??""),brand:String(item.brand??""),defaultUnitAmount:priceTiers[0]?.unitAmount??Number(item.defaultUnitAmount),priceTiers,currency:String(item.currency),weightAmount:item.weightAmount===null||item.weightAmount===undefined?null:Number(item.weightAmount),weightUnit:item.weightUnit?String(item.weightUnit) as WeightUnit:null,shippingClassId:item.shippingClassId?String(item.shippingClassId):null,shippingClass:item.shippingClass?String(item.shippingClass):null,imageMediaId:item.imageMediaId?String(item.imageMediaId):null,imageName:String(item.imageName??""),tags:Array.isArray(item.tags)?(item.tags as Array<Record<string,unknown>>).map(mapTag):[],variants,createdAt:String(item.createdAt),updatedAt:String(item.updatedAt)};}
