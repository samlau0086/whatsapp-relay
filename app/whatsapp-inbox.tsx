...
function OrderDialog({
  ...
  const displayFees=useMemo(()=>paypalFeeAmount>0?[...manualFees,{id:"paypal-fee",name:"PayPal 手续费",amount:paypalFeeAmount,source:"paypal" as const}]:manualFees,[manualFees,paypalFeeAmount]);
...
function PaymentMethodsSettingsPanel({token,onToken,onToast}:{token:string;onToken:(token:string)=>void;onToast:(text:string)=>void}){
  ...
  async function copyProfile(){if(!method||!profile)return;const body=await call(`/api/v1/admin/payment-methods/${method.id}/profiles`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({name:`${profile.name} Copy`,enabled:false,publicFields:profile.publicFields,instructions:profile.instructions,environment:profile.environment??undefined,referenceTemplate:profile.referenceTemplate,noteTemplate:profile.noteTemplate,itemNameTemplate:profile.itemNameTemplate,paypalFeeRatePercent:profile.paypalFeeRatePercent,paypalFixedFee:profile.paypalFixedFee})},"付款 Profile 已复制");if(body?.id)setSelectedProfileId(String(body.id));}
