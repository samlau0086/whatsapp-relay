...
type ProductVariant={id:string;attributes:Record<string,string>;sku:string;priceTiers:ProductPriceTier[];imageMediaId:string|null};
...
type OrderItem={id:string;orderNumber:string;conversationId:string;accountId:string;accountName:string;customerName:string;customerPhone:string;amount:number;currency:string;weightUnit:WeightUnit;description:string;status:string;businessStatus:OrderBusinessStatus;sendFormat:string;translateOnSend:boolean;targetLanguage:string;createdAt:string;createdByName:string;messageStatus:string;items:OrderProductItem[];fees:OrderFeeItem[];shippingAmount:number|null;shippingTemplateId:string|null;shippingQuote:ShippingQuote|null;addressId:string|null;address:CustomerAddress|null;paymentProfileId:string|null;paymentProfile:PaymentProfile|null;paymentRequest:PaymentRequest|null};
...
function mapCustomerAddress(item:Record<string,unknown>,id=String(item.id??"")):CustomerAddress{return{id,label:String(item.label??"地址"),recipientName:String(item.recipientName??item.recipient_name??""),phone:String(item.phone??""),address:String(item.address??""),countryCode:String(item.countryCode??item.country_code??""),province:String(item.province??""),isDefault:Boolean(item.isDefault??item.is_default)};}
...
function OrderDialog({
  ...
  const [products, setProducts] = useState<DraftProduct[]>(() =>
    ...
    [addressDraft, setAddressDraft] = useState({label:order?.address?.label??"收货地址",recipientName:order?.address?.recipientName??active.name,phone:order?.address?.phone??active.phone,address:order?.address?.address??"",countryCode:order?.address?.countryCode??"",province:order?.address?.province??""}),
  ...
  const total = useMemo(
    () =>
      products.reduce(
        ...
        0,
      ) + fees.reduce((sum, fee) => sum + (Number(fee.amount) || 0), 0)+(Number(shippingAmount)||0),
    [products, fees,shippingAmount],
  );
  ...
  async function submit() {
    ...
    try {
      ...
      const payload = {
        ...
        ...(addingAddress?{newAddress:{label:addressDraft.label.trim(),recipientName:addressDraft.recipientName.trim()||undefined,phone:addressDraft.phone.trim()||undefined,address:addressDraft.address.trim(),countryCode:addressDraft.countryCode.trim().toUpperCase()||undefined,province:addressDraft.province.trim()||undefined}}:{addressId:addressId||null}),
      };
      ...
      if(order){const addressSaved=await authorizedFetch(`/api/v1/conversations/${active.id}/orders/${order.id}/address`,saved.token,{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify(addingAddress?{newAddress:{label:addressDraft.label.trim(),recipientName:addressDraft.recipientName.trim()||undefined,phone:addressDraft.phone.trim()||undefined,address:addressDraft.address.trim(),countryCode:addressDraft.countryCode.trim().toUpperCase()||undefined,province:addressDraft.province.trim()||undefined}}:{addressId:addressId||null})});if(addressSaved.token!==token)onToken(addressSaved.token);if(!addressSaved.response.ok)throw new Error("订单已更新，但地址保存失败，请重试");}
...
function SettingsPanel({token,role,accounts,onToken,onToast}:{token:string;role:string;accounts:Account[];onToken:(token:string)=>void;onToast:(text:string)=>void}){
  const [tab,setTab]=useState<"cloud"|"messenger"|"apiKeys"|"agent"|"tasks"|"knowledge"|"translation"|"speech"|"email"|"currency"|"orders">(()=>typeof window!=="undefined"&&(new URLSearchParams(window.location.search).has("messengerOauth")||new URLSearchParams(window.location.search).has("messengerOauthError"))?"messenger":"cloud");
...
function ContactManagement({token,role,accounts,onToken,onToast,onConversation}:{token:string;role:string;accounts:Account[];onToken:(token:string)=>void;onToast:(text:string)=>void;onConversation:(conversationId:string)=>void}){
  const [contacts,setContacts]=useState<ContactProfile[]>([]),[query,setQuery]=useState(""),[accountId,setAccountId]=useState(""),[offset,setOffset]=useState(0),[pageSize,setPageSize]=useState<number>(CONTACT_PAGE_SIZES[0]),[view,setView]=useState<"card"|"list">("list"),[jumpPage,setJumpPage]=useState(""),[total,setTotal]=useState(0),[loading,setLoading]=useState(true),[error,setError]=useState(""),[creating,setCreating]=useState(false),[importing,setImporting]=useState(false),[exporting,setExporting]=useState(false),[editingId,setEditingId]=useState<string|null>(null),[addressEditingId,setAddressEditingId]=useState<string|null>(null),[selected,setSelected]=useState<string[]>([]),[bulkEditing,setBulkEditing]=useState(false);
...
function ContactAddressDialog({contactId,token,onToken,onClose,onSaved}:{contactId:string;token:string;onToken:(token:string)=>void;onClose:()=>void;onSaved:(profile:ContactProfile)=>Promise<void>}){
  ...
  function addAddress(){setAddresses(items=>[...items,{id:"",label:"收货地址",recipientName:profile?.name??"",phone:profile?.phone??"",address:"",countryCode:"",province:"",isDefault:items.length===0}]);}
  ...
  async function save(){if(!profile)return;if(addresses.some(item=>!item.label.trim()||!item.address.trim())){setError("请填写地址名称和详细地址，或移除未完成的地址");return;}if(addresses.some(item=>item.countryCode&&!/^[A-Za-z]{2}$/.test(item.countryCode.trim())||item.province.trim()&&!item.countryCode.trim())){setError("国家代码需为两位字母，填写省份时国家代码必填");return;}setBusy(true);setError("");try{const result=await authorizedFetch(`/api/v1/contacts/${contactId}`,token,{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({alias:profile.alias,firstName:profile.firstName,middleName:profile.middleName,lastName:profile.lastName,note:profile.note,emails:profile.emails.map(({label,email,isPrimary})=>({label,email,isPrimary})),methods:profile.methods.map(({type,label,value})=>({type,label,value})),addresses:addresses.map(item=>({...(item.id?{id:item.id}:{}),label:item.label,recipientName:item.recipientName,phone:item.phone,address:item.address,countryCode:item.countryCode.trim().toUpperCase()||undefined,province:item.province.trim()||undefined,isDefault:item.isDefault}))})});if(result.token!==token)onToken(result.token);const body=await result.response.json().catch(()=>({})) as Record<string,unknown>&{error?:string};if(!result.response.ok)throw new Error(body.error??`地址保存失败（HTTP ${result.response.status}）`);await onSaved(mapContactProfile(body));}catch(reason){setError(reason instanceof Error?reason.message:"地址保存失败");setBusy(false);}}
