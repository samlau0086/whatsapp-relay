...
ORDER BY COALESCE(c.last_message_at,c.created_at) DESC,c.id DESC LIMIT 40;
EXPLAIN (ANALYZE,BUFFERS,FORMAT JSON)
WITH search_conversations AS MATERIALIZED (
  SELECT search_conversation.id,search_conversation.last_message_at,search_conversation.created_at
  FROM conversations search_conversation WHERE search_conversation.last_message_text ILIKE '%summary 9999%'
  UNION
  ...
  WHERE (COALESCE(search_contact.alias,'') || ' ' || COALESCE(search_contact.display_name,'') || ' ' ||
    COALESCE(search_contact.phone_e164,'') || ' ' || COALESCE(search_contact.provider_user_id,'') || ' ' ||
    COALESCE(search_contact.whatsapp_username,'')) ILIKE '%Contact 9999%'
)
