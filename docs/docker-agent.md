...
在服务器本机浏览器打开 `http://127.0.0.1:8788`，即可查看 Agent 状态并扫描 WhatsApp 关联二维码。远程服务器推荐通过 SSH 隧道访问：

```bash
ssh -L 8788:127.0.0.1:8788 <user>@<server>
```

然后在本机浏览器打开 `http://127.0.0.1:8788`。用手机 WhatsApp 的 **关联设备** 扫描界面显示的二维码；页面变为“已连接”即完成。
