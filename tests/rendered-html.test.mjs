...
test("workspace includes the reliable-sync UI and responsive breakpoints", async()=>{
  ...
  assert.match(component,/当前会话 · AI 双向翻译/);assert.match(component,/此会话偏好会跨浏览器同步/);assert.match(component,/TranslationPreviewDialog/);assert.match(component,/IncomingTranslation/);assert.match(component,/VoiceTranslation/);assert.match(component,/AI 翻译语音为/);assert.match(component,/翻译服务暂时不可用/);assert.match(component,/gpt-4o-mini-transcribe/);assert.match(component,/conversationId/);assert.match(component,/\/api\/v1\/me\/translation-preferences/);assert.match(component,/\/api\/v1\/translations\/preview/);assert.match(component,/\/api\/v1\/translations\/messages/);assert.match(component,/Custom Provider/);assert.match(component,/gpt-5\.6-luna/);assert.match(css,/\.translation-menu/);assert.match(css,/\.incoming-translation/);assert.match(css,/\.voice-translate-action/);assert.match(css,/\.settings-tabs/);
  assert.match(component,/原文（仅坐席可见）/);assert.match(component,/translationSourceText/);assert.match(component,/generateAudio/);assert.match(component,/value\.status==="idle"/);assert.match(css,/\.outgoing-translation-source/);
  assert.match(component,/CrmDetailsPanel/);assert.match(component,/创建订单/);assert.match(component,/编辑订单/);assert.match(component,/重新发送/);assert.match(component,/clientSendId:crypto\.randomUUID/);assert.match(component,/method:order\?"PATCH":"POST"/);assert.match(component,/客户阶段/);assert.match(component,/我的提醒/);assert.match(component,/添加团队共享备注/);assert.match(component,/新标签名称/);assert.match(component,/Add product/);assert.match(component,/Additional fees/);assert.match(component,/AI translation on send/);assert.match(component,/Save order draft/);assert.match(component,/orders\/\$\{order\.id\}\/send/);assert.match(component,/method:"DELETE"/);assert.match(component,/文字版详情/);assert.match(component,/图片版完整详情/);assert.match(component,/不会撤回已发送的 WhatsApp 消息/);assert.match(component,/\/reminder/);assert.match(css,/\.crm-details/);assert.match(css,/\.details-backdrop/);assert.match(css,/\.order-builder/);assert.match(css,/\.order-send-options/);assert.match(css,/\.order-edit/);assert.match(css,/\.order-delete/);
});
