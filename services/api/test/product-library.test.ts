...
test("product routes enforce shared media, snapshots, idempotency, and soft deletion",async()=>{
  ...
  assert.match(server,/INSERT INTO order_items\(order_id,position,product_name,product_sku,quantity,unit_amount,weight_amount,weight_unit,image_media_id,image_url,product_id,variant_id,shipping_class_id,shipping_class_name,internal_note_snapshot\)/);
