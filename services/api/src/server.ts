...
app.patch("/api/v1/contacts/:id",{preHandler:authenticate},async(request,reply)=>{
  ...
  const updated=await transaction(async client=>{
    ...
    await client.query("UPDATE contacts SET alias=$2,note=$3,phone_e164=CASE WHEN $4::text IS NULL THEN phone_e164 ELSE $4 END,wa_jid=CASE WHEN $5::text IS NULL THEN wa_jid ELSE $5 END,birthday_month=CASE WHEN $6 THEN $7 ELSE birthday_month END,birthday_day=CASE WHEN $6 THEN $8 ELSE birthday_day END,birthday_year=CASE WHEN $6 THEN $9 ELSE birthday_year END,timezone=CASE WHEN $10 THEN $11 ELSE timezone END,preferred_language=CASE WHEN $12 THEN $13 ELSE preferred_language END,updated_at=now() WHERE id=$1",[id,parsed.data.alias||null,parsed.data.note||null,nextPhone,parsed.data.phone?`${parsed.data.phone}@s.whatsapp.net`:null,parsed.data.birthday!==undefined,parsed.data.birthday?.month??null,parsed.data.birthday?.day??null,parsed.data.birthday?.year??null,parsed.data.timezone!==undefined,parsed.data.timezone||null,parsed.data.preferredLanguage!==undefined,parsed.data.preferredLanguage||null]);
