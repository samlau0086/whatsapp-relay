...
app.get("/api/v1/orders/:orderId/documents/:document",{preHandler:authenticate},async(request,reply)=>{
  ...
  const [itemsResult,feesResult,settingsResult]=await Promise.all([
    pool.query("SELECT i.product_name name,i.product_sku sku,i.quantity,i.unit_amount,i.image_url,p.brand,p.category,m.object_key FROM order_items i LEFT JOIN media m ON m.id=i.image_media_id LEFT JOIN products p ON p.id=i.product_id WHERE i.order_id=$1 ORDER BY i.position",[orderId]),
  ...
  const row=found.rows[0],items:OrderSummaryItem[]=itemsResult.rows.map(item=>({name:String(item.name),sku:String(item.sku??""),quantity:Number(item.quantity),unitAmount:Number(item.unit_amount),brand:String(item.brand??""),category:String(item.category??"")})),fees:OrderSummaryFee[]=feesResult.rows.map(fee=>({name:String(fee.name),amount:Number(fee.amount)}));
