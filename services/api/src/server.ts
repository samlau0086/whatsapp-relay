...
app.get("/api/v1/admin/paypal-settings",{preHandler:authenticate},async(request,reply)=>{
  ...
  const result=await pool.query("SELECT enabled,environment,client_id_encrypted,client_secret_encrypted,reference_template,note_template,item_name_template,updated_at FROM paypal_settings WHERE singleton=true"),row=result.rows[0]??{};
...
app.put("/api/v1/admin/paypal-settings",{preHandler:authenticate},async(request,reply)=>{
  ...
  const saved=await pool.query("INSERT INTO paypal_settings(singleton,enabled,environment,client_id_encrypted,client_secret_encrypted,reference_template,note_template,item_name_template,updated_by) VALUES(true,$1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT(singleton) DO UPDATE SET enabled=EXCLUDED.enabled,environment=EXCLUDED.environment,client_id_encrypted=EXCLUDED.client_id_encrypted,client_secret_encrypted=EXCLUDED.client_secret_encrypted,reference_template=EXCLUDED.reference_template,note_template=EXCLUDED.note_template,item_name_template=EXCLUDED.item_name_template,updated_by=EXCLUDED.updated_by,updated_at=now() RETURNING enabled,environment,reference_template,note_template,item_name_template,updated_at",[parsed.data.enabled,parsed.data.environment,clientIdEncrypted,clientSecretEncrypted,parsed.data.referenceTemplate,parsed.data.noteTemplate,parsed.data.itemNameTemplate,request.principal.id]);
  await auditCrm(request.principal.id,"paypal.settings.update","paypal_settings","workspace",{enabled:parsed.data.enabled,environment:parsed.data.environment,clientIdChanged:Boolean(parsed.data.clientId),clientSecretChanged:Boolean(parsed.data.clientSecret),templatesChanged:true});clearPayPalTokenCache();
  return{enabled:saved.rows[0].enabled,environment:saved.rows[0].environment,clientIdConfigured:Boolean(clientIdEncrypted),clientSecretConfigured:Boolean(clientSecretEncrypted),referenceTemplate:saved.rows[0].reference_template,noteTemplate:saved.rows[0].note_template,itemNameTemplate:saved.rows[0].item_name_template,updatedAt:saved.rows[0].updated_at};
});
