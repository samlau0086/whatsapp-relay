...
async function acceptedShipping(client:PoolClient,data:{currency:string;items:OrderProductInput[];shippingAmount?:number|null;shippingTemplateId?:string|null;acceptCalculatedShipping:boolean}){
  ...
  const quote=await calculateShippingQuote(client,{templateId:data.shippingTemplateId!,currency:data.currency,items},{enabledOnly:true});
