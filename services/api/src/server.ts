...
import { conversationTagsSchema, currencySchema, customerStageSchema, enrollmentSchema, loginSchema, messageSchema, messageTranslationsSchema, newConversationSchema, noteSchema, orderAddressSchema, orderSchema, orderSendSchema, orderSettingsSchema, orderUpdateSchema, productCreateSchema, productUpdateSchema, reminderSchema, tagCreateSchema, tagUpdateSchema, textToSpeechSchema, translationPreferenceQuerySchema, translationPreferenceSchema, translationPreviewSchema, translationProviderSettingsSchema, ttsProviderSettingsSchema } from "./schemas.js";
...
app.get("/api/v1/orders",{preHandler:authenticate},async(request,reply)=>{
  ...
  const result=await pool.query(`SELECT o.id,o.display_order_number,o.order_number,o.conversation_id,c.account_id,a.display_name account_name,co.display_name customer_name,co.phone_e164 customer_phone,o.amount,o.currency,o.description,o.status,o.send_format,o.translate_on_send,o.target_language,o.address_id,o.shipping_address_snapshot,o.created_at,u.display_name created_by_name,COALESCE(m.status::text,o.status) message_status,COUNT(*) OVER()::int total_count,COALESCE(item_list.items,'[]'::json) items,COALESCE(fee_list.fees,'[]'::json) fees
...
app.post("/api/v1/conversations/:conversationId/orders/:orderId/send",{preHandler:authenticate},async(request,reply)=>{
  ...
  if(shouldTranslate){if(!targetLanguage)return reply.code(400).send({error:"target_language_required",message:"请选择订单翻译的目标语言"});const setting=await activeTranslationSetting();if(!setting)return reply.code(409).send({error:"translation_not_configured",message:"AI 翻译服务尚未配置，订单未发送"});try{outgoingText=await translateText(setting,{text:englishSummary,targetLanguage});}catch(error){request.log.error({orderId,error:String(error)},"Order translation failed");return reply.code(502).send({error:"translation_failed",message:"订单翻译失败，订单尚未发送"});}}
  ...
  if(parsed.data.format==="image"){
    try{const products=await Promise.all(itemResult.rows.map(async item=>{if(!item.object_key)return{name:String(item.name)};const object=await s3.send(new GetObjectCommand({Bucket:config.S3_BUCKET,Key:item.object_key}));if(!object.Body)return{name:String(item.name)};return{name:String(item.name),image:Buffer.from(await object.Body.transformToByteArray())};}));
      const png=await renderOrderImage(outgoingText,products,feeResult.rows.length),sha256=createHash("sha256").update(png).digest("hex"),objectKey=`orders/${order.account_id}/${orderId}/${sha256}.png`,fileName=`order-${safeFileName(String(order.display_order_number))}.png`;
  ...
  const queued=await transaction(async client=>{const locked=await client.query("SELECT deleted_at FROM orders WHERE id=$1 FOR UPDATE",[orderId]);if(!locked.rowCount||locked.rows[0].deleted_at)return null;const existing=await client.query("SELECT id FROM messages WHERE account_id=$1 AND client_message_id=$2",[order.account_id,requestMessageId]);if(existing.rowCount)return{messageId:existing.rows[0].id,format:parsed.data.format,deduplicated:true};const kind=parsed.data.format==="image"?"image":"text",caption=parsed.data.format==="image"?`Order #${String(order.display_order_number)}`:outgoingText;const message=await client.query("INSERT INTO messages(conversation_id,account_id,sender_user_id,client_message_id,direction,kind,text_content,translation_source_text,media_id,status,occurred_at) VALUES($1,$2,$3,$4,'out',$5,$6,$7,$8,'queued',now()) RETURNING id,status",[conversationId,order.account_id,principal.id,requestMessageId,kind,caption,parsed.data.format==="text"&&shouldTranslate?englishSummary:null,renderedMediaId]);await queueOrderCommand(client,order,conversationId,message.rows[0].id,requestMessageId,kind,caption,renderedMediaId??undefined);
...
app.get("/api/v1/admin/agent-provider",{preHandler:authenticate},async(request,reply)=>{
  ...
  const defaults=[{provider:"openai",base_url:"https://api.openai.com/v1",model:"gpt-5.6-luna",embedding_model:"text-embedding-3-small"},{provider:"openai_compatible",base_url:"",model:"",embedding_model:""}];
...
app.put("/api/v1/admin/agent-provider/:provider",{preHandler:authenticate},async(request,reply)=>{
  if(request.principal?.role!=="admin")return reply.code(403).send({error:"admin_required"});const {provider}=request.params as {provider:string};if(!["openai","openai_compatible"].includes(provider))return reply.code(404).send({error:"not_found"});
