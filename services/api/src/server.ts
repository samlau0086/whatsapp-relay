...
app.post("/api/v1/conversations/:id/orders",{preHandler:authenticate},async(request,reply)=>{
  ...
  const result=await transaction(async client=>{
    ...
    const total=calculateOrderTotal(parsed.data.items,parsed.data.fees),number=await allocateOrderNumber(client),orderAddress=await resolveOrderAddress(client,conversation.rows[0].contact_id,principal.id,parsed.data.addressId,parsed.data.newAddress);const order=await client.query("INSERT INTO orders(client_order_id,conversation_id,created_by,amount,currency,weight_unit,description,status,translate_on_send,target_language,display_order_number,sequence_date,daily_sequence,address_id,shipping_address_snapshot) VALUES($1,$2,$3,$4,$5,$6,$7,'draft',$8,$9,$10,$11,$12,$13,$14) RETURNING id,display_order_number,status",[parsed.data.clientOrderId,id,principal.id,total,parsed.data.currency,parsed.data.weightUnit,parsed.data.description??null,parsed.data.translateOnSend,parsed.data.targetLanguage??null,number.displayOrderNumber,number.sequenceDate,number.dailySequence,orderAddress?.id??null,orderAddress?JSON.stringify(orderAddress.snapshot):null]);const orderRow=order.rows[0];
