...
app.get("/api/v1/conversations/:id/details",{preHandler:authenticate},async(request,reply)=>{
  ...
  const [tags,notes,reminder,orders,addresses]=await Promise.all([
    ...
    pool.query("SELECT o.id,o.display_order_number,o.order_number,o.amount,o.currency,o.description,o.status,o.send_format,o.translate_on_send,o.target_language,o.address_id,o.shipping_address_snapshot,o.created_at,u.display_name created_by_name,COALESCE(m.status::text,o.status) message_status,COALESCE(item_list.items,'[]'::json) items,COALESCE(fee_list.fees,'[]'::json) fees,payment.payment_request FROM orders o LEFT JOIN users u ON u.id=o.created_by LEFT JOIN messages m ON m.id=o.summary_message_id LEFT JOIN LATERAL (SELECT json_agg(json_build_object('id',i.id,'name',i.product_name,'quantity',i.quantity,'unitAmount',i.unit_amount,'imageMediaId',i.image_media_id,'imageName',media.file_name,'productId',i.product_id) ORDER BY i.position) items FROM order_items i LEFT JOIN media ON media.id=i.image_media_id WHERE i.order_id=o.id)item_list ON true LEFT JOIN LATERAL (SELECT json_agg(json_build_object('id',f.id,'name',f.name,'amount',f.amount) ORDER BY f.position) fees FROM order_fees f WHERE f.order_id=o.id)fee_list ON true LEFT JOIN LATERAL (SELECT json_build_object('id',pr.id,'invoiceId',pr.provider_request_id,'url',pr.payment_url,'status',pr.status,'amount',pr.amount,'currency',pr.currency,'environment',pr.environment,'createdAt',pr.created_at,'lastSyncedAt',pr.last_synced_at) payment_request FROM order_payment_requests pr WHERE pr.order_id=o.id AND pr.is_current ORDER BY pr.created_at DESC LIMIT 1)payment ON true WHERE o.conversation_id=$1 AND o.deleted_at IS NULL ORDER BY o.created_at DESC LIMIT 20",[id]),
...
app.post("/api/v1/admin/currencies/refresh-rates",{preHandler:authenticate},async(request,reply)=>{
  ...
  await transaction(async client=>{await client.query("UPDATE currency_settings SET is_base=false");await client.query("DELETE FROM currency_settings WHERE NOT(code=ANY($1::text[]))",[nextCodes]);for(const [position,item] of currencies.entries())await client.query("INSERT INTO currency_settings(code,name,rate,is_base,position,updated_by) VALUES($1,$2,$3,$4,$5,$6) ON CONFLICT(code) DO UPDATE SET name=EXCLUDED.name,rate=EXCLUDED.rate,is_base=EXCLUDED.is_base,position=EXCLUDED.position,updated_by=EXCLUDED.updated_by,updated_at=now()",[item.code,item.name,item.rate,item.code===parsed.data.baseCurrency,position,request.principal!.id]);await client.query("INSERT INTO audit_log(actor_type,actor_id,action,target_type,target_id,metadata) VALUES('user',$1,'currency.rates.refresh','currency_settings','workspace',$2)",[request.principal!.id,JSON.stringify({baseCurrency:parsed.data.baseCurrency,currencies:currencies.map(item=>item.code),source:"Frankfurter",rateDate:latest.date})]);});
  return{baseCurrency:parsed.data.baseCurrency,currencies,source:"Frankfurter",rateDate:latest.date,updatedCount:Math.max(0,currencies.length-1)};
});
...
app.put("/api/v1/admin/paypal-settings",{preHandler:authenticate},async(request,reply)=>{
  ...
  const current=await pool.query("SELECT client_id_encrypted,client_secret_encrypted FROM paypal_settings WHERE singleton=true"),row=current.rows[0]??{};
  const clientIdEncrypted=parsed.data.clientId?encryptAtRest(parsed.data.clientId,config.DATA_ENCRYPTION_KEY):row.client_id_encrypted??null,clientSecretEncrypted=parsed.data.clientSecret?encryptAtRest(parsed.data.clientSecret,config.DATA_ENCRYPTION_KEY):row.client_secret_encrypted??null;
  if(parsed.data.enabled&&(!clientIdEncrypted||!clientSecretEncrypted))return reply.code(400).send({error:"credentials_required",message:"启用 PayPal 收款前必须填写 Client ID 和 Client Secret"});
  if(parsed.data.enabled){try{clearPayPalTokenCache();const client=new PayPalClient({environment:parsed.data.environment,clientId:decryptAtRest(clientIdEncrypted,config.DATA_ENCRYPTION_KEY),clientSecret:decryptAtRest(clientSecretEncrypted,config.DATA_ENCRYPTION_KEY)});await client.verify();}catch(error){request.log.warn({error:error instanceof PayPalApiError?error.code:String(error)},"PayPal credential verification failed");return reply.code(400).send({error:"paypal_credentials_invalid",message:"PayPal 凭据验证失败，请检查环境、Client ID 和 Client Secret"});}}
  const saved=await pool.query("INSERT INTO paypal_settings(singleton,enabled,environment,client_id_encrypted,client_secret_encrypted,updated_by) VALUES(true,$1,$2,$3,$4,$5) ON CONFLICT(singleton) DO UPDATE SET enabled=EXCLUDED.enabled,environment=EXCLUDED.environment,client_id_encrypted=EXCLUDED.client_id_encrypted,client_secret_encrypted=EXCLUDED.client_secret_encrypted,updated_by=EXCLUDED.updated_by,updated_at=now() RETURNING enabled,environment,updated_at",[parsed.data.enabled,parsed.data.environment,clientIdEncrypted,clientSecretEncrypted,request.principal.id]);
  await auditCrm(request.principal.id,"paypal.settings.update","paypal_settings","workspace",{enabled:parsed.data.enabled,environment:parsed.data.environment,clientIdChanged:Boolean(parsed.data.clientId),clientSecretChanged:Boolean(parsed.data.clientSecret)});clearPayPalTokenCache();
  return{enabled:saved.rows[0].enabled,environment:saved.rows[0].environment,clientIdConfigured:Boolean(clientIdEncrypted),clientSecretConfigured:Boolean(clientSecretEncrypted),updatedAt:saved.rows[0].updated_at};
});

app.post("/api/v1/orders/:orderId/payment-request",{preHandler:authenticate},async(request,reply)=>{
  if(request.principal?.kind!=="user")return reply.code(403).send({error:"user_required"});const principal=request.principal,{orderId}=request.params as {orderId:string};
  ...
  const context=await pool.query("SELECT o.id,o.display_order_number,o.amount,o.currency,o.description,c.account_id FROM orders o JOIN conversations c ON c.id=o.conversation_id WHERE o.id=$1 AND o.deleted_at IS NULL",[orderId]);if(!context.rowCount||!canAccessAccount(principal,context.rows[0].account_id))return reply.code(404).send({error:"not_found"});const order=context.rows[0];
  ...
  const [items,fees]=await Promise.all([pool.query("SELECT product_name name,quantity,unit_amount FROM order_items WHERE order_id=$1 ORDER BY position",[orderId]),pool.query("SELECT name,amount FROM order_fees WHERE order_id=$1 ORDER BY position",[orderId])]);
  try{const client=new PayPalClient(setting),created=await client.createPayableInvoice({requestId:String(record.row.id),orderNumber:String(order.display_order_number),currency:String(order.currency),description:String(order.description??""),items:[...items.rows.map(item=>({name:String(item.name),quantity:Number(item.quantity),unitAmount:Number(item.unit_amount)})),...fees.rows.map(fee=>({name:String(fee.name),quantity:1,unitAmount:Number(fee.amount)}))]});const saved=await pool.query("UPDATE order_payment_requests SET provider_request_id=$2,payment_url=$3,status=$4,last_synced_at=now(),updated_at=now() WHERE id=$1 RETURNING *",[record.row.id,created.invoiceId,created.paymentUrl,created.status]);await auditCrm(principal.id,"payment_request.create","order",orderId,{paymentRequestId:record.row.id,paypalInvoiceId:created.invoiceId,environment:setting.environment,amount:Number(order.amount),currency:order.currency});return reply.code(201).send(paymentRequestResponse(saved.rows[0]));}
...
type PayPalSetting={environment:PayPalEnvironment;clientId:string;clientSecret:string};
async function activePayPalSetting(requiredEnvironment?:string,requireEnabled=true):Promise<PayPalSetting|null>{const result=await pool.query("SELECT enabled,environment,client_id_encrypted,client_secret_encrypted FROM paypal_settings WHERE singleton=true"),row=result.rows[0];if(!row||requireEnabled&&!row.enabled||!row.client_id_encrypted||!row.client_secret_encrypted||requiredEnvironment&&row.environment!==requiredEnvironment)return null;return{environment:row.environment as PayPalEnvironment,clientId:decryptAtRest(row.client_id_encrypted,config.DATA_ENCRYPTION_KEY),clientSecret:decryptAtRest(row.client_secret_encrypted,config.DATA_ENCRYPTION_KEY)};}
