...
app.get("/api/v1/openapi.json", async () => ({ openapi:"3.1.0", info:{title:"RelayDesk API",version:"0.1.0"}, paths:{
  ...
  "/api/v1/conversations/{id}/orders/{orderId}/send":{post:{summary:"翻译（可选）并发送订单草稿"}},
...
app.get("/api/v1/conversations/:id/details",{preHandler:authenticate},async(request,reply)=>{
  ...
  const [tags,notes,reminder,orders]=await Promise.all([
    ...
    pool.query("SELECT o.id,o.order_number,o.amount,o.currency,o.description,o.status,o.translate_on_send,o.target_language,o.created_at,u.display_name created_by_name,COALESCE(m.status,o.status) message_status,COALESCE(item_list.items,'[]'::json) items,COALESCE(fee_list.fees,'[]'::json) fees FROM orders o LEFT JOIN users u ON u.id=o.created_by LEFT JOIN messages m ON m.id=o.summary_message_id LEFT JOIN LATERAL (SELECT json_agg(json_build_object('id',i.id,'name',i.product_name,'quantity',i.quantity,'unitAmount',i.unit_amount,'imageMediaId',i.image_media_id,'imageName',media.file_name) ORDER BY i.position) items FROM order_items i LEFT JOIN media ON media.id=i.image_media_id WHERE i.order_id=o.id)item_list ON true LEFT JOIN LATERAL (SELECT json_agg(json_build_object('id',f.id,'name',f.name,'amount',f.amount) ORDER BY f.position) fees FROM order_fees f WHERE f.order_id=o.id)fee_list ON true WHERE o.conversation_id=$1 ORDER BY o.created_at DESC LIMIT 20",[id]),
  ]);
...
app.post("/api/v1/conversations/:conversationId/orders/:orderId/send",{preHandler:authenticate},async(request,reply)=>{
  if(request.principal?.kind!=="user")return reply.code(403).send({error:"user_required"});const principal=request.principal;const {conversationId,orderId}=request.params as {conversationId:string;orderId:string};
  const found=await pool.query("SELECT o.id,o.order_number,o.client_order_id,o.currency,o.description,o.status,o.translate_on_send,o.target_language,o.summary_message_id,c.account_id,a.agent_id,co.wa_jid,m.status message_status FROM orders o JOIN conversations c ON c.id=o.conversation_id JOIN whatsapp_accounts a ON a.id=c.account_id JOIN contacts co ON co.id=c.contact_id LEFT JOIN messages m ON m.id=o.summary_message_id WHERE o.id=$1 AND o.conversation_id=$2",[orderId,conversationId]);if(!found.rowCount||!canAccessAccount(principal,found.rows[0].account_id))return reply.code(404).send({error:"not_found"});const order=found.rows[0];if(order.status!=="draft")return reply.code(202).send({orderId,orderNumber:Number(order.order_number),messageId:order.summary_message_id,status:order.message_status??order.status,deduplicated:true});
  const [itemResult,feeResult]=await Promise.all([pool.query("SELECT id,product_name name,quantity,unit_amount,image_media_id FROM order_items WHERE order_id=$1 ORDER BY position",[orderId]),pool.query("SELECT name,amount FROM order_fees WHERE order_id=$1 ORDER BY position",[orderId])]);const items:OrderSummaryItem[]=itemResult.rows.map(item=>({name:String(item.name),quantity:Number(item.quantity),unitAmount:Number(item.unit_amount)})),fees:OrderSummaryFee[]=feeResult.rows.map(fee=>({name:String(fee.name),amount:Number(fee.amount)}));const englishSummary=formatOrderSummary(Number(order.order_number),items,fees,order.currency,order.description??undefined);let outgoingText=englishSummary;
  ...
  const queued=await transaction(async client=>{const locked=await client.query("SELECT status,summary_message_id FROM orders WHERE id=$1 FOR UPDATE",[orderId]);if(!locked.rowCount)return null;if(locked.rows[0].status!=="draft")return{messageId:locked.rows[0].summary_message_id,deduplicated:true};const summaryClientId=`${order.client_order_id}:summary`;const message=await client.query("INSERT INTO messages(conversation_id,account_id,sender_user_id,client_message_id,direction,kind,text_content,translation_source_text,status,occurred_at) VALUES($1,$2,$3,$4,'out','text',$5,$6,'queued',now()) RETURNING id,status",[conversationId,order.account_id,principal.id,summaryClientId,outgoingText,order.translate_on_send?englishSummary:null]);await queueOrderCommand(client,order,conversationId,message.rows[0].id,summaryClientId,"text",outgoingText);
    const imageItems=itemResult.rows.filter(item=>item.image_media_id);for(const [index,item] of imageItems.entries()){const clientMessageId=`${order.client_order_id}:item:${item.id}`;const caption=`Product image ${index+1}/${imageItems.length}: ${item.name}`;const attachment=await client.query("INSERT INTO messages(conversation_id,account_id,sender_user_id,client_message_id,direction,kind,text_content,media_id,status,occurred_at) VALUES($1,$2,$3,$4,'out','image',$5,$6,'queued',now()) RETURNING id",[conversationId,order.account_id,principal.id,clientMessageId,caption,item.image_media_id]);await queueOrderCommand(client,order,conversationId,attachment.rows[0].id,clientMessageId,"image",caption,item.image_media_id);}
    await client.query("UPDATE orders SET status='queued',summary_message_id=$2,translated_text=$3,sent_at=now() WHERE id=$1",[orderId,message.rows[0].id,order.translate_on_send?outgoingText:null]);await client.query("UPDATE conversations SET status='open',closed_at=NULL,last_message_at=now() WHERE id=$1",[conversationId]);await client.query("INSERT INTO audit_log(actor_type,actor_id,action,target_type,target_id,metadata) VALUES('user',$1,'order.send','order',$2,$3)",[principal.id,orderId,JSON.stringify({conversationId,translated:Boolean(order.translate_on_send),targetLanguage:order.target_language??null,imageCount:imageItems.length})]);return{messageId:message.rows[0].id,deduplicated:false};});
  if(!queued)return reply.code(404).send({error:"not_found"});if(order.agent_id)void dispatchPending(order.agent_id);return reply.code(202).send({orderId,orderNumber:Number(order.order_number),messageId:queued.messageId,status:"queued",deduplicated:queued.deduplicated});
});
