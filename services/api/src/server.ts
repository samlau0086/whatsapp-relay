...
app.patch("/api/v1/products/:id/stock",{preHandler:authenticate},async(request,reply)=>{
  ...
  const updated=await pool.query("UPDATE products SET is_in_stock=$2,updated_at=now() WHERE id=$1 AND deleted_at IS NULL RETURNING id,is_in_stock",[id,body.isInStock]);
  if(!updated.rowCount)return reply.code(404).send({error:"not_found"});
  return{ id:String(updated.rows[0].id), isInStock:Boolean(updated.rows[0].is_in_stock) };
});
