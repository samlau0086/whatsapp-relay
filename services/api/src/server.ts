...
import { conversationTagsSchema, currencySchema, customerStageSchema, enrollmentSchema, loginSchema, messageSchema, messageTranslationsSchema, newConversationSchema, noteSchema, orderSchema, orderSendSchema, orderSettingsSchema, orderUpdateSchema, productCreateSchema, productUpdateSchema, reminderSchema, tagCreateSchema, tagUpdateSchema, textToSpeechSchema, translationPreferenceQuerySchema, translationPreferenceSchema, translationPreviewSchema, translationProviderSettingsSchema, ttsProviderSettingsSchema } from "./schemas.js";
...
app.get("/api/v1/orders",{preHandler:authenticate},async(request,reply)=>{
  ...
  const result=await pool.query(`SELECT o.id,o.display_order_number,o.order_number,o.conversation_id,c.account_id,a.display_name account_name,co.display_name customer_name,co.phone_e164 customer_phone,o.amount,o.currency,o.description,o.status,o.send_format,o.translate_on_send,o.target_language,o.created_at,u.display_name created_by_name,COALESCE(m.status::text,o.status) message_status,COUNT(*) OVER()::int total_count,COALESCE(item_list.items,'[]'::json) items,COALESCE(fee_list.fees,'[]'::json) fees
    ...
    LEFT JOIN LATERAL (SELECT json_agg(json_build_object('id',i.id,'name',i.product_name,'quantity',i.quantity,'unitAmount',i.unit_amount,'imageMediaId',i.image_media_id,'imageName',media.file_name,'productId',i.product_id) ORDER BY i.position) items FROM order_items i LEFT JOIN media ON media.id=i.image_media_id WHERE i.order_id=o.id)item_list ON true
    ...
    WHERE o.deleted_at IS NULL AND ($1::uuid IS NULL OR c.account_id=$1) AND ($2::text IS NULL OR o.status=$2) AND ($3::text IS NULL OR o.display_order_number ILIKE '%'||$3||'%' OR co.display_name ILIKE '%'||$3||'%' OR co.phone_e164 ILIKE '%'||$3||'%') AND ($4::date IS NULL OR o.created_at >= $4::date) AND ($5::date IS NULL OR o.created_at < $5::date + interval '1 day') AND ($6::timestamptz IS NULL OR (o.created_at,o.id)<($6::timestamptz,$7::uuid)) AND ($8::uuid[] IS NULL OR c.account_id=ANY($8))
...
app.post("/api/v1/conversations/:id/orders",{preHandler:authenticate},async(request,reply)=>{
  ...
  const result=await transaction(async client=>{
    ...
    const total=calculateOrderTotal(parsed.data.items,parsed.data.fees),number=await allocateOrderNumber(client);const order=await client.query("INSERT INTO orders(client_order_id,conversation_id,created_by,amount,currency,description,status,translate_on_send,target_language,display_order_number,sequence_date,daily_sequence) VALUES($1,$2,$3,$4,$5,$6,'draft',$7,$8,$9,$10,$11) RETURNING id,display_order_number,status",[parsed.data.clientOrderId,id,principal.id,total,parsed.data.currency,parsed.data.description??null,parsed.data.translateOnSend,parsed.data.targetLanguage??null,number.displayOrderNumber,number.sequenceDate,number.dailySequence]);const orderRow=order.rows[0];
...
app.post("/api/v1/conversations/:conversationId/orders/:orderId/send",{preHandler:authenticate},async(request,reply)=>{
  ...
  if(parsed.data.format==="image"){
    try{const products=await Promise.all(itemResult.rows.map(async item=>{if(!item.object_key)return{name:String(item.name)};const object=await s3.send(new GetObjectCommand({Bucket:config.S3_BUCKET,Key:item.object_key}));if(!object.Body)return{name:String(item.name)};return{name:String(item.name),image:Buffer.from(await object.Body.transformToByteArray())};}));
      const png=await renderOrderImage(outgoingText,products),sha256=createHash("sha256").update(png).digest("hex"),objectKey=`orders/${order.account_id}/${orderId}/${sha256}.png`,fileName=`order-${safeFileName(String(order.display_order_number))}.png`;
  ...
  const queued=await transaction(async client=>{const locked=await client.query("SELECT deleted_at FROM orders WHERE id=$1 FOR UPDATE",[orderId]);if(!locked.rowCount||locked.rows[0].deleted_at)return null;const existing=await client.query("SELECT id FROM messages WHERE account_id=$1 AND client_message_id=$2",[order.account_id,requestMessageId]);if(existing.rowCount)return{messageId:existing.rows[0].id,format:parsed.data.format,deduplicated:true};const kind=parsed.data.format==="image"?"image":"text",caption=parsed.data.format==="image"?`Order #${String(order.display_order_number)}`:outgoingText;const message=await client.query("INSERT INTO messages(conversation_id,account_id,sender_user_id,client_message_id,direction,kind,text_content,translation_source_text,media_id,status,occurred_at) VALUES($1,$2,$3,$4,'out',$5,$6,$7,$8,'queued',now()) RETURNING id,status",[conversationId,order.account_id,principal.id,requestMessageId,kind,caption,parsed.data.format==="text"&&order.translate_on_send?englishSummary:null,renderedMediaId]);await queueOrderCommand(client,order,conversationId,message.rows[0].id,requestMessageId,kind,caption,renderedMediaId??undefined);
