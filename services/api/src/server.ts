...
app.get("/api/v1/conversations", { preHandler:authenticate }, async (request,reply) => {
  const query = request.query as { accountId?:string; status?:string; q?:string; limit?:string; before?:string; lastMessageFrom?:string; lastMessageBefore?:string };
