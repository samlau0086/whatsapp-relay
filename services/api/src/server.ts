...
app.get("/api/v1/products",{preHandler:authenticate},async(request,reply)=>{
  ...
  const [result,tagOptions,categoryOptions,brandOptions]=await Promise.all([pool.query(`SELECT p.id,p.sku,p.name,p.description,p.category,p.brand,p.default_unit_amount,p.weight_amount,p.weight_unit,p.shipping_class_id,sc.name shipping_class_name,p.currency,p.image_media_id,m.file_name image_name,p.created_at,p.updated_at,COUNT(*) OVER()::int total_count,COALESCE(label_list.tags,'[]'::json) tags,COALESCE(price_list.price_tiers,'[]'::json) price_tiers,COALESCE(variant_list.variants,'[]'::json) variants
    ...
    LEFT JOIN LATERAL (SELECT json_agg(json_build_object('id',v.id,'attributes',v.attributes,'sku',v.sku,'imageMediaId',v.image_media_id,'priceTiers',COALESCE((SELECT json_agg(json_build_object('minQuantity',t.min_quantity,'unitAmount',t.unit_amount) ORDER BY t.min_quantity) FROM product_variant_price_tiers t WHERE t.variant_id=v.id),'[]'::json)) ORDER BY v.created_at,v.id) variants FROM product_variants v WHERE v.product_id=p.id) variant_list ON true
...
app.post("/api/v1/products/selection",{preHandler:authenticate},async(request,reply)=>{
  ...
  const result=await pool.query(`SELECT p.id,p.sku,p.name,p.description,p.category,p.brand,p.default_unit_amount,p.currency,p.image_media_id,m.file_name image_name,p.created_at,p.updated_at,COALESCE(label_list.tags,'[]'::json) tags,COALESCE(price_list.price_tiers,'[]'::json) price_tiers,COALESCE(variant_list.variants,'[]'::json) variants FROM products p LEFT JOIN media m ON m.id=p.image_media_id LEFT JOIN LATERAL (SELECT json_agg(json_build_object('id',label.id,'name',label.name,'color',label.color) ORDER BY lower(label.name)) tags FROM product_labels label WHERE label.product_id=p.id) label_list ON true LEFT JOIN LATERAL (SELECT json_agg(json_build_object('minQuantity',tier.min_quantity,'unitAmount',tier.unit_amount) ORDER BY tier.min_quantity) price_tiers FROM product_price_tiers tier WHERE tier.product_id=p.id) price_list ON true LEFT JOIN LATERAL (SELECT json_agg(json_build_object('id',v.id,'attributes',v.attributes,'sku',v.sku,'imageMediaId',v.image_media_id,'priceTiers',COALESCE((SELECT json_agg(json_build_object('minQuantity',t.min_quantity,'unitAmount',t.unit_amount) ORDER BY t.min_quantity) FROM product_variant_price_tiers t WHERE t.variant_id=v.id),'[]'::json)) ORDER BY v.created_at,v.id) variants FROM product_variants v WHERE v.product_id=p.id) variant_list ON true WHERE p.deleted_at IS NULL AND p.id=ANY($1::uuid[]) ORDER BY array_position($1::uuid[],p.id)`,[ids]);
...
function mapProductRow(row:Record<string,unknown>){const tiers=Array.isArray(row.price_tiers)?(row.price_tiers as Array<Record<string,unknown>>).map(tier=>({minQuantity:Number(tier.minQuantity),unitAmount:Number(tier.unitAmount)})):[],variants=Array.isArray(row.variants)?(row.variants as Array<Record<string,unknown>>).map(variant=>({...variant,id:String(variant.id),sku:String(variant.sku),attributes:variant.attributes&&typeof variant.attributes==="object"?variant.attributes:{},imageMediaId:variant.imageMediaId?String(variant.imageMediaId):null,priceTiers:Array.isArray(variant.priceTiers)?(variant.priceTiers as Array<Record<string,unknown>>).map(tier=>({minQuantity:Number(tier.minQuantity),unitAmount:Number(tier.unitAmount)})):[]})):[];return{id:String(row.id),sku:String(row.sku),name:String(row.name),description:String(row.description??""),category:String(row.category??""),brand:String(row.brand??""),defaultUnitAmount:tiers[0]?.unitAmount??Number(row.default_unit_amount),priceTiers:tiers,currency:String(row.currency),weightAmount:row.weight_amount===null||row.weight_amount===undefined?null:Number(row.weight_amount),weightUnit:row.weight_unit?String(row.weight_unit):null,shippingClassId:row.shipping_class_id?String(row.shipping_class_id):null,shippingClass:row.shipping_class_name?String(row.shipping_class_name):null,imageMediaId:row.image_media_id?String(row.image_media_id):null,imageName:String(row.image_name??""),tags:Array.isArray(row.tags)?row.tags:[],variants,createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}

async function productById(client:PoolClient,id:string){const result=await client.query(`SELECT p.id,p.sku,p.name,p.description,p.category,p.brand,p.default_unit_amount,p.weight_amount,p.weight_unit,p.shipping_class_id,sc.name shipping_class_name,p.currency,p.image_media_id,m.file_name image_name,p.created_at,p.updated_at,COALESCE(label_list.tags,'[]'::json) tags,COALESCE(price_list.price_tiers,'[]'::json) price_tiers,COALESCE(variant_list.variants,'[]'::json) variants FROM products p LEFT JOIN media m ON m.id=p.image_media_id LEFT JOIN shipping_classes sc ON sc.id=p.shipping_class_id LEFT JOIN LATERAL (SELECT json_agg(json_build_object('id',label.id,'name',label.name,'color',label.color) ORDER BY lower(label.name)) tags FROM product_labels label WHERE label.product_id=p.id) label_list ON true LEFT JOIN LATERAL (SELECT json_agg(json_build_object('minQuantity',tier.min_quantity,'unitAmount',tier.unit_amount) ORDER BY tier.min_quantity) price_tiers FROM product_price_tiers tier WHERE tier.product_id=p.id) price_list ON true LEFT JOIN LATERAL (SELECT json_agg(json_build_object('id',v.id,'attributes',v.attributes,'sku',v.sku,'imageMediaId',v.image_media_id,'priceTiers',COALESCE((SELECT json_agg(json_build_object('minQuantity',t.min_quantity,'unitAmount',t.unit_amount) ORDER BY t.min_quantity) FROM product_variant_price_tiers t WHERE t.variant_id=v.id),'[]'::json)) ORDER BY v.created_at,v.id) variants FROM product_variants v WHERE v.product_id=p.id) variant_list ON true WHERE p.id=$1`,[id]);return result.rowCount?mapProductRow(result.rows[0]):null;}
