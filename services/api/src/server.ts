...
app.post("/api/v1/conversations/:id/product-cards/send",{preHandler:authenticate},async(request,reply)=>{
  ...
  const gridPageCount=parsed.data.mode==="grid"?Math.ceil(parsed.data.productIds.length/(parsed.data.grid!.rows*parsed.data.grid!.columns)):0;
  const clientMessageIds=parsed.data.mode==="individual"?parsed.data.productIds.map((_,index)=>`${parsed.data.clientBatchId}:p:${index}`):parsed.data.mode==="grid"?Array.from({length:gridPageCount},(_,index)=>`${parsed.data.clientBatchId}:grid:${index}`):[`${parsed.data.clientBatchId}:${parsed.data.mode}`];
  ...
  try{for(const [index,bytes] of rendered.entries()){const sha256=createHash("sha256").update(bytes).digest("hex"),fileName=parsed.data.mode==="grid"?`product-grid-${parsed.data.clientBatchId}-${index+1}-of-${rendered.length}.png`:parsed.data.mode==="combined"?`product-cards-${parsed.data.clientBatchId}.png`:`product-${products[index].sku}.png`,objectKey=`product-cards/${parsed.data.accountId}/${new Date().toISOString().slice(0,10)}/${randomBytes(16).toString("hex")}.png`;await s3.send(new PutObjectCommand({Bucket:config.S3_BUCKET,Key:objectKey,Body:bytes,ContentType:"image/png",Metadata:{sha256,source:"product-card"}}));uploaded.push({objectKey,sha256,bytes,fileName});}
    const result=await transaction(async client=>{const duplicate=await client.query("SELECT id FROM messages WHERE account_id=$1 AND client_message_id=ANY($2::text[])",[parsed.data.accountId,clientMessageIds]);if(duplicate.rowCount)throw Object.assign(new Error("product_card_batch_conflict"),{statusCode:409});const messageIds:string[]=[];const baseTime=Date.now();for(const [index,item] of uploaded.entries()){const media=await client.query("INSERT INTO media(account_id,object_key,file_name,mime_type,byte_size,sha256) VALUES($1,$2,$3,'image/png',$4,$5) RETURNING id",[parsed.data.accountId,item.objectKey,item.fileName,item.bytes.length,item.sha256]),clientMessageId=clientMessageIds[index],messageCaption=index===0?(caption||null):null,messageTranslationSource=index===0?translationSourceText:null,occurredAt=new Date(baseTime+index).toISOString();const message=await client.query("INSERT INTO messages(conversation_id,account_id,sender_user_id,client_message_id,direction,kind,text_content,translation_source_text,translation_target_language,media_id,status,occurred_at) VALUES($1,$2,$3,$4,'out','image',$5,$6,$7,$8,'queued',$9) RETURNING id",[id,parsed.data.accountId,principal.id,clientMessageId,messageCaption,messageTranslationSource,messageTranslationSource?parsed.data.translationTargetLanguage??null:null,media.rows[0].id,occurredAt]);await queueChannelCommand(client,{accountId:parsed.data.accountId,conversationId:id,messageId:message.rows[0].id,payload:{accountId:parsed.data.accountId,conversationId:id,clientMessageId,type:"image",...(messageCaption?{text:messageCaption}:{}),mediaId:media.rows[0].id,messageId:message.rows[0].id,toJid:context.provider_user_id}});messageIds.push(String(message.rows[0].id));}await client.query("UPDATE conversations SET status='open',closed_at=NULL WHERE id=$1",[id]);await pauseAgentForHuman(client,id);await client.query("INSERT INTO audit_log(actor_type,actor_id,action,target_type,target_id,metadata) VALUES('user',$1,'product_card.send','conversation',$2,$3)",[principal.id,id,JSON.stringify({clientBatchId:parsed.data.clientBatchId,productIds:parsed.data.productIds,mode:parsed.data.mode,showPrice:parsed.data.showPrice,captioned:Boolean(caption),translatedCaption:Boolean(translationSourceText),translatedProductNames:translatedNames.size,messageIds})]);return{deduplicated:false,messageIds};});if(context.agent_id)void dispatchPending(context.agent_id);return reply.code(202).send(result);
