...
app.get("/api/v1/orders",{preHandler:authenticate},async(request,reply)=>{
  ...
  const result=await pool.query(`SELECT o.id,o.display_order_number,o.order_number,o.conversation_id,c.account_id,a.display_name account_name,COALESCE(NULLIF(co.alias,''),co.display_name,co.phone_e164,co.provider_user_id) customer_name,co.phone_e164 customer_phone,o.amount,o.currency,o.weight_unit,o.description,o.status,o.business_status,o.send_format,o.translate_on_send,o.target_language,o.address_id,o.shipping_address_snapshot,o.payment_profile_id,o.payment_profile_snapshot,o.shipping_amount,o.shipping_template_id,o.shipping_quote_snapshot,o.created_at,u.display_name created_by_name,COALESCE(m.status::text,o.status) message_status,COUNT(*) OVER()::int total_count,COALESCE(item_list.items,'[]'::json) items,COALESCE(fee_list.fees,'[]'::json) fees,payment.payment_request
    ...
    LEFT JOIN LATERAL (SELECT json_agg(json_build_object('id',i.id,'name',i.product_name,'sku',i.product_sku,'quantity',i.quantity,'unitAmount',i.unit_amount,'weightAmount',i.weight_amount,'weightUnit',i.weight_unit,'shippingClassId',i.shipping_class_id,'shippingClassName',i.shipping_class_name,'imageMediaId',i.image_media_id,'imageName',media.file_name,'productId',i.product_id,'internalNote',i.internal_note_snapshot) ORDER BY i.position) items FROM order_items i LEFT JOIN media ON media.id=i.image_media_id WHERE i.order_id=o.id)item_list ON true
...
app.post("/api/v1/conversations/:id/product-cards/send",{preHandler:authenticate},async(request,reply)=>{
  ...
  const productsResult=await pool.query(`SELECT p.id,p.name,p.sku,p.currency,m.object_key,COALESCE(price_list.price_tiers,'[]'::json) price_tiers,COALESCE(label_list.tags,'[]'::json) tags,COALESCE(variant_list.variants,'[]'::json) variants FROM products p LEFT JOIN media m ON m.id=p.image_media_id AND m.status='ready' LEFT JOIN LATERAL (SELECT json_agg(json_build_object('minQuantity',t.min_quantity,'unitAmount',t.unit_amount) ORDER BY t.min_quantity) price_tiers FROM product_price_tiers t WHERE t.product_id=p.id) price_list ON true LEFT JOIN LATERAL (SELECT json_agg(json_build_object('attributes',v.attributes,'sku',v.sku,'priceTiers',COALESCE((SELECT json_agg(json_build_object('minQuantity',t.min_quantity,'unitAmount',t.unit_amount) ORDER BY t.min_quantity) FROM product_variant_price_tiers t WHERE t.variant_id=v.id),'[]'::json)) ORDER BY v.created_at,v.id) variants FROM product_variants v WHERE v.product_id=p.id) variant_list ON true LEFT JOIN LATERAL (SELECT json_agg(json_build_object('name',l.name) ORDER BY lower(l.name)) tags FROM product_labels l WHERE l.product_id=p.id) label_list ON true WHERE p.deleted_at IS NULL AND p.id=ANY($1::uuid[]) ORDER BY array_position($1::uuid[],p.id)`,[parsed.data.productIds]);if(productsResult.rowCount!==parsed.data.productIds.length)return reply.code(409).send({error:"product_unavailable"});
  const translatedNames=new Map((parsed.data.translatedProductNames??[]).map(item=>[item.productId,item.name]));
  const products:ProductCardRenderProduct[]=await Promise.all(productsResult.rows.map(async row=>{let image:Buffer|undefined;if(row.object_key){const object=await s3.send(new GetObjectCommand({Bucket:config.S3_BUCKET,Key:row.object_key}));if(object.Body)image=Buffer.from(await object.Body.transformToByteArray());}return{name:translatedNames.get(String(row.id))??String(row.name),sku:String(row.sku),currency:String(row.currency),priceTiers:(row.price_tiers as Array<Record<string,unknown>>).map(tier=>({minQuantity:Number(tier.minQuantity),unitAmount:Number(tier.unitAmount)})),variants:(row.variants as Array<Record<string,unknown>>).map(variant=>({attributes:variant.attributes&&typeof variant.attributes==="object"?variant.attributes as Record<string,string>:{},sku:String(variant.sku),priceTiers:Array.isArray(variant.priceTiers)?(variant.priceTiers as Array<Record<string,unknown>>).map(tier=>({minQuantity:Number(tier.minQuantity),unitAmount:Number(tier.unitAmount)})):[]})),tags:(row.tags as Array<Record<string,unknown>>).map(tag=>({name:String(tag.name)})),image};}));
