...
app.patch("/api/v1/contacts/:id",{preHandler:authenticate},async(request,reply)=>{
  ...
  const updated=await transaction(async client=>{
    ...
    await client.query("UPDATE contacts SET alias=$2,note=$3,phone_e164=CASE WHEN $4::text IS NULL THEN phone_e164 ELSE $4 END,provider_user_id=CASE WHEN $5::text IS NULL THEN provider_user_id ELSE $5 END,whatsapp_username=CASE WHEN $6::boolean THEN NULLIF($7,'') ELSE whatsapp_username END,birthday_month=CASE WHEN $8 THEN $9 ELSE birthday_month END,birthday_day=CASE WHEN $8 THEN $10 ELSE birthday_day END,birthday_year=CASE WHEN $8 THEN $11 ELSE birthday_year END,timezone=CASE WHEN $12 THEN $13 ELSE timezone END,preferred_language=CASE WHEN $14 THEN $15 ELSE preferred_language END,first_name=CASE WHEN $16 THEN $17 ELSE first_name END,middle_name=CASE WHEN $18 THEN $19 ELSE middle_name END,last_name=CASE WHEN $20 THEN $21 ELSE last_name END,company_name=CASE WHEN $22 THEN $23 ELSE company_name END,job_title=CASE WHEN $24 THEN $25 ELSE job_title END,country=CASE WHEN $26 THEN $27 ELSE country END,province=CASE WHEN $28 THEN $29 ELSE province END,city=CASE WHEN $30 THEN $31 ELSE city END,updated_at=now() WHERE id=$1",[id,parsed.data.alias||null,parsed.data.note||null,nextPhone,parsed.data.phone?`${parsed.data.phone}@s.whatsapp.net`:null,parsed.data.whatsappUsername!==undefined,parsed.data.whatsappUsername??null,parsed.data.birthday!==undefined,parsed.data.birthday?.month??null,parsed.data.birthday?.day??null,parsed.data.birthday?.year??null,parsed.data.timezone!==undefined,parsed.data.timezone||null,parsed.data.preferredLanguage!==undefined,parsed.data.preferredLanguage||null,parsed.data.firstName!==undefined,parsed.data.firstName||null,parsed.data.middleName!==undefined,parsed.data.middleName||null,parsed.data.lastName!==undefined,parsed.data.lastName||null,parsed.data.companyName!==undefined,parsed.data.companyName||null,parsed.data.jobTitle!==undefined,parsed.data.jobTitle||null,parsed.data.country!==undefined,parsed.data.country||null,parsed.data.province!==undefined,parsed.data.province||null,parsed.data.city!==undefined,parsed.data.city||null]);
