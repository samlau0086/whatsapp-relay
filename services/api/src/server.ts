...
app.get("/api/v1/products",{preHandler:authenticate},async(request,reply)=>{
  ...
  const [result,tagOptions,categoryOptions,brandOptions]=await Promise.all([pool.query(`SELECT p.id,p.sku,p.name,p.description,p.category,p.brand,p.supplier_links,p.internal_note,p.default_unit_amount,p.weight_amount,p.weight_unit,p.shipping_class_id,sc.name shipping_class_name,p.currency,p.image_media_id,p.image_url,m.file_name image_name,COALESCE(gallery_list.images,'[]'::json) gallery_images,p.created_at,p.updated_at,COUNT(*) OVER()::int total_count,COALESCE(label_list.tags,'[]'::json) tags,COALESCE(price_list.price_tiers,'[]'::json) price_tiers,COALESCE(variant_list.variants,'[]'::json) variants
...
app.post("/api/v1/products/query",{preHandler:authenticate},async(request,reply)=>{
  ...
  const normalizedSkus=parsed.data.skus.map(sku=>sku.toLocaleLowerCase()),result=await pool.query(`SELECT p.id,p.sku,p.name,p.description,p.category,p.brand,p.supplier_links,p.internal_note,p.default_unit_amount,p.weight_amount,p.weight_unit,p.shipping_class_id,sc.name shipping_class_name,p.currency,p.image_media_id,p.image_url,m.file_name image_name,p.created_at,p.updated_at,COALESCE(label_list.tags,'[]'::json) tags,COALESCE(price_list.price_tiers,'[]'::json) price_tiers,COALESCE(variant_list.variants,'[]'::json) variants
