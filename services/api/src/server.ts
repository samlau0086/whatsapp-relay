...
app.post("/api/v1/conversations/:id/orders",{preHandler:authenticate},async(request,reply)=>{
  ...
  const result=await transaction(async client=>{
    ...
    const conversationTags=await client.query("SELECT t.name,t.color FROM conversation_tags ct JOIN tags t ON t.id=ct.tag_id WHERE ct.conversation_id=$1 ORDER BY lower(t.name)",[id]);for(const [position,item] of parsed.data.items.entries()){const productId=await resolveOrderProduct(client,item,String(orderRow.display_order_number),parsed.data.currency,principal.id,conversationTags.rows),shippingClass=await orderShippingClassSnapshot(client,item,productId);await client.query("INSERT INTO order_items(order_id,position,product_name,product_sku,quantity,unit_amount,weight_amount,weight_unit,image_media_id,image_url,product_id,variant_id,shipping_class_id,shipping_class_name,internal_note_snapshot,is_out_of_stock) VALUES($1,$2,$3,COALESCE($4::text,(SELECT sku FROM products WHERE id=$11::uuid)),$5,$6,$7,$8,$9::uuid,CASE WHEN $9::uuid IS NULL THEN COALESCE($10::text,(SELECT image_url FROM products WHERE id=$11::uuid)) ELSE NULL END,$11::uuid,$12::uuid,$13::uuid,$14::text,COALESCE((SELECT internal_note FROM products WHERE id=$11::uuid),''),CASE WHEN $11::uuid IS NULL THEN false ELSE COALESCE((SELECT NOT is_in_stock FROM products WHERE id=$11::uuid),false) END)",[orderRow.id,position,item.name,item.sku??null,item.quantity,item.unitAmount,item.weightAmount??null,item.weightUnit??null,item.imageMediaId??null,item.imageUrl??null,productId,item.variantId??null,shippingClass.shippingClassId,shippingClass.shippingClassName]);}
