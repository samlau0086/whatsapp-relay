...
app.patch("/api/v1/contacts/:id",{preHandler:authenticate},async(request,reply)=>{
  ...
  const principal=request.principal,{id}=request.params as {id:string};
  const updated=await transaction(async client=>{
    const current=await client.query("SELECT co.account_id,co.phone_e164,co.provider_user_id,EXISTS(SELECT 1 FROM conversations c WHERE c.contact_id=co.id) has_conversation FROM contacts co WHERE co.id=$1 AND co.entity_type='person' FOR UPDATE",[id]);
