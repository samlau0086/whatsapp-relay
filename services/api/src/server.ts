...
import { conversationTagsSchema, currencySchema, customerStageSchema, enrollmentSchema, loginSchema, messageSchema, messageTranslationsSchema, newConversationSchema, noteSchema, orderSchema, orderSendSchema, orderUpdateSchema, productCreateSchema, productUpdateSchema, reminderSchema, tagCreateSchema, tagUpdateSchema, textToSpeechSchema, translationPreferenceQuerySchema, translationPreferenceSchema, translationPreviewSchema, translationProviderSettingsSchema, ttsProviderSettingsSchema } from "./schemas.js";
...
import { calculateOrderTotal, canManageSharedRecord, ensureCrmTables, formatOrderSummary, type OrderSummaryFee, type OrderSummaryItem } from "./crm.js";
import { renderOrderImage } from "./order-image.js";
...
app.get("/api/v1/products",{preHandler:authenticate},async(request,reply)=>{
  ...
  const result=await pool.query(`SELECT p.id,p.name,p.default_unit_amount,p.currency,p.image_media_id,m.file_name image_name,p.created_at,p.updated_at,COALESCE(label_list.tags,'[]'::json) tags
    ...
    WHERE p.deleted_at IS NULL AND ($1::text IS NULL OR p.name ILIKE '%'||$1||'%') AND ($2::text IS NULL OR p.currency=$2) AND ($3::text IS NULL OR EXISTS(SELECT 1 FROM product_labels filter_label WHERE filter_label.product_id=p.id AND lower(filter_label.name)=lower($3)))
...
app.patch("/api/v1/products/:id",{preHandler:authenticate},async(request,reply)=>{
  ...
  const result=await transaction(async client=>{const found=await client.query("SELECT id FROM products WHERE id=$1 AND deleted_at IS NULL FOR UPDATE",[id]);if(!found.rowCount)return undefined;if(parsed.data.imageMediaId){const image=await client.query("SELECT id FROM media WHERE id=$1 AND account_id IS NULL AND status='ready' AND mime_type IN ('image/png','image/jpeg')",[parsed.data.imageMediaId]);if(!image.rowCount)return null;}const hasImage=Object.prototype.hasOwnProperty.call(parsed.data,"imageMediaId");await client.query("UPDATE products SET name=CASE WHEN $2 THEN $3 ELSE name END,default_unit_amount=CASE WHEN $4 THEN $5 ELSE default_unit_amount END,currency=CASE WHEN $6 THEN $7 ELSE currency END,image_media_id=CASE WHEN $8 THEN $9 ELSE image_media_id END,updated_at=now() WHERE id=$1",[id,parsed.data.name!==undefined,parsed.data.name??null,parsed.data.defaultUnitAmount!==undefined,parsed.data.defaultUnitAmount??null,parsed.data.currency!==undefined,parsed.data.currency??null,hasImage,parsed.data.imageMediaId??null]);if(parsed.data.tags)await replaceProductLabels(client,id,parsed.data.tags);await client.query("INSERT INTO audit_log(actor_type,actor_id,action,target_type,target_id,metadata) VALUES('user',$1,'product.update','product',$2,$3)",[principal.id,id,JSON.stringify({fields:Object.keys(parsed.data)})]);return await productById(client,id);});
  if(result===undefined)return reply.code(404).send({error:"not_found"});if(result===null)return reply.code(400).send({error:"invalid_product_image"});return result;
});
...
app.get("/api/v1/conversations/:id/details",{preHandler:authenticate},async(request,reply)=>{
  ...
  const [tags,notes,reminder,orders]=await Promise.all([
    ...
    pool.query("SELECT o.id,o.order_number,o.amount,o.currency,o.description,o.status,o.send_format,o.translate_on_send,o.target_language,o.created_at,u.display_name created_by_name,COALESCE(m.status::text,o.status) message_status,COALESCE(item_list.items,'[]'::json) items,COALESCE(fee_list.fees,'[]'::json) fees FROM orders o LEFT JOIN users u ON u.id=o.created_by LEFT JOIN messages m ON m.id=o.summary_message_id LEFT JOIN LATERAL (SELECT json_agg(json_build_object('id',i.id,'name',i.product_name,'quantity',i.quantity,'unitAmount',i.unit_amount,'imageMediaId',i.image_media_id,'imageName',media.file_name,'productId',i.product_id) ORDER BY i.position) items FROM order_items i LEFT JOIN media ON media.id=i.image_media_id WHERE i.order_id=o.id)item_list ON true LEFT JOIN LATERAL (SELECT json_agg(json_build_object('id',f.id,'name',f.name,'amount',f.amount) ORDER BY f.position) fees FROM order_fees f WHERE f.order_id=o.id)fee_list ON true WHERE o.conversation_id=$1 AND o.deleted_at IS NULL ORDER BY o.created_at DESC LIMIT 20",[id]),
  ]);
...
app.post("/api/v1/conversations/:id/orders",{preHandler:authenticate},async(request,reply)=>{
  ...
  const result=await transaction(async client=>{
    const conversation=await client.query("SELECT c.account_id FROM conversations c WHERE c.id=$1",[id]);if(!conversation.rowCount||!canAccessAccount(principal,conversation.rows[0].account_id))return null;
    const duplicate=await client.query("SELECT id,order_number,conversation_id,status FROM orders WHERE client_order_id=$1",[parsed.data.clientOrderId]);if(duplicate.rowCount){if(duplicate.rows[0].conversation_id!==id)throw Object.assign(new Error("client_order_id_conflict"),{statusCode:409});return{...duplicate.rows[0],deduplicated:true};}
    ...
    const total=calculateOrderTotal(parsed.data.items,parsed.data.fees);const order=await client.query("INSERT INTO orders(client_order_id,conversation_id,created_by,amount,currency,description,status,translate_on_send,target_language) VALUES($1,$2,$3,$4,$5,$6,'draft',$7,$8) RETURNING id,order_number,status",[parsed.data.clientOrderId,id,principal.id,total,parsed.data.currency,parsed.data.description??null,parsed.data.translateOnSend,parsed.data.targetLanguage??null]);const orderRow=order.rows[0];
    const conversationTags=await client.query("SELECT t.name,t.color FROM conversation_tags ct JOIN tags t ON t.id=ct.tag_id WHERE ct.conversation_id=$1 ORDER BY lower(t.name)",[id]);for(const [position,item] of parsed.data.items.entries()){const productId=await resolveOrderProduct(client,item,Number(orderRow.order_number),parsed.data.currency,principal.id,conversationTags.rows);await client.query("INSERT INTO order_items(order_id,position,product_name,quantity,unit_amount,image_media_id,product_id) VALUES($1,$2,$3,$4,$5,$6,$7)",[orderRow.id,position,item.name,item.quantity,item.unitAmount,item.imageMediaId??null,productId]);}
    ...
    await client.query("INSERT INTO audit_log(actor_type,actor_id,action,target_type,target_id,metadata) VALUES('user',$1,'order.draft','order',$2,$3)",[principal.id,orderRow.id,JSON.stringify({conversationId:id,itemCount:parsed.data.items.length,feeCount:parsed.data.fees.length,translateOnSend:parsed.data.translateOnSend})]);return{id:orderRow.id,order_number:orderRow.order_number,status:orderRow.status,deduplicated:false};
  });
  if(!result)return reply.code(404).send({error:"not_found"});return reply.code(201).send({orderId:result.id,orderNumber:Number(result.order_number),status:result.status,deduplicated:result.deduplicated});
});
...
app.patch("/api/v1/conversations/:conversationId/orders/:orderId",{preHandler:authenticate},async(request,reply)=>{
  ...
  const result=await transaction(async client=>{const found=await client.query("SELECT o.order_number,o.status,c.account_id FROM orders o JOIN conversations c ON c.id=o.conversation_id WHERE o.id=$1 AND o.conversation_id=$2 AND o.deleted_at IS NULL FOR UPDATE OF o",[orderId,conversationId]);if(!found.rowCount||!canAccessAccount(principal,found.rows[0].account_id))return null;const clientProductIds=parsed.data.items.flatMap(item=>item.clientProductId?[item.clientProductId]:[]);if(new Set(clientProductIds).size!==clientProductIds.length)throw Object.assign(new Error("duplicate_client_product_id"),{statusCode:400});const mediaIds=parsed.data.items.flatMap(item=>item.imageMediaId?[item.imageMediaId]:[]);if(new Set(mediaIds).size!==mediaIds.length)throw Object.assign(new Error("duplicate_product_image"),{statusCode:400});if(mediaIds.length){const media=await client.query("SELECT id FROM media WHERE id=ANY($1::uuid[]) AND (account_id=$2 OR account_id IS NULL) AND status='ready' AND mime_type IN ('image/png','image/jpeg')",[mediaIds,found.rows[0].account_id]);if(media.rowCount!==mediaIds.length)throw Object.assign(new Error("invalid_product_image"),{statusCode:400});}const total=calculateOrderTotal(parsed.data.items,parsed.data.fees);await client.query("UPDATE orders SET amount=$2,currency=$3,description=$4,translate_on_send=$5,target_language=$6 WHERE id=$1",[orderId,total,parsed.data.currency,parsed.data.description??null,parsed.data.translateOnSend,parsed.data.targetLanguage??null]);const conversationTags=await client.query("SELECT t.name,t.color FROM conversation_tags ct JOIN tags t ON t.id=ct.tag_id WHERE ct.conversation_id=$1 ORDER BY lower(t.name)",[conversationId]);const productIds=[];for(const item of parsed.data.items)productIds.push(await resolveOrderProduct(client,item,Number(found.rows[0].order_number),parsed.data.currency,principal.id,conversationTags.rows));await client.query("DELETE FROM order_items WHERE order_id=$1",[orderId]);await client.query("DELETE FROM order_fees WHERE order_id=$1",[orderId]);for(const [position,item] of parsed.data.items.entries())await client.query("INSERT INTO order_items(order_id,position,product_name,quantity,unit_amount,image_media_id,product_id) VALUES($1,$2,$3,$4,$5,$6,$7)",[orderId,position,item.name,item.quantity,item.unitAmount,item.imageMediaId??null,productIds[position]]);for(const [position,fee] of parsed.data.fees.entries())await client.query("INSERT INTO order_fees(order_id,position,name,amount) VALUES($1,$2,$3,$4)",[orderId,position,fee.name,fee.amount]);await client.query("INSERT INTO audit_log(actor_type,actor_id,action,target_type,target_id,metadata) VALUES('user',$1,'order.update','order',$2,$3)",[principal.id,orderId,JSON.stringify({conversationId,itemCount:parsed.data.items.length,feeCount:parsed.data.fees.length,translateOnSend:parsed.data.translateOnSend,previouslySent:found.rows[0].status!=="draft"})]);return{orderNumber:Number(found.rows[0].order_number)};});if(!result)return reply.code(404).send({error:"not_found"});return{orderId,orderNumber:result.orderNumber,status:"updated"};
});
...
app.post("/api/v1/conversations/:conversationId/orders/:orderId/send",{preHandler:authenticate},async(request,reply)=>{
  ...
  const found=await pool.query("SELECT o.id,o.order_number,o.client_order_id,o.currency,o.description,o.status,o.send_format,o.translate_on_send,o.target_language,o.summary_message_id,c.account_id,a.agent_id,co.wa_jid,m.status message_status FROM orders o JOIN conversations c ON c.id=o.conversation_id JOIN whatsapp_accounts a ON a.id=c.account_id JOIN contacts co ON co.id=c.contact_id LEFT JOIN messages m ON m.id=o.summary_message_id WHERE o.id=$1 AND o.conversation_id=$2 AND o.deleted_at IS NULL",[orderId,conversationId]);if(!found.rowCount||!canAccessAccount(principal,found.rows[0].account_id))return reply.code(404).send({error:"not_found"});const order=found.rows[0],requestMessageId=parsed.data.clientSendId?`${order.client_order_id}:send:${parsed.data.clientSendId}`:`${order.client_order_id}:${parsed.data.format}`;
  const [itemResult,feeResult]=await Promise.all([pool.query("SELECT i.id,i.product_name name,i.quantity,i.unit_amount,i.image_media_id,m.object_key,m.mime_type FROM order_items i LEFT JOIN media m ON m.id=i.image_media_id WHERE i.order_id=$1 ORDER BY i.position",[orderId]),pool.query("SELECT name,amount FROM order_fees WHERE order_id=$1 ORDER BY position",[orderId])]);const items:OrderSummaryItem[]=itemResult.rows.map(item=>({name:String(item.name),quantity:Number(item.quantity),unitAmount:Number(item.unit_amount)})),fees:OrderSummaryFee[]=feeResult.rows.map(fee=>({name:String(fee.name),amount:Number(fee.amount)}));const englishSummary=formatOrderSummary(Number(order.order_number),items,fees,order.currency,order.description??undefined);let outgoingText=englishSummary;
  if(order.translate_on_send){const setting=await activeTranslationSetting();if(!setting)return reply.code(409).send({error:"translation_not_configured",message:"AI 翻译服务尚未配置，订单草稿未发送"});try{outgoingText=await translateText(setting,{text:englishSummary,targetLanguage:order.target_language});}catch(error){request.log.error({orderId,error:String(error)},"Order translation failed");return reply.code(502).send({error:"translation_failed",message:"订单翻译失败，草稿仍保留且尚未发送"});}}
  ...
  if(parsed.data.format==="image"){
    try{const products=await Promise.all(itemResult.rows.map(async item=>{if(!item.object_key)return{name:String(item.name)};const object=await s3.send(new GetObjectCommand({Bucket:config.S3_BUCKET,Key:item.object_key}));if(!object.Body)return{name:String(item.name)};return{name:String(item.name),image:Buffer.from(await object.Body.transformToByteArray())};}));
      const png=await renderOrderImage(outgoingText,products),sha256=createHash("sha256").update(png).digest("hex"),objectKey=`orders/${order.account_id}/${orderId}/${sha256}.png`,fileName=`order-${String(order.order_number).padStart(6,"0")}.png`;
  ...
  const queued=await transaction(async client=>{const locked=await client.query("SELECT deleted_at FROM orders WHERE id=$1 FOR UPDATE",[orderId]);if(!locked.rowCount||locked.rows[0].deleted_at)return null;const existing=await client.query("SELECT id FROM messages WHERE account_id=$1 AND client_message_id=$2",[order.account_id,requestMessageId]);if(existing.rowCount)return{messageId:existing.rows[0].id,format:parsed.data.format,deduplicated:true};const kind=parsed.data.format==="image"?"image":"text",caption=parsed.data.format==="image"?`Order #${String(order.order_number).padStart(6,"0")}`:outgoingText;const message=await client.query("INSERT INTO messages(conversation_id,account_id,sender_user_id,client_message_id,direction,kind,text_content,translation_source_text,media_id,status,occurred_at) VALUES($1,$2,$3,$4,'out',$5,$6,$7,$8,'queued',now()) RETURNING id,status",[conversationId,order.account_id,principal.id,requestMessageId,kind,caption,parsed.data.format==="text"&&order.translate_on_send?englishSummary:null,renderedMediaId]);await queueOrderCommand(client,order,conversationId,message.rows[0].id,requestMessageId,kind,caption,renderedMediaId??undefined);
    await client.query("UPDATE orders SET status='queued',send_format=$2,summary_message_id=$3,rendered_media_id=$4,translated_text=$5,sent_at=now() WHERE id=$1",[orderId,parsed.data.format,message.rows[0].id,renderedMediaId,order.translate_on_send?outgoingText:null]);await client.query("UPDATE conversations SET status='open',closed_at=NULL,last_message_at=now() WHERE id=$1",[conversationId]);await client.query("INSERT INTO audit_log(actor_type,actor_id,action,target_type,target_id,metadata) VALUES('user',$1,'order.send','order',$2,$3)",[principal.id,orderId,JSON.stringify({conversationId,resend:order.status!=="draft",format:parsed.data.format,translated:Boolean(order.translate_on_send),targetLanguage:order.target_language??null,productImageCount:itemResult.rows.filter(item=>item.image_media_id).length})]);return{messageId:message.rows[0].id,format:parsed.data.format,deduplicated:false};});
  if(!queued)return reply.code(404).send({error:"not_found"});if(order.agent_id)void dispatchPending(order.agent_id);return reply.code(202).send({orderId,orderNumber:Number(order.order_number),messageId:queued.messageId,status:"queued",format:queued.format,deduplicated:queued.deduplicated});
});

app.delete("/api/v1/conversations/:conversationId/orders/:orderId",{preHandler:authenticate},async(request,reply)=>{
  if(request.principal?.kind!=="user")return reply.code(403).send({error:"user_required"});const principal=request.principal;const {conversationId,orderId}=request.params as {conversationId:string;orderId:string};const deleted=await transaction(async client=>{const found=await client.query("SELECT o.status,c.account_id FROM orders o JOIN conversations c ON c.id=o.conversation_id WHERE o.id=$1 AND o.conversation_id=$2 AND o.deleted_at IS NULL FOR UPDATE",[orderId,conversationId]);if(!found.rowCount||!canAccessAccount(principal,found.rows[0].account_id))return false;await client.query("UPDATE orders SET deleted_at=now() WHERE id=$1",[orderId]);await client.query("INSERT INTO audit_log(actor_type,actor_id,action,target_type,target_id,metadata) VALUES('user',$1,'order.delete','order',$2,$3)",[principal.id,orderId,JSON.stringify({conversationId,wasSent:found.rows[0].status!=="draft"})]);return true;});if(!deleted)return reply.code(404).send({error:"not_found"});return reply.code(204).send();
});
...
type OrderProductInput={name:string;quantity:number;unitAmount:number;imageMediaId?:string;productId?:string;clientProductId?:string};

function mapProductRow(row:Record<string,unknown>){return{id:String(row.id),name:String(row.name),defaultUnitAmount:Number(row.default_unit_amount),currency:String(row.currency),imageMediaId:row.image_media_id?String(row.image_media_id):null,imageName:String(row.image_name??""),tags:Array.isArray(row.tags)?row.tags:[],createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}

async function productById(client:PoolClient,id:string){const result=await client.query(`SELECT p.id,p.name,p.default_unit_amount,p.currency,p.image_media_id,m.file_name image_name,p.created_at,p.updated_at,COALESCE(label_list.tags,'[]'::json) tags FROM products p LEFT JOIN media m ON m.id=p.image_media_id LEFT JOIN LATERAL (SELECT json_agg(json_build_object('id',label.id,'name',label.name,'color',label.color) ORDER BY lower(label.name)) tags FROM product_labels label WHERE label.product_id=p.id) label_list ON true WHERE p.id=$1`,[id]);return result.rowCount?mapProductRow(result.rows[0]):null;}
...
async function resolveOrderProduct(client:PoolClient,item:OrderProductInput,orderNumber:number,currency:string,actorId:string,conversationLabels:ProductLabelInput[]):Promise<string|null>{
  ...
  const created=await client.query("INSERT INTO products(client_product_id,name,default_unit_amount,currency,image_media_id,created_by) VALUES($1,$2,$3,$4,$5,$6) RETURNING id",[item.clientProductId,item.name,item.unitAmount,currency,item.imageMediaId??null,actorId]);
  const labels=[{name:`订单 #${String(orderNumber).padStart(6,"0")}`,color:"#E8EEF7"},...conversationLabels];await replaceProductLabels(client,created.rows[0].id,labels);
