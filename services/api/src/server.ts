...
import { conversationTagsSchema, customerStageSchema, enrollmentSchema, loginSchema, messageSchema, messageTranslationsSchema, newConversationSchema, noteSchema, orderSchema, orderSendSchema, reminderSchema, tagCreateSchema, tagUpdateSchema, textToSpeechSchema, translationPreferenceQuerySchema, translationPreferenceSchema, translationPreviewSchema, translationProviderSettingsSchema, ttsProviderSettingsSchema } from "./schemas.js";
...
app.get("/api/v1/openapi.json", async () => ({ openapi:"3.1.0", info:{title:"RelayDesk API",version:"0.1.0"}, paths:{
  ...
  "/api/v1/conversations/{id}/orders/{orderId}/send":{post:{summary:"以文字或完整图片格式发送订单草稿"}},
  "/api/v1/conversations/{id}/orders/{orderId}":{delete:{summary:"从联系人资料中删除订单"}},
...
app.post("/api/v1/conversations/:conversationId/orders/:orderId/send",{preHandler:authenticate},async(request,reply)=>{
  ...
  const queued=await transaction(async client=>{const locked=await client.query("SELECT status,summary_message_id,send_format,deleted_at FROM orders WHERE id=$1 FOR UPDATE",[orderId]);if(!locked.rowCount||locked.rows[0].deleted_at)return null;if(locked.rows[0].status!=="draft")return{messageId:locked.rows[0].summary_message_id,format:locked.rows[0].send_format,deduplicated:true};const clientMessageId=`${order.client_order_id}:${parsed.data.format}`,kind=parsed.data.format==="image"?"image":"text",caption=parsed.data.format==="image"?`Order #${String(order.order_number).padStart(6,"0")}`:outgoingText;const message=await client.query("INSERT INTO messages(conversation_id,account_id,sender_user_id,client_message_id,direction,kind,text_content,translation_source_text,media_id,status,occurred_at) VALUES($1,$2,$3,$4,'out',$5,$6,$7,$8,'queued',now()) RETURNING id,status",[conversationId,order.account_id,principal.id,clientMessageId,kind,caption,parsed.data.format==="text"&&order.translate_on_send?englishSummary:null,renderedMediaId]);await queueOrderCommand(client,order,conversationId,message.rows[0].id,clientMessageId,kind,caption,renderedMediaId??undefined);
    await client.query("UPDATE orders SET status='queued',send_format=$2,summary_message_id=$3,rendered_media_id=$4,translated_text=$5,sent_at=now() WHERE id=$1",[orderId,parsed.data.format,message.rows[0].id,renderedMediaId,order.translate_on_send?outgoingText:null]);await client.query("UPDATE conversations SET status='open',closed_at=NULL,last_message_at=now() WHERE id=$1",[conversationId]);await client.query("INSERT INTO audit_log(actor_type,actor_id,action,target_type,target_id,metadata) VALUES('user',$1,'order.send','order',$2,$3)",[principal.id,orderId,JSON.stringify({conversationId,format:parsed.data.format,translated:Boolean(order.translate_on_send),targetLanguage:order.target_language??null,productImageCount:itemResult.rows.filter(item=>item.image_media_id).length})]);return{messageId:message.rows[0].id,format:parsed.data.format,deduplicated:false};});
