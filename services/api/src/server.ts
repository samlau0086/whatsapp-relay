...
app.get("/api/v1/conversations", { preHandler:authenticate }, async (request,reply) => {
  ...
  const searchCte=keyword?`search_conversations AS MATERIALIZED (
    ...
    WHERE (COALESCE(search_contact.alias,'') || ' ' || COALESCE(search_contact.display_name,'') || ' ' || COALESCE(search_contact.phone_e164,'') || ' ' || search_contact.provider_user_id || ' ' || COALESCE(search_contact.whatsapp_username,'')) ILIKE '%'||$4||'%'
  ),`:"";
