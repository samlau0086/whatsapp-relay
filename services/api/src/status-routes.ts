...
export async function registerStatusRoutes(app:FastifyInstance):Promise<void>{
  ...
  app.patch("/api/v1/status-posts/:id",{preHandler:authenticate},async(request,reply)=>{
    if(!canManage(request.principal))return reply.code(403).send({error:"supervisor_required"});const parsed=postItemObject.partial().refine(value=>Object.keys(value).length>0).safeParse(request.body);if(!parsed.success)return reply.code(400).send({error:"invalid_request"});const {id}=request.params as {id:string},found=await pool.query("SELECT p.*,c.status campaign_status FROM status_posts p JOIN status_campaigns c ON c.id=p.campaign_id WHERE p.id=$1",[id]);if(!found.rowCount||!await canRead(request.principal,String(found.rows[0].account_id)))return reply.code(404).send({error:"not_found"});if(!["queued","scheduled","failed","expired"].includes(String(found.rows[0].status)))return reply.code(409).send({error:"status_post_immutable"});const merged={clientPostId:String(found.rows[0].client_post_id),type:parsed.data.type??found.rows[0].content_type,text:parsed.data.text??found.rows[0].text_content,mediaId:parsed.data.mediaId??found.rows[0].media_id,backgroundColor:parsed.data.backgroundColor??found.rows[0].background_color,font:parsed.data.font??found.rows[0].font},valid=postItemSchema.safeParse(merged);if(!valid.success)return reply.code(400).send({error:"invalid_request",details:valid.error.flatten()});await transaction(async client=>{await validateMedia(client,String(found.rows[0].account_id),valid.data);await client.query("UPDATE status_posts SET content_type=$2,text_content=$3,media_id=$4,background_color=$5,font=$6,updated_at=now() WHERE id=$1",[id,valid.data.type,valid.data.text??null,valid.data.mediaId??null,valid.data.backgroundColor??null,valid.data.font??null]);});return reply.code(204).send();
  });
