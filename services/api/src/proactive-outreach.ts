...
export async function ensureProactiveOutreachTables():Promise<void>{
  proactiveSchemaReady??=(async()=>{
    ...
    await pool.query("ALTER TABLE proactive_outreach_settings ADD COLUMN IF NOT EXISTS updated_at timestamptz NOT NULL DEFAULT now()");
...
const audit=async(client:PoolClient,accountId:string,contactId:string,jobId:string|undefined,type:string,reason:string,plannedAt?:Date|string|null,metadata:Record<string,unknown>={})=>client.query("INSERT INTO proactive_outreach_events(account_id,contact_id,job_id,event_type,reason,planned_at,metadata) VALUES($1,$2,$3,$4,$5,$6,$7)",[accountId,contactId,jobId??null,type,reason,plannedAt??null,JSON.stringify(metadata)]);
export async function cancelProactiveForConversation(client:PoolClient,conversationId:string,reason:string){await ensureProactiveOutreachTables();const rows=await client.query("UPDATE proactive_outreach_jobs SET state='cancelled',completed_at=now(),last_error=$2 WHERE conversation_id=$1 AND state IN ('pending','processing') RETURNING id,account_id,contact_id,planned_at",[conversationId,reason]);for(const row of rows.rows)await audit(client,row.account_id,row.contact_id,row.id,"cancelled",reason,row.planned_at);}
...
export async function scanProactiveOutreach(force=false){
  await ensureProactiveOutreachTables();
  if(!force&&Date.now()-lastScan<60_000)return;
  lastScan=Date.now();
  await transaction(async client=>{
    const stale=await client.query(`SELECT d.id FROM ai_drafts d JOIN proactive_outreach_jobs pj ON pj.payload->>'draftId'=d.id::text WHERE pj.state='awaiting_approval' AND (d.status='dismissed' OR (d.status='pending' AND EXISTS (SELECT 1 FROM messages m WHERE m.conversation_id=d.conversation_id AND m.direction='out' AND m.status IN ('queued','dispatching','sent','delivered','read') AND (m.occurred_at>=d.created_at-interval '7 days' OR regexp_replace(lower(btrim(m.text_content)), '[[:space:]]+', ' ', 'g')=regexp_replace(lower(btrim(d.text_content)), '[[:space:]]+', ' ', 'g'))))) FOR UPDATE OF d`);
    if(!stale.rowCount)return;
    const draftIds=stale.rows.map(row=>row.id);
    await client.query("UPDATE ai_drafts SET status='dismissed',resolved_at=now() WHERE id=ANY($1::uuid[]) AND status='pending'",[draftIds]);
    const skipped=await client.query("UPDATE proactive_outreach_jobs SET state='skipped',completed_at=now(),last_error='draft_superseded',updated_at=now() WHERE state='awaiting_approval' AND payload->>'draftId'=ANY($1::text[]) RETURNING id,account_id,contact_id,planned_at",[draftIds]);
    for(const row of skipped.rows)await audit(client,row.account_id,row.contact_id,row.id,"skipped","draft_superseded",row.planned_at);
  });
  const rows=await pool.query(`SELECT c.id contact_id,c.account_id,cv.id conversation_id,s.max_touches_per_year,c.country country_code,c.timezone,c.preferred_language,c.company_name,c.alias,c.display_name,c.phone_e164,COALESCE(st.mode,'human_paused') mode,(SELECT max(m.occurred_at) FROM messages m WHERE m.conversation_id=cv.id) last_contact_at,(SELECT count(*)::int FROM proactive_outreach_jobs sent_job JOIN messages sent_message ON sent_message.id=sent_job.message_id WHERE sent_job.contact_id=c.id AND sent_job.state='sent' AND sent_message.status IN ('sent','delivered','read') AND sent_message.occurred_at>now()-interval '365 days') touches FROM contacts c JOIN conversations cv ON cv.contact_id=c.id JOIN proactive_outreach_settings s ON s.account_id=c.account_id AND s.enabled LEFT JOIN conversation_agent_state st ON st.conversation_id=cv.id WHERE c.entity_type='person' AND c.whatsapp_blocked_at IS NULL AND c.proactive_suppressed_at IS NULL AND cv.status='open' AND cv.customer_stage IN ('new','considering','qualified') AND COALESCE(st.mode,'human_paused')<>'human_paused'`);
  ...
  try{
    await transaction(async client=>{
      ...
      if(!state||state.whatsapp_blocked_at||state.proactive_suppressed_at||state.status!=="open"||!["new","considering","qualified"].includes(state.customer_stage)||state.mode==="human_paused"||Number(state.touches)>=Number(job.max_touches_per_year)||Number(state.touches)>Number((job.payload as Record<string,unknown>|null)?.touches??0)||Date.now()-new Date(String(state.last_contact_at??0)).getTime()<proactiveCadenceDays(Number(state.touches))*DAY){
        ...
        await audit(client,job.account_id,job.contact_id,job.id,"skipped","eligibility_changed",job.planned_at);
        return;
      }
      ...
      if(blocked.blocked||!windowOpen){const reason=blocked.blocked?`holiday_or_weekend:${blocked.reason}`:"outside_local_window",nextRun=nextEligibleProactiveRunAt(now,timezone,windowStart,windowEnd,job.country_holidays,contactCountry),local=localParts(now,timezone);await deferProactiveJob(client,job,nextRun,reason,{generationMode:"deferred",fallbackReason:reason,usedTemplateId:null,holidaySkipped:blocked.blocked,contactTimezone:timezone,contactLocalTime:`${local.year}-${String(local.month).padStart(2,"0")}-${String(local.day).padStart(2,"0")} ${String(local.hour).padStart(2,"0")}:${String(local.minute).padStart(2,"0")}`,allowedWindow:`${windowStart}-${windowEnd}`,rescheduledAt:nextRun.toISOString()});return;}
      ...
      const dailyCount=await client.query("SELECT count(*)::int count FROM proactive_outreach_jobs j JOIN messages m ON m.id=j.message_id WHERE j.account_id=$1 AND j.state='sent' AND m.status IN ('queued','dispatching','sent','delivered','read') AND m.occurred_at>=date_trunc('day',now() AT TIME ZONE $2) AT TIME ZONE $2",[job.account_id,accountTimezone]);
      ...
      const orders=await client.query("SELECT o.id,o.display_order_number order_number,o.business_status status,o.amount,o.currency,o.description,o.created_at,COALESCE((SELECT json_agg(json_build_object('name',i.product_name,'quantity',i.quantity,'unitAmount',i.unit_amount) ORDER BY i.position) FROM order_items i WHERE i.order_id=o.id),'[]'::json) items FROM orders o WHERE o.conversation_id=$1 AND o.deleted_at IS NULL ORDER BY o.created_at DESC LIMIT 5",[job.conversation_id]);
      ...
      const duplicate=await client.query("SELECT 1 FROM messages WHERE conversation_id=$1 AND direction='out' AND status IN ('queued','dispatching','sent','delivered','read') AND regexp_replace(lower(btrim(text_content)), '[[:space:]]+', ' ', 'g')=regexp_replace(lower(btrim($2::text)), '[[:space:]]+', ' ', 'g') LIMIT 1",[job.conversation_id,reply]);
      if(duplicate.rowCount){await client.query("UPDATE proactive_outreach_jobs SET state='skipped',completed_at=now(),last_error='already_sent' WHERE id=$1",[job.id]);await audit(client,job.account_id,job.contact_id,job.id,"skipped","already_sent",job.planned_at);return;}
    ...
    await pool.query("INSERT INTO proactive_outreach_events(account_id,contact_id,job_id,event_type,reason,planned_at) VALUES($1,$2,$3,'failed',$4,$5)",[job.account_id,job.contact_id,job.id,reason,job.planned_at]);
  }
  return true;
}
