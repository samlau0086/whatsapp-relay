...
export class PayPalClient{
  ...
  private async addTrackers(trackers:Record<string,unknown>[]):Promise<void>{
    const result=await this.api("/v1/shipping/trackers-batch",{method:"POST",body:JSON.stringify({trackers})}),errors=Array.isArray(result.errors)?result.errors:[],first=errors[0]&&typeof errors[0]==="object"?errors[0] as Record<string,unknown>:null;
