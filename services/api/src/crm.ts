...
export type OrderSummaryItem={name:string;sku?:string;quantity:number;unitAmount:number;isOutOfStock?:boolean;brand?:string;category?:string;description?:string};
...
export function formatOrderSummary(orderNumber:string|number,items:OrderSummaryItem[],fees:OrderSummaryFee[],currency:string,description?:string):string{
  ...
  const lines=[`Order #${displayNumber}`,"","Items:",...items.map((item,index)=>item.isOutOfStock?`${index+1}. ${item.name} x ${item.quantity} - Out of stock`:`${index+1}. ${item.name} x ${item.quantity} - ${currency} ${item.unitAmount.toFixed(2)} each - ${currency} ${(item.quantity*item.unitAmount).toFixed(2)}`)];
...
export async function ensureCrmTables(db:Queryable):Promise<void>{
  ...
  await db.query("ALTER TABLE notes ADD COLUMN IF NOT EXISTS note_type text NOT NULL DEFAULT 'normal'");
  await db.query("ALTER TABLE notes DROP CONSTRAINT IF EXISTS notes_note_type_check");
  await db.query("ALTER TABLE notes ADD CONSTRAINT notes_note_type_check CHECK(note_type IN ('order','normal'))");
  ...
  await db.query("ALTER TABLE orders ADD COLUMN IF NOT EXISTS business_status text NOT NULL DEFAULT 'quotation'");
  ...
  await db.query("ALTER TABLE order_settings ADD COLUMN IF NOT EXISTS pdf_template jsonb");
  await db.query("ALTER TABLE order_settings ADD COLUMN IF NOT EXISTS inq_template jsonb");
  await db.query("UPDATE order_settings SET pdf_template=COALESCE(pdf_template,image_template) WHERE pdf_template IS NULL");
  ...
  await db.query("ALTER TABLE orders DROP CONSTRAINT IF EXISTS orders_business_status_check");
  await db.query(`ALTER TABLE orders ADD CONSTRAINT orders_business_status_check CHECK(business_status IN ('quotation','pending_confirmation','pending_payment','paid','processing','shipped','completed','cancelled'))`);
  await db.query("CREATE INDEX IF NOT EXISTS orders_business_status_created_idx ON orders(business_status,created_at DESC) WHERE deleted_at IS NULL");
  await db.query("ALTER TABLE orders DROP CONSTRAINT IF EXISTS orders_send_format_check");
  await db.query("ALTER TABLE orders ADD CONSTRAINT orders_send_format_check CHECK(send_format IS NULL OR send_format IN ('text','image','pdf'))");
  ...
  await db.query(`CREATE TABLE IF NOT EXISTS product_variants (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),product_id uuid NOT NULL REFERENCES products(id) ON DELETE CASCADE,attributes jsonb NOT NULL,sku text NOT NULL,default_unit_amount numeric(12,2) NOT NULL CHECK(default_unit_amount>=0),image_media_id uuid REFERENCES media(id) ON DELETE SET NULL,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),UNIQUE(product_id,attributes))`);
  await db.query(`CREATE UNIQUE INDEX IF NOT EXISTS product_variants_sku_idx ON product_variants(lower(btrim(sku)))`);
  await db.query(`CREATE TABLE IF NOT EXISTS product_variant_price_tiers (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),variant_id uuid NOT NULL REFERENCES product_variants(id) ON DELETE CASCADE,min_quantity integer NOT NULL CHECK(min_quantity BETWEEN 1 AND 999999),unit_amount numeric(12,2) NOT NULL CHECK(unit_amount>=0),UNIQUE(variant_id,min_quantity))`);
  await db.query("ALTER TABLE product_variant_price_tiers ADD COLUMN IF NOT EXISTS cost_amount numeric(12,2)");
  await db.query("ALTER TABLE product_variant_price_tiers ADD COLUMN IF NOT EXISTS profit_margin numeric(10,4)");
  await db.query("CREATE INDEX IF NOT EXISTS product_variant_price_tiers_variant_quantity_idx ON product_variant_price_tiers(variant_id,min_quantity)");
