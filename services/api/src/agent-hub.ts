...
async function processBatch(agentId: string, frame: AgentFrame): Promise<{ackedCursor:number;failedCursor?:number;error?:string}> {
  const events = Array.isArray(frame.events) ? frame.events as Array<{cursor?:number;kind:string;live?:boolean;payload:Record<string,unknown>}> : [];
    ...
    try{
      ...
      await transaction(async client=>{
        ...
        if(inserted.rowCount){
          if(event.kind==="message")await ingestNormalizedMessage(client,event.payload,{agentId,live:event.live});
...
export async function ingestNormalizedMessage(client: import("pg").PoolClient, payload: Record<string,unknown>, source:{agentId?:string;transport?:'cloud';live?:boolean}): Promise<void> {
  ...
  if(chatJid.endsWith("@g.us")){
    ...
    if(message.rowCount){
      await client.query("UPDATE conversations SET unread_count=unread_count+CASE WHEN $2='in' THEN 1 ELSE 0 END,status=CASE WHEN $3::boolean THEN 'open'::conversation_status ELSE status END,closed_at=CASE WHEN $3::boolean THEN NULL ELSE closed_at END WHERE id=$1",[peer.conversationId,payload.direction,source.live!==false]);
...
async function upsertGroupPeer(client:import("pg").PoolClient,accountId:string,groupJid:string,subject:string):Promise<{groupId:string;contactId:string;conversationId:string}>{
  ...
  const conversation=await client.query("INSERT INTO conversations(account_id,contact_id,unread_count) VALUES($1,$2,0) ON CONFLICT(account_id,contact_id) DO UPDATE SET status=CASE WHEN conversations.status='archived' THEN 'open'::conversation_status ELSE conversations.status END RETURNING id",[accountId,contactId]);
  const contactGroup=await client.query("SELECT id FROM whatsapp_groups WHERE contact_id=$1 FOR UPDATE",[contactId]);
  if(contactGroup.rowCount){
    const groupId=String(contactGroup.rows[0].id);
    await client.query("UPDATE whatsapp_groups SET group_jid=$2,subject=$3,active=true,updated_at=now() WHERE id=$1",[groupId,groupJid,subject]);
    return{groupId,contactId,conversationId:String(conversation.rows[0].id)};
  }
