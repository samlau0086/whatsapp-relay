...
export async function ensureEmailTables():Promise<void>{
  ...
  await pool.query("ALTER TABLE email_messages DROP CONSTRAINT IF EXISTS email_messages_content_type_check");
  await pool.query("ALTER TABLE email_messages ADD CONSTRAINT email_messages_content_type_check CHECK(content_type IN ('text','order_text','order_image','order_pdf','product_cards'))");
...
async function sendResend(job:EmailJob,attachments:Awaited<ReturnType<typeof loadAttachments>>):Promise<string>{
  ...
  const response=await fetch("https://api.resend.com/emails",{method:"POST",headers:{authorization:`Bearer ${apiKey}`,"content-type":"application/json","idempotency-key":`relaydesk/email/${job.id}`},body:JSON.stringify({from:`${cfg.fromName} <${cfg.fromEmail}>`,to:job.recipients.map(item=>item.email),reply_to:cfg.replyTo||undefined,subject:job.subject,text:job.text_body,html:job.html_body,headers:job.in_reply_to?{"In-Reply-To":job.in_reply_to,"References":job.references_header||job.in_reply_to}:undefined,attachments:attachments.map(item=>({filename:item.filename,content:item.content.toString("base64"),content_type:item.contentType,content_disposition:item.contentType==="application/pdf"?"attachment":"inline",content_id:item.cid}))})});
