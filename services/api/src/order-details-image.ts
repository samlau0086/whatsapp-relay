...
import { parseOrderTemplate, parseTranslatedSemanticOrder, renderSemanticOrder, serializeSemanticOrder } from "./order-template.js";
import { decryptAtRest } from "./security.js";
import { translateText, type TranslationProvider, type TranslationProviderSetting } from "./translation-providers.js";
...
export async function ensureOrderDetailsImage(orderId:string,targetLanguage:string|null=null):Promise<string|null>{
  ...
  const template=parseOrderTemplate(templateResult.rows[0]?.image_template,"image"),sourceBlocks=renderSemanticOrder(template,{orderNumber:String(order.display_order_number),businessStatus:order.business_status,currency:String(order.currency),customerName:String(order.customer_name??""),customerPhone:String(order.customer_phone??""),description:String(order.description??""),items,fees,address:order.shipping_address_snapshot??null,contact:{firstName:order.contact_first_name,lastName:order.contact_last_name,companyName:order.contact_company_name,country:order.contact_country,province:order.contact_province,city:order.contact_city,email:order.contact_email}});
  let blocks=sourceBlocks;
  if(targetLanguage&&targetLanguage!=="en"){
    const setting=await activeTranslationSetting();if(!setting)throw new Error("translation_not_configured");
    blocks=parseTranslatedSemanticOrder(await translateText(setting,{text:serializeSemanticOrder(sourceBlocks),targetLanguage}),sourceBlocks);
  }
  ...
  const png=await renderTemplateOrderImage(template,blocks,products),sha256=createHash("sha256").update(png).digest("hex"),objectKey=`orders/${order.account_id}/${orderId}/${sha256}.png`,language=targetLanguage??"en",fileName=`order-${safeFileName(String(order.display_order_number))}-${safeFileName(language)}.png`;
  await s3.send(new PutObjectCommand({Bucket:config.S3_BUCKET,Key:objectKey,Body:png,ContentType:"image/png",Metadata:{sha256,orderId,source:"ai-agent",language}}));
...
async function activeTranslationSetting():Promise<TranslationProviderSetting|null>{
  const result=await pool.query("SELECT provider,api_key_encrypted,base_url,model,transcription_model FROM translation_provider_settings WHERE enabled=true AND api_key_encrypted IS NOT NULL LIMIT 1");if(!result.rowCount)return null;const row=result.rows[0];return{provider:row.provider as TranslationProvider,apiKey:decryptAtRest(row.api_key_encrypted,config.DATA_ENCRYPTION_KEY),baseUrl:row.base_url,model:row.model,transcriptionModel:row.transcription_model};
}
