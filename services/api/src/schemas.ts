...
export const contactUpdateSchema=z.object({alias:z.string().trim().max(80),phone:whatsappPhoneSchema.optional(),note:z.string().trim().max(5000),birthday:calendarDateSchema.nullable().optional(),specialDates:z.array(contactSpecialDateSchema).max(30).optional(),emails:z.array(contactEmailSchema).max(20),methods:z.array(contactMethodSchema).max(30),addresses:z.array(contactAddressSchema).max(20).default([])}).superRefine((value,ctx)=>{
