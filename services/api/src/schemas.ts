...
export const newConversationSchema = z.object({
  ...
  clientMessageId: z.string().min(8).max(128),
}).refine(value=>Boolean(value.phone||value.whatsappUsername),{path:["phone"],message:"手机号和 WhatsApp 用户名至少填写一个"});
...
export const contactUpdateSchema=z.object({alias:z.string().trim().max(80),firstName:contactNamePartSchema.optional(),middleName:contactNamePartSchema.optional(),lastName:contactNamePartSchema.optional(),companyName:contactOrganizationFieldSchema.optional(),jobTitle:contactOrganizationFieldSchema.optional(),country:contactLocationFieldSchema.optional(),province:contactLocationFieldSchema.optional(),city:contactLocationFieldSchema.optional(),phone:whatsappPhoneSchema.optional(),whatsappUsername:whatsappUsernameSchema.nullable().optional(),note:z.string().trim().max(5000),timezone:z.string().trim().max(100).nullable().optional(),preferredLanguage:z.string().trim().regex(/^[A-Za-z]{2,3}(?:-[A-Za-z0-9]{2,8})*$/).max(35).nullable().optional(),birthday:calendarDateSchema.nullable().optional(),specialDates:z.array(contactSpecialDateSchema).max(30).optional(),emails:z.array(contactEmailSchema).max(20),methods:z.array(contactMethodSchema).max(30),addresses:z.array(contactAddressSchema).max(20).default([])}).superRefine((value,ctx)=>{
  if(value.phone===null&&value.whatsappUsername===null&&!value.emails.length)ctx.addIssue({code:"custom",path:["phone"],message:"WhatsApp 号码、用户名或邮箱至少填写一个"});
...
const productExternalImageUrlSchema=z.string().trim().url().max(2000).refine(value=>/^https?:\/\//i.test(value),"external image URL must use http or https");
const productContentFields={name:z.string().trim().min(1).max(120),sku:z.string().trim().min(1).max(80),description:z.string().trim().max(2000).default(""),category:z.string().trim().max(80).default(""),brand:z.string().trim().max(80).default(""),priceTiers:productPriceTiersSchema,currency:currencySchema,isInStock:z.boolean().default(true),imageMediaId:z.string().uuid().nullable().optional(),galleryMediaIds:z.array(z.string().uuid()).max(12).refine(value=>new Set(value).size===value.length,"gallery images must be unique").optional(),galleryExternalUrls:z.array(productExternalImageUrlSchema).max(12).refine(value=>new Set(value).size===value.length,"gallery image URLs must be unique").optional(),shippingClassId:z.string().uuid().nullable().optional(),tags:z.array(productLabelSchema).max(30).default([]),supplierLinks:z.array(supplierLinkSchema).max(30).default([]),internalNote:z.string().trim().max(4000).default(""),variants:productVariantsSchema.optional(),...optionalWeightFields};
function validateGallerySources(value:{galleryMediaIds?:string[];galleryExternalUrls?:string[]},ctx:z.RefinementCtx){if((value.galleryMediaIds?.length??0)+(value.galleryExternalUrls?.length??0)>12)ctx.addIssue({code:"custom",path:["galleryExternalUrls"],message:"at most 12 gallery images are supported"});}
const productContentSchema=z.object(productContentFields).superRefine((value,ctx)=>{validateWeightPair(value,ctx);validateGallerySources(value,ctx);});
export const productCreateSchema=z.object({clientProductId:z.string().uuid()}).and(productContentSchema);
const productBulkImportItemSchema=z.object({clientProductId:z.string().uuid(),sku:z.string().trim().min(1).max(80),name:z.string().trim().min(1).max(120),description:z.string().trim().max(2000).optional(),category:z.string().trim().max(80).optional(),brand:z.string().trim().max(80).optional(),priceTiers:productPriceTiersSchema.optional(),currency:currencySchema.optional(),imageMediaId:z.string().uuid().nullable().optional(),galleryMediaIds:z.array(z.string().uuid()).max(12).refine(value=>new Set(value).size===value.length,"gallery images must be unique").optional(),galleryExternalUrls:z.array(productExternalImageUrlSchema).max(12).refine(value=>new Set(value).size===value.length,"gallery image URLs must be unique").optional(),shippingClassId:z.string().uuid().nullable().optional(),tags:z.array(productLabelSchema).max(30).optional(),supplierLinks:z.array(supplierLinkSchema).max(30).optional(),internalNote:z.string().trim().max(4000).optional(),variants:productVariantsSchema.optional(),...optionalWeightFields}).superRefine((value,ctx)=>{validateWeightPair(value,ctx);validateGallerySources(value,ctx);});
...
export const productUpdateSchema=z.object(productContentFields).partial().superRefine((value,ctx)=>{if(!Object.keys(value).length)ctx.addIssue({code:"custom",message:"at least one field is required"});validateWeightPair(value,ctx);validateGallerySources(value,ctx);});
...
export const mailboxSettingsSchema=z.object({accountId:z.string().uuid(),address:z.string().trim().toLowerCase().email().max(254),displayName:z.string().trim().max(120).default(""),isPrimary:z.boolean().default(false),enabled:z.boolean().default(true),imapHost:z.string().trim().min(1).max(253),imapPort:z.coerce.number().int().min(1).max(65535),imapUsername:z.string().trim().min(1).max(254),imapPassword:z.string().min(1),smtpHost:z.string().trim().min(1).max(253),smtpPort:z.coerce.number().int().min(1).max(65535),smtpTls:z.enum(["tls","starttls"]),smtpUsername:z.string().trim().min(1).max(254),smtpPassword:z.string().min(1)});
export const conversationEmailSchema=z.object({clientSendId:z.string().uuid(),contactId:z.string().uuid().optional(),mailboxId:z.string().uuid(),recipientEmailId:z.string().uuid().optional(),recipientEmail:z.string().email().optional(),subject:emailSubjectSchema,body:z.string().trim().min(1).max(5000),attachmentIds:z.array(z.string().uuid()).max(10).default([]),replyToMessageId:z.string().uuid().optional()}).refine(value=>Boolean(value.recipientEmailId||value.recipientEmail),{path:["recipientEmail"],message:"recipient email is required"});
