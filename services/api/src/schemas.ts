...
export const productPriceTierSchema=z.object({minQuantity:z.coerce.number().int().min(1).max(999999),unitAmount:moneySchema,costAmount:moneySchema.optional(),profitMargin:z.coerce.number().min(0).max(100000).optional()}).superRefine((value,ctx)=>{if((value.costAmount===undefined)!==(value.profitMargin===undefined))ctx.addIssue({code:"custom",path:[value.costAmount===undefined?"costAmount":"profitMargin"],message:"cost amount and profit margin must be provided together"});});
