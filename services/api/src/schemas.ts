...
export const productPriceTierSchema=z.object({minQuantity:z.coerce.number().int().min(1).max(999999),unitAmount:moneySchema,costAmount:moneySchema.optional(),profitMargin:z.coerce.number().min(0).max(100000).refine(value=>Math.abs(value*10000-Math.round(value*10000))<1e-7,"profit margin supports at most four decimals").optional()}).superRefine((value,ctx)=>{if((value.costAmount===undefined)!==(value.profitMargin===undefined))ctx.addIssue({code:"custom",path:[value.costAmount===undefined?"costAmount":"profitMargin"],message:"cost amount and profit margin must be provided together"});if(value.costAmount!==undefined&&value.profitMargin!==undefined&&Math.round(value.costAmount*(1+value.profitMargin/100)*100)/100!==value.unitAmount)ctx.addIssue({code:"custom",path:["unitAmount"],message:"unit amount must match cost amount and profit margin"});});
...
const supplierLinkSchema=z.object({label:z.string().trim().max(120).default(""),url:z.string().trim().url().max(2000)});
