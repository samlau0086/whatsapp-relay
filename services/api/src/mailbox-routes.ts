...
export function registerMailboxRoutes(app:FastifyInstance):void{
  ...
  const send=async(request:FastifyRequest,reply:FastifyReply)=>{
    ...
    const [mailbox,recipient,media]=await Promise.all([
    ...
    const row=mailbox.rows[0],providerConfig:EmailProviderConfig={fromName:row.display_name||row.address,fromEmail:row.address,host:row.smtp_host,port:row.smtp_port,tls:row.smtp_tls,username:row.smtp_username};
    const result=await transaction(async client=>{
      ...
      const job=await client.query("INSERT INTO email_messages(client_send_id,conversation_id,contact_id,sender_user_id,provider,provider_config,provider_secret_encrypted,recipients,subject,message_body,text_body,html_body,content_type,mailbox_id,in_reply_to,references_header) VALUES($1,$2,$3,$4,'smtp',$5,$6,$7,$8,$9,$9,$10,'text',$11,$12,$13) RETURNING id",[value.clientSendId,conversationId,contactId,request.principal!.id,JSON.stringify(providerConfig),row.smtp_secret_encrypted,JSON.stringify([{email:recipient.rows[0].email,label:""}]),value.subject,value.body,emailShell(value.body,""),row.id,inReplyTo,references]);
