...
function startCentral():void{
  ...
  client=new CentralClient(store,baseUrl,agentId,credential,VERSION,`docker-${process.platform}-${process.arch}`,PROTOCOL_VERSION,["publish_status_v1"],executeCommand,status=>{store.set("connection",status);console.log(`Central connection: ${status}`);},()=>undefined);
  client.start();
}
