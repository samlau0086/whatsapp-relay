...
export class CentralClient {
  ...
  constructor(private store:AgentStore,private baseUrl:string,private agentId:string,private credential:string,private agentVersion:string,private protocolVersion:number,private capabilities:string[],private onCommand:CommandHandler,private onStatus:(value:string)=>void,private onAttentionCleared:AttentionClearedHandler){}
  ...
  private connect():void{
    ...
    this.socket.on("open",()=>{this.retry=0;this.onStatus("online");this.socket?.send(JSON.stringify({type:"hello",protocolVersion:this.protocolVersion,agentId:this.agentId,agentVersion:this.agentVersion,platform:`win32-${process.arch}`,lastAckedCursor:Number(this.store.get("lastAckedCursor")??0),capabilities:this.capabilities}));this.heartbeat=setInterval(()=>{this.socket?.send(JSON.stringify({type:"heartbeat",at:new Date().toISOString(),accounts:this.store.accounts().map((account)=>({accountId:account.id,status:account.status,queueDepth:0}))}));this.flush();},10_000);this.flush();});
