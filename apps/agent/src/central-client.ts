...
export class CentralClient {
  ...
  constructor(private store:AgentStore,private baseUrl:string,private agentId:string,private credential:string,private agentVersion:string,private platform:string,private protocolVersion:number,private capabilities:string[],private onCommand:CommandHandler,private onStatus:(value:string)=>void,private onAttentionCleared:AttentionClearedHandler,private proxyUrl?:string){}
  ...
  private connect():void{
    ...
    this.socket=new WebSocket(url,{headers:{authorization:`Bearer ${this.credential}`},agent});
    this.socket.on("open",()=>{this.retry=0;this.onStatus("online");this.socket?.send(JSON.stringify({type:"hello",protocolVersion:this.protocolVersion,agentId:this.agentId,agentVersion:this.agentVersion,platform:this.platform,lastAckedCursor:Number(this.store.get("lastAckedCursor")??0),capabilities:this.capabilities}));this.heartbeat=setInterval(()=>{this.socket?.send(JSON.stringify({type:"heartbeat",at:new Date().toISOString(),accounts:this.store.accounts().map((account)=>({accountId:account.id,status:account.status,queueDepth:0}))}));this.flush();},10_000);this.flush();});
