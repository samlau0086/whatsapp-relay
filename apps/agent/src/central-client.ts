...
type AccountReassignedHandler = (input:{accountId:string;accountName:string;action:"add"|"remove"})=>void;
export type CentralClientOptions = {
  ...
  onAccountReassigned:AccountReassignedHandler;
};
...
export class CentralClient {
  ...
  constructor(private store:AgentStore,private options:CentralClientOptions){}
  ...
  private connect():void{
    const url=new URL("/agent/ws",this.options.baseUrl.replace(/^http/,"ws"));
    const agent=this.options.proxyUrl?new HttpsProxyAgent(this.options.proxyUrl):undefined;
    this.socket=new WebSocket(url,{headers:{authorization:`Bearer ${this.options.credential}`},agent});
    this.socket.on("open",()=>{this.retry=0;this.options.onStatus("online");this.socket?.send(JSON.stringify({type:"hello",protocolVersion:this.options.protocolVersion,agentId:this.options.agentId,agentVersion:this.options.agentVersion,platform:this.options.platform,lastAckedCursor:Number(this.store.get("lastAckedCursor")??0),capabilities:this.options.capabilities}));this.heartbeat=setInterval(()=>{this.socket?.send(JSON.stringify({type:"heartbeat",at:new Date().toISOString(),accounts:this.store.accounts().map((account)=>({accountId:account.id,status:account.status,queueDepth:0}))}));this.flush();},10_000);this.flush();});
    this.socket.on("message",(data)=>void this.handle(JSON.parse(data.toString())));
    this.socket.on("close",()=>{clearInterval(this.heartbeat);this.options.onStatus("offline");if(!this.stopped)setTimeout(()=>this.connect(),this.nextDelay());});
  ...
  private async handle(frame:Record<string,unknown>):Promise<void>{
    ...
    if(frame.type==="account_reassigned"){const action=frame.action==="add"?"add":"remove";this.options.onAccountReassigned({accountId:String(frame.accountId??""),accountName:String(frame.accountName??"WhatsApp"),action});return;}
    ...
    try{result=await this.options.onCommand(command);}catch(error){result={type:"command_result",sequence:command.sequence,commandId:command.commandId,outcome:"uncertain",errorCode:"executor_interrupted",errorMessage:error instanceof Error?error.message:String(error),completedAt:new Date().toISOString()};}
