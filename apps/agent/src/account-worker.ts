...
async function resolveUsernameJid(activeSocket:ReturnType<typeof makeWASocket>,username:string):Promise<string>{const result=await activeSocket.executeUSyncQuery(new USyncQuery().withContext("message").withContactProtocol().withUsernameProtocol().withUser(new USyncUser().withUsername(username)));const resolved=jidNormalizedUser(String(result?.list?.find(item=>{const jid=jidNormalizedUser(String(item?.id??""));return jid.endsWith("@s.whatsapp.net")||jid.endsWith("@lid");})?.id??""));if(resolved.endsWith("@s.whatsapp.net"))return resolved;if(resolved.endsWith("@lid")){const mapped=await activeSocket.signalRepository.lidMapping.getPNForLID(resolved);const phoneJid=jidNormalizedUser(mapped??"");if(phoneJid.endsWith("@s.whatsapp.net"))return phoneJid;return resolved;}throw new Error("WhatsApp username not found");}
...
async function connect(options:Init):Promise<void>{
  ...
  chatEphemeralSettings.clear();chatJidAliases.clear();
  ...
  activeSocket.ev.on("messages.upsert",({messages,type})=>{if(generation!==connectionGeneration)return;void (async()=>{
    for(const item of messages){
      ...
      const remoteJidUsername=!item.key.fromMe?String(item.key.remoteJidUsername??"").trim().replace(/^@/,"").toLowerCase()||undefined:undefined;
      const participantUsername=!item.key.fromMe?String(item.key.participantUsername??"").trim().replace(/^@/,"").toLowerCase()||undefined:undefined;
      emit({type:"event",kind:"message",live:type==="notify",payload:{eventId:`message:${options.accountId}:${item.key.id}`,accountId:options.accountId,whatsappMessageId:item.key.id,chatJid:jid,rawChatJid:rawJid,chatType:isGroup?"group":"direct",senderJid,senderName:remotePushName,senderUsername:remoteJidUsername??participantUsername,direction:item.key.fromMe?"out":"in",kind,text,adReferral,quotedWhatsappMessageId,quotedParticipantJid,occurredAt:messageTime(item.messageTimestamp),media}});
    }
...
function rememberChatEphemeralExpirations(chats:ReadonlyArray<{id?:string|null;ephemeralExpiration?:number|null;ephemeralSettingTimestamp?:number|{toNumber():number}|null;disappearingMode?:proto.IDisappearingMode|null}>):void{
  for(const chat of chats){
    ...
    const settingTimestamp=Number(chat.ephemeralSettingTimestamp??0);
    if(Number.isFinite(expiration)&&expiration>0){const setting:ChatEphemeralSetting={expiration,...(Number.isFinite(settingTimestamp)&&settingTimestamp>0?{settingTimestamp}:{}),...(chat.disappearingMode?{disappearingMode:chat.disappearingMode}:{})};chatEphemeralSettings.set(jid,setting);if(alias)chatEphemeralSettings.set(alias,setting);}else{chatEphemeralSettings.delete(jid);if(alias)chatEphemeralSettings.delete(alias);}
  }
}
...
function rememberChatJidAlias(first:string,second:string):void{
  ...
  const setting=chatEphemeralSettings.get(left)??chatEphemeralSettings.get(right);
  if(setting){chatEphemeralSettings.set(left,setting);chatEphemeralSettings.set(right,setting);}
}
...
async function execute(command:Command):Promise<void>{
  ...
  try{
    ...
    const ephemeralSetting=chatEphemeralSettings.get(jidNormalizedUser(toJid));
    if(ephemeralSetting)content={...content,contextInfo:{...(content as {contextInfo?:proto.IContextInfo}).contextInfo,expiration:ephemeralSetting.expiration,...(ephemeralSetting.settingTimestamp?{ephemeralSettingTimestamp:ephemeralSetting.settingTimestamp}:{}),...(ephemeralSetting.disappearingMode?{disappearingMode:ephemeralSetting.disappearingMode}:{})}} as AnyMessageContent;
    const sendOptions={...(quoted?{quoted}:{}),...(ephemeralSetting?{ephemeralExpiration:ephemeralSetting.expiration}:{})};
